...
This document covers how to use and develop ZannaIDE as it exists today.

ZannaIDE is frame-driven and project-oriented. Most workflows begin with a
...
When a project opens, ZannaIDE does not immediately index every source file for
...
If the project has a `zanna.project` file, ZannaIDE reads project name, entry,
...
ZannaIDE tracks one `Document` object per open path. The editor widget displays
...
BASIC workspace navigation uses ZannaIDE's in-process semantic scanner. It scans
...
Before launching build/debug flows, ZannaIDE can save modified files according
...
This is expected. ZannaIDE tracks the scene document kind but does not currently
