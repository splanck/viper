...
Current ZannaIDE-related CTest entries are registered in
`src/tests/CMakeLists.txt`.

| Test | Purpose | Labels |
| --- | --- | --- |
| `zia_smoke_zannaide` | Compile/run smoke for `src/probes/smoke_probe.zia`. | `zia;zannaide;smoke;requires_display` |
| `zia_smoke_zannaide_project_compile` | Build the `zannaide/` project to IL. | `zia;zannaide` |
| `zia_zannaide_phase0_phase1` | Core document/project/command/location/session/language-service regression coverage. | `zia;zannaide;phase0;phase1` |
| `zia_zannaide_phase2_phase3` | Build/run jobs, project entry execution, breakpoints, debug boundary, scene data contracts. | `zia;zannaide;phase2;phase3` |
| `zia_zannaide_editor_hot_path` | Editor revision/performance hot-path guard. | `zia;zannaide;editor;requires_display;perf` |
| `zia_zannaide_intellisense` | Completion, diagnostics, hover, signature, and IntelliSense UI behavior. | `zia;zannaide;intellisense;requires_display` |
| `zia_zannaide_file_tree` | Explorer interactions and file-tree workflows. | `zia;zannaide;file-tree;requires_display` |
| `zia_zannaide_activity_bar` | Activity bar and workbench visibility behavior. | `zia;zannaide;activity-bar;requires_display` |
| `zia_zannaide_multi_root` | Multi-root workspace behavior. | `zia;zannaide;multi-root;requires_display` |
| `zia_zannaide_scm` | Git Source Control command layer, async job pump, spaces/rename/conflict parsing, and view model smoke. | `zia;zannaide;scm` |
| `zia_zannaide_terminal` | Terminal session/controller non-display behavior. | `zia;zannaide;terminal` |
| `zia_zannaide_terminal_open` | Terminal panel open/start behavior. | `zia;zannaide;terminal;requires_display` |
| `zia_zannaide_terminal_hidden_start` | Terminal hidden/open lifecycle and hidden-output draining behavior. | `zia;zannaide;terminal;requires_display` |
| `zia_zannaide_terminal_render` | Terminal rendering path. | `zia;zannaide;terminal;requires_display` |
| `zia_zannaide_context_menu` | Context menu routing and enabled state. | `zia;zannaide;context-menu;requires_display` |
| `zia_zannaide_syntax_render` | Syntax rendering path. | `zia;zannaide;syntax;requires_display` |
| `zia_zannaide_formatting` | Formatting commands and helpers. | `zia;zannaide;format` |
| `zia_zannaide_debug` | VM-backed debug adapter integration. | `zia;zannaide;debug` |
| `zia_zannaide_semantic_tokens` | Semantic token rendering behavior. | `zia;zannaide;semantic;requires_display` |
| `zia_zannaide_console_search` | Output panel helpers, console behavior, docked search panel, workspace-symbol discovery, and Quick Open ranking. | `zia;zannaide;console;search;requires_display` |
| `native_smoke_zannaide_completion_arm64` | Native completion link/e2e smoke on arm64 when enabled. | native/e2e labels from CMake |
| `native_smoke_zannaide_completion_x64` | Native completion link/e2e smoke on x64 when enabled. | native/e2e labels from CMake |
...
Compile the ZannaIDE project:

```sh
ctest --test-dir build -R zia_smoke_zannaide_project_compile --output-on-failure
```

Run all ZannaIDE-labelled tests:

```sh
ctest --test-dir build -L zannaide --output-on-failure
```

Run non-display ZannaIDE tests in a headless environment:

```sh
ctest --test-dir build -L zannaide -LE requires_display --output-on-failure
```

Run display-dependent editor and UI probes:

```sh
ctest --test-dir build -R 'zia_zannaide_(editor_hot_path|intellisense|file_tree|activity_bar|console_search)' --output-on-failure
```
...
```sh
ctest --test-dir build -R zia_zannaide_scm --output-on-failure
```
...
ZannaIDE probes live in `zannaide/src/probes/`.
...
Set `ZANNAIDE_PERF_LOG` before launching the IDE to write frame/controller and
