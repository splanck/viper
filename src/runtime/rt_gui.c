...
int64_t rt_shortcuts_was_triggered(rt_string id) {
    if (!g_shortcuts_global_enabled) return 0;
...
void* rt_menubar_get_menu(void* menubar, int64_t index) {
    ...
    if (index < 0 || index >= mb->menu_count) return NULL;
...
rt_string rt_menu_get_title(void* menu) {
    if (!menu) return rt_str_empty();
    const char* title = ((vg_menu_t*)menu)->title;
    if (!title) return rt_str_empty();
...
void* rt_menu_get_item(void* menu, int64_t index) {
    if (!menu) return NULL;
    vg_menu_t* m = (vg_menu_t*)menu;
    if (index < 0 || index >= m->item_count) return NULL;
...
rt_string rt_menuitem_get_shortcut(void* item) {
    ...
    if (!shortcut) return rt_str_empty();
...
void* rt_contextmenu_add_item(void* menu, rt_string text) {
    if (!menu) return NULL;
    char* ctext = rt_string_to_cstr(text);
    vg_menu_item_t* item = vg_contextmenu_add_item((vg_contextmenu_t*)menu, ctext, NULL, NULL, NULL);
    free(ctext);
    return item;
}
...
void* rt_contextmenu_add_item_with_shortcut(void* menu, rt_string text, rt_string shortcut) {
    ...
    char* ctext = rt_string_to_cstr(text);
    char* cshortcut = rt_string_to_cstr(shortcut);
    vg_menu_item_t* item = vg_contextmenu_add_item((vg_contextmenu_t*)menu, ctext, cshortcut, NULL, NULL);
    free(ctext);
    free(cshortcut);
    return item;
}
...
void* rt_statusbar_add_text(void* bar, rt_string text, int64_t zone) {
    if (!bar) return NULL;
    char* ctext = rt_string_to_cstr(text);
    vg_statusbar_item_t* item = vg_statusbar_add_text((vg_statusbar_t*)bar,
                                                       (vg_statusbar_zone_t)zone, ctext);
    free(ctext);
    return item;
}
...
void* rt_statusbar_add_button(void* bar, rt_string text, int64_t zone) {
    if (!bar) return NULL;
    char* ctext = rt_string_to_cstr(text);
    vg_statusbar_item_t* item = vg_statusbar_add_button((vg_statusbar_t*)bar,
                                                         (vg_statusbar_zone_t)zone, ctext, NULL, NULL);
    free(ctext);
    return item;
}
...
void* rt_toolbar_add_button(void* toolbar, rt_string icon_path, rt_string tooltip) {
    ...
    vg_toolbar_item_t* item = vg_toolbar_add_button((vg_toolbar_t*)toolbar,
...
void* rt_toolbar_add_toggle(void* toolbar, rt_string icon_path, rt_string tooltip) {
    ...
    vg_toolbar_item_t* item = vg_toolbar_add_toggle((vg_toolbar_t*)toolbar,
                                                     NULL, NULL, icon, false, NULL, NULL);
    if (item) {
        vg_toolbar_item_set_tooltip(item, ctooltip);
    }
...
void* rt_toolbar_add_dropdown(void* toolbar, rt_string tooltip) {
    ...
    vg_toolbar_item_t* item = vg_toolbar_add_dropdown((vg_toolbar_t*)toolbar,
                                                       NULL, NULL, icon, NULL);
    if (item) {
        vg_toolbar_item_set_tooltip(item, ctooltip);
    }
...
int64_t rt_toolbar_get_icon_size(void* toolbar) {
    if (!toolbar) return RT_TOOLBAR_ICON_MEDIUM;
...
void* rt_toolbar_get_item(void* toolbar, int64_t index) {
    ...
    if (index < 0 || index >= (int64_t)tb->item_count) return NULL;
...
int64_t rt_codeeditor_get_show_line_numbers(void* editor) {
    if (!editor) return 1;  // Default to showing
...
int64_t rt_codeeditor_get_cursor_line_at(void* editor, int64_t index) {
    if (!editor) return 0;
    if (index != 0) return 0;  // Only primary cursor supported
...
void rt_codeeditor_set_cursor_position_at(void* editor, int64_t index, int64_t line, int64_t col) {
    if (!editor) return;
    if (index != 0) return;  // Only primary cursor supported
    vg_codeeditor_set_cursor((vg_codeeditor_t*)editor, (int)line, (int)col);
}
...
int64_t rt_messagebox_warning(rt_string title, rt_string message) {
    char* ctitle = rt_string_to_cstr(title);
    char* cmsg = rt_string_to_cstr(message);
    vg_dialog_t* dlg = vg_dialog_message(ctitle, cmsg, VG_DIALOG_ICON_WARNING, VG_DIALOG_BUTTONS_OK);
    if (ctitle) free(ctitle);
    if (cmsg) free(cmsg);
    if (!dlg) return 0;
    vg_dialog_show(dlg);
...
int64_t rt_messagebox_question(rt_string title, rt_string message) {
    char* ctitle = rt_string_to_cstr(title);
    char* cmsg = rt_string_to_cstr(message);
    vg_dialog_t* dlg = vg_dialog_message(ctitle, cmsg, VG_DIALOG_ICON_QUESTION, VG_DIALOG_BUTTONS_YES_NO);
    if (ctitle) free(ctitle);
    if (cmsg) free(cmsg);
    if (!dlg) return 0;
    vg_dialog_show(dlg);
...
int64_t rt_messagebox_confirm(rt_string title, rt_string message) {
    char* ctitle = rt_string_to_cstr(title);
    char* cmsg = rt_string_to_cstr(message);
    vg_dialog_t* dlg = vg_dialog_message(ctitle, cmsg, VG_DIALOG_ICON_QUESTION, VG_DIALOG_BUTTONS_OK_CANCEL);
    if (ctitle) free(ctitle);
    if (cmsg) free(cmsg);
    if (!dlg) return 0;
    vg_dialog_show(dlg);
...
void* rt_messagebox_new(rt_string title, rt_string message, int64_t type) {
    ...
    switch (type) {
        case RT_MESSAGEBOX_INFO: icon = VG_DIALOG_ICON_INFO; break;
        case RT_MESSAGEBOX_WARNING: icon = VG_DIALOG_ICON_WARNING; break;
        case RT_MESSAGEBOX_ERROR: icon = VG_DIALOG_ICON_ERROR; break;
        case RT_MESSAGEBOX_QUESTION: icon = VG_DIALOG_ICON_QUESTION; break;
    }
...
void* rt_filedialog_new(int64_t type) {
    ...
    switch (type) {
        case RT_FILEDIALOG_OPEN: mode = VG_FILEDIALOG_OPEN; break;
        case RT_FILEDIALOG_SAVE: mode = VG_FILEDIALOG_SAVE; break;
        case RT_FILEDIALOG_FOLDER: mode = VG_FILEDIALOG_SELECT_FOLDER; break;
        default: mode = VG_FILEDIALOG_OPEN; break;
    }
...
rt_string rt_filedialog_get_path(void* dialog) {
    if (!dialog) return rt_string_from_bytes("", 0);
    rt_filedialog_data_t* data = (rt_filedialog_data_t*)dialog;
...
rt_string rt_filedialog_get_path_at(void* dialog, int64_t index) {
    ...
    if (data->selected_paths && index >= 0 && (size_t)index < data->selected_count) {
        return rt_string_from_bytes(data->selected_paths[index], strlen(data->selected_paths[index]));
    }
    return rt_string_from_bytes("", 0);
}
...
void rt_findbar_destroy(void* bar) {
    ...
    if (data->find_text) free(data->find_text);
    if (data->replace_text) free(data->replace_text);
    free(data);
}
...
rt_string rt_findbar_get_find_text(void* bar) {
    if (!bar) return rt_string_from_bytes("", 0);
    rt_findbar_data_t* data = (rt_findbar_data_t*)bar;
    if (data->find_text) {
...
static void rt_commandpalette_on_execute(vg_commandpalette_t* palette, vg_command_t* cmd, void* user_data) {
    rt_commandpalette_data_t* data = (rt_commandpalette_data_t*)user_data;
    if (data && cmd && cmd->id) {
        if (data->selected_command) free(data->selected_command);
...
void* rt_commandpalette_new(void* parent) {
    ...
    rt_commandpalette_data_t* data = (rt_commandpalette_data_t*)malloc(sizeof(rt_commandpalette_data_t));
    if (!data) {
...
void rt_commandpalette_add_command_with_shortcut(void* palette, rt_string id, rt_string label,
                                                  rt_string category, rt_string shortcut) {
    if (!palette) return;
    rt_commandpalette_data_t* data = (rt_commandpalette_data_t*)palette;
    char* cid = rt_string_to_cstr(id);
    char* clabel = rt_string_to_cstr(label);
...
rt_string rt_commandpalette_get_selected_command(void* palette) {
    if (!palette) return rt_string_from_bytes("", 0);
    rt_commandpalette_data_t* data = (rt_commandpalette_data_t*)palette;
...
static vg_notification_type_t rt_toast_type_to_vg(int64_t type) {
    switch (type) {
        case RT_TOAST_INFO:    return VG_NOTIFICATION_INFO;
        case RT_TOAST_SUCCESS: return VG_NOTIFICATION_SUCCESS;
        case RT_TOAST_WARNING: return VG_NOTIFICATION_WARNING;
        case RT_TOAST_ERROR:   return VG_NOTIFICATION_ERROR;
        default:               return VG_NOTIFICATION_INFO;
    }
}

static vg_notification_position_t rt_toast_position_to_vg(int64_t position) {
    switch (position) {
        case RT_TOAST_POSITION_TOP_RIGHT:      return VG_NOTIFICATION_TOP_RIGHT;
        case RT_TOAST_POSITION_TOP_LEFT:       return VG_NOTIFICATION_TOP_LEFT;
        case RT_TOAST_POSITION_BOTTOM_RIGHT:   return VG_NOTIFICATION_BOTTOM_RIGHT;
        case RT_TOAST_POSITION_BOTTOM_LEFT:    return VG_NOTIFICATION_BOTTOM_LEFT;
        case RT_TOAST_POSITION_TOP_CENTER:     return VG_NOTIFICATION_TOP_CENTER;
        case RT_TOAST_POSITION_BOTTOM_CENTER:  return VG_NOTIFICATION_BOTTOM_CENTER;
        default:                               return VG_NOTIFICATION_TOP_RIGHT;
    }
}
...
void* rt_toast_new(rt_string message, int64_t type, int64_t duration_ms) {
    ...
    data->id = vg_notification_show(mgr, rt_toast_type_to_vg(type),
                                     NULL, cmsg, (uint32_t)duration_ms);
...
void rt_breadcrumb_destroy(void* crumb) {
    ...
    if (data->clicked_data) free(data->clicked_data);
    free(data);
}
...
rt_string rt_breadcrumb_get_clicked_data(void* crumb) {
    if (!crumb) return rt_string_from_bytes("", 0);
    rt_breadcrumb_data_t* data = (rt_breadcrumb_data_t*)crumb;
    if (data->clicked_data) {
...
rt_string rt_widget_get_drop_type(void* widget) {
    if (!widget) return rt_string_from_bytes("", 0);
    // Note: Would need to extend vg_widget to support drag/drop
    return rt_string_from_bytes("", 0);
}
