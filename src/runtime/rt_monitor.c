...
//===----------------------------------------------------------------------===//
// ViperDOS Monitor Implementation
//===----------------------------------------------------------------------===//
// TODO: ViperDOS - implement using ViperDOS threading primitives when available.
// For now, provide a single-threaded stub that traps on contention.
// This allows single-threaded programs to work while multi-threaded programs
...
/// @brief Minimal monitor for single-threaded ViperDOS programs.
typedef struct RtMonitor
{
    int owner_valid;  ///< Non-zero if monitor is currently owned.
    size_t recursion; ///< Re-entry count.
} RtMonitor;

typedef struct RtMonitorEntry
{
    void *key;
    struct RtMonitorEntry *next;
    RtMonitor monitor;
} RtMonitorEntry;

#define RT_MONITOR_BUCKETS 256u
static RtMonitorEntry *g_monitor_table[RT_MONITOR_BUCKETS];
...
void rt_monitor_enter(void *obj)
{
    if (!obj)
    {
        rt_trap("Monitor.Enter: object is null");
        return;
    }
    RtMonitor *m = get_monitor_for(obj);
    if (m->owner_valid)
    {
        // Single-threaded: re-entry is OK
        m->recursion++;
    }
    else
    {
...
int rt_monitor_try_enter(void *obj)
{
    if (!obj)
        return 0;
...
int rt_monitor_try_enter_for(void *obj, int64_t timeout_ms)
{
    (void)timeout_ms;
    return rt_monitor_try_enter(obj);
}
...
void rt_monitor_exit(void *obj)
{
    if (!obj)
    {
        rt_trap("Monitor.Exit: object is null");
        return;
    }
    RtMonitor *m = get_monitor_for(obj);
    if (!m->owner_valid)
    {
        rt_trap("Monitor.Exit: not owned");
        return;
    }
    if (--m->recursion == 0)
    {
...
void rt_monitor_wait(void *obj)
{
    // Single-threaded: Wait would block forever
    rt_trap("Monitor.Wait: not supported in single-threaded ViperDOS mode");
}

int rt_monitor_wait_for(void *obj, int64_t timeout_ms)
{
    (void)obj;
    (void)timeout_ms;
    rt_trap("Monitor.WaitFor: not supported in single-threaded ViperDOS mode");
    return 0;
}

void rt_monitor_pause(void *obj)
{
    // Single-threaded: Pause is a no-op (no waiters)
    (void)obj;
}

void rt_monitor_pause_all(void *obj)
{
    // Single-threaded: PauseAll is a no-op (no waiters)
    (void)obj;
}
