...
// Purpose: Implements runtime string allocation, slicing, and search helpers.
// Error policy: Functions call rt_trap on invalid inputs and never rely on errno.
// Ownership: Functions returning rt_string allocate or retain references; callers must release.
