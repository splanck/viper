...
static inline uint64_t rt_fnv1a(const void *data, size_t len)
{
    ...
    size_t remain = len & 7;
    if (remain >= 7) last |= ((uint64_t)tail[6]) << 48;
    if (remain >= 6) last |= ((uint64_t)tail[5]) << 40;
    if (remain >= 5) last |= ((uint64_t)tail[4]) << 32;
    if (remain >= 4) last |= ((uint64_t)tail[3]) << 24;
    if (remain >= 3) last |= ((uint64_t)tail[2]) << 16;
    if (remain >= 2) last |= ((uint64_t)tail[1]) << 8;
    if (remain >= 1) last |= ((uint64_t)tail[0]);
