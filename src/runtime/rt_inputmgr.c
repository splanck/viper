...
int8_t rt_inputmgr_up(rt_inputmgr mgr)
{
    ...
    for (int64_t i = 0; i < 4; i++) {
        if (rt_pad_is_connected(i)) {
            if (rt_pad_is_down(i, VIPER_PAD_UP)) return 1;
            if (rt_pad_left_y(i) < -0.5) return 1;
        }
    }
...
int8_t rt_inputmgr_down(rt_inputmgr mgr)
{
    ...
    for (int64_t i = 0; i < 4; i++) {
        if (rt_pad_is_connected(i)) {
            if (rt_pad_is_down(i, VIPER_PAD_DOWN)) return 1;
            if (rt_pad_left_y(i) > 0.5) return 1;
        }
    }
...
int8_t rt_inputmgr_left(rt_inputmgr mgr)
{
    ...
    for (int64_t i = 0; i < 4; i++) {
        if (rt_pad_is_connected(i)) {
            if (rt_pad_is_down(i, VIPER_PAD_LEFT)) return 1;
            if (rt_pad_left_x(i) < -0.5) return 1;
        }
    }
...
int8_t rt_inputmgr_right(rt_inputmgr mgr)
{
    ...
    for (int64_t i = 0; i < 4; i++) {
        if (rt_pad_is_connected(i)) {
            if (rt_pad_is_down(i, VIPER_PAD_RIGHT)) return 1;
            if (rt_pad_left_x(i) > 0.5) return 1;
        }
    }
...
double rt_inputmgr_axis_x(rt_inputmgr mgr)
{
    ...
    for (int64_t i = 0; i < 4; i++) {
        if (rt_pad_is_connected(i)) {
            ...
            // D-pad
            if (rt_pad_is_down(i, VIPER_PAD_LEFT) && value > -1.0) value = -1.0;
            if (rt_pad_is_down(i, VIPER_PAD_RIGHT) && value < 1.0) value = 1.0;
        }
    }
...
double rt_inputmgr_axis_y(rt_inputmgr mgr)
{
    ...
    for (int64_t i = 0; i < 4; i++) {
        if (rt_pad_is_connected(i)) {
            ...
            if (rt_pad_is_down(i, VIPER_PAD_UP) && value > -1.0) value = -1.0;
            if (rt_pad_is_down(i, VIPER_PAD_DOWN) && value < 1.0) value = 1.0;
        }
    }
