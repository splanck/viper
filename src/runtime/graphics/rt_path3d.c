...
void *rt_path3d_new(void)
{
    ...
    if (!p) { rt_trap("Path3D.New: allocation failed"); return NULL; }
    p->vptr = NULL;
...
static int32_t path_idx(const rt_path3d *p, int32_t i)
{
    ...
    if (i < 0) return 0;
    if (i >= p->point_count) return p->point_count - 1;
    return i;
}
...
void *rt_path3d_get_position_at(void *obj, double t)
{
    ...
    rt_path3d *p = (rt_path3d *)obj;
    if (p->point_count < 2) {
        if (p->point_count == 1) return rt_vec3_new(p->xs[0], p->ys[0], p->zs[0]);
