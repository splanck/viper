                                ...
                                const float center[3], float radius)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        float dist = f->planes[i][0] * center[0] +
                     f->planes[i][1] * center[1] +
                     f->planes[i][2] * center[2] +
                     f->planes[i][3];
        if (dist < -radius)
                            ...
                            float out_min[3], float out_max[3])
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        float wx = (float)(world_matrix[0] * cx + world_matrix[1] * cy +
                           world_matrix[2] * cz + world_matrix[3]);
        float wy = (float)(world_matrix[4] * cx + world_matrix[5] * cy +
                           world_matrix[6] * cz + world_matrix[7]);
        float wz = (float)(world_matrix[8] * cx + world_matrix[9] * cy +
                               ...
                               float out_min[3], float out_max[3])
{
    ...
    for (uint32_t i = 0; i < vertex_count; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            if (pos[j] < out_min[j]) out_min[j] = pos[j];
            if (pos[j] > out_max[j]) out_max[j] = pos[j];
        }
    }
}
