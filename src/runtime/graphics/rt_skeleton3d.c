...
void *rt_skeleton3d_new(void)
{
    ...
    if (!s) { rt_trap("Skeleton3D.New: memory allocation failed"); return NULL; }
    s->vptr = NULL;
    s->bones = NULL;
    s->bone_count = 0;
...
int64_t rt_skeleton3d_add_bone(void *obj, rt_string name, int64_t parent_index, void *bind_mat4)
{
    ...
    vgfx3d_bone_t *nb = (vgfx3d_bone_t *)realloc(s->bones,
                                                    (size_t)new_count * sizeof(vgfx3d_bone_t));
    if (!nb) return -1;
    s->bones = nb;
...
rt_string rt_skeleton3d_get_bone_name(void *obj, int64_t index)
{
    ...
    rt_skeleton3d *s = (rt_skeleton3d *)obj;
    if (index < 0 || index >= s->bone_count) return rt_const_cstr("");
...
void *rt_skeleton3d_get_bone_bind_pose(void *obj, int64_t index)
{
    if (!obj) return NULL;
    rt_skeleton3d *s = (rt_skeleton3d *)obj;
    if (index < 0 || index >= s->bone_count) return NULL;
...
void *rt_animation3d_new(rt_string name, double duration)
{
    ...
    if (!a) { rt_trap("Animation3D.New: memory allocation failed"); return NULL; }
    a->vptr = NULL;
    memset(a->name, 0, 64);
    if (name)
    {
...
void rt_animation3d_add_keyframe(void *obj, int64_t bone_index, double time,
...
{
    if (obj) ((rt_animation3d *)obj)->looping = loop;
}
...
static void sample_channel(const vgfx3d_anim_channel_t *ch, float t, float *out_local)
{
    ...
    {
        build_trs_float(ch->keyframes[0].position, ch->keyframes[0].rotation,
                        ch->keyframes[0].scale_xyz, out_local);
        return;
    }
...
{
    if (!skeleton) { rt_trap("AnimPlayer3D.New: null skeleton"); return NULL; }
    rt_skeleton3d *skel = (rt_skeleton3d *)skeleton;

    rt_anim_player3d *p = (rt_anim_player3d *)rt_obj_new_i64(
        0, (int64_t)sizeof(rt_anim_player3d));
    if (!p) { rt_trap("AnimPlayer3D.New: memory allocation failed"); return NULL; }
    p->vptr = NULL;
    p->skeleton = skel;
    p->current = NULL;
...
static void compute_bone_palette(rt_anim_player3d *p)
{
    ...
    if (p->current)
    {
        for (int32_t c = 0; c < p->current->channel_count; c++)
        {
            ...
            if (bone >= 0 && bone < skel->bone_count)
                sample_channel(&p->current->channels[c], p->current_time,
    ...
    if (p->crossfade_from && p->crossfade_duration > 0.0f)
    {
        ...
        for (int32_t c = 0; c < p->crossfade_from->channel_count; c++)
        {
            ...
            if (bone < 0 || bone >= skel->bone_count) continue;
            sample_channel(&p->crossfade_from->channels[c], p->crossfade_from_time,
                           from_local);
...
{
    if (obj) ((rt_anim_player3d *)obj)->speed = (float)speed;
}
...
{
    if (obj) ((rt_anim_player3d *)obj)->current_time = (float)time;
}
...
void *rt_anim_player3d_get_bone_matrix(void *obj, int64_t bone_index)
{
    if (!obj) return NULL;
    rt_anim_player3d *p = (rt_anim_player3d *)obj;
    if (bone_index < 0 || bone_index >= p->skeleton->bone_count) return NULL;
...
void rt_mesh3d_set_bone_weights(void *obj, int64_t vertex_index,
                                 int64_t b0, double w0, int64_t b1, double w1,
                                 int64_t b2, double w2, int64_t b3, double w3)
{
    if (!obj) return;
    rt_mesh3d *m = (rt_mesh3d *)obj;
    if (vertex_index < 0 || vertex_index >= m->vertex_count) return;
...
{
    if (!canvas || !mesh || !transform || !material || !anim_player) return;

    rt_mesh3d *m = (rt_mesh3d *)mesh;
    rt_anim_player3d *p = (rt_anim_player3d *)anim_player;

    if (m->vertex_count == 0 || !p->bone_palette) return;
    ...
    /* Apply CPU skinning */
    vgfx3d_skin_vertices(m->vertices, skinned, m->vertex_count,
                          p->bone_palette, p->skeleton->bone_count);
