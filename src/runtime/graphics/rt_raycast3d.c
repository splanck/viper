...
double rt_ray3d_intersect_triangle(void *origin, void *dir,
                                    void *v0_obj, void *v1_obj, void *v2_obj)
{
    if (!origin || !dir || !v0_obj || !v1_obj || !v2_obj) return -1.0;
    ...
    if (fabs(det) < EPSILON) return -1.0; /* parallel */
...
{
    if (!origin || !dir || !aabb_min || !aabb_max) return -1.0;
    ...
    for (int i = 0; i < 3; i++)
    {
        if (fabs(d[i]) < EPSILON)
        {
            if (o[i] < mn[i] || o[i] > mx[i]) return -1.0;
        }
        else
        {
            inv[i] = 1.0 / d[i];
...
{
    if (!origin || !dir || !center) return -1.0;
...
void *rt_ray3d_intersect_mesh(void *origin, void *dir,
                               void *mesh_obj, void *transform_obj)
{
    if (!origin || !dir || !mesh_obj) return NULL;
    ...
    for (uint32_t i = 0; i + 2 < m->index_count; i += 3)
    {
        ...
        double ax = m->vertices[i0].pos[0], ay = m->vertices[i0].pos[1], az = m->vertices[i0].pos[2];
        double bx = m->vertices[i1].pos[0], by = m->vertices[i1].pos[1], bz = m->vertices[i1].pos[2];
        double cx = m->vertices[i2].pos[0], cy = m->vertices[i2].pos[1], cz = m->vertices[i2].pos[2];
        ...
        if (t >= 0.0 && t < best_t)
        {
            ...
            double nlen = sqrt(best_normal[0] * best_normal[0] +
...
{
    if (!min_a || !max_a || !min_b || !max_b) return 0;
...
{
    if (!min_a || !max_a || !min_b || !max_b) return rt_vec3_new(0, 0, 0);
...
{
    if (!hit) return rt_vec3_new(0, 0, 0);
...
{
    if (!hit) return rt_vec3_new(0, 1, 0);
    rt_rayhit3d *h = (rt_rayhit3d *)hit;
