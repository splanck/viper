...
void rt_virtual_tree_add_node(void *tree, rt_string parent_id, rt_string id_s, rt_string text) {
    ...
    if (id.empty())
        return;
    if (!h->state->nodes.count(parent))
        h->state->nodes[parent] = TreeNode{parent, parent, "", {}, false, true};
    h->state->nodes[id] = TreeNode{id, toStd(text), parent, {}, false, true};
    auto &children = h->state->nodes[parent].children;
    if (std::find(children.begin(), children.end(), id) == children.end())
        children.push_back(id);
    h->state->nodes[parent].loaded = true;
}
...
void *rt_virtual_tree_visible_rows(void *tree) {
    ...
    for (const auto &id : h->state->nodes[""].children)
