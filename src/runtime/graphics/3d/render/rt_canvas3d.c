...
static int canvas3d_sort_tuple_after(float key,
    ...
    if (previous_enqueue_index < 0)
        return 1;
    if (key == previous_key)
        return stable_sort_id == previous_stable_sort_id ? enqueue_index > previous_enqueue_index
...
/// @brief Return non-zero when a live Pixels object contains at least one non-opaque texel.
/// @details Used only for draw classification. The scan is conservative: invalid handles and
///          empty buffers return 0, while any alpha byte below 255 promotes the material to an
...
///          assets without exact metadata are treated as alpha-bearing until metadata proves them
///          opaque, avoiding one-frame cutout/depth pops while native texture metadata catches up.
static int canvas3d_texture_source_has_alpha(void *texture_ref) {
    void *pixels;

    if (!texture_ref)
        return 0;
    if (canvas3d_pixels_has_alpha_texels(texture_ref))
        return 1;
...
static void canvas3d_fill_material_cmd(const rt_material3d *mat, vgfx3d_draw_cmd_t *cmd) {
    ...
    cmd->has_alpha_texture = canvas3d_texture_source_has_alpha(mat->texture) ? 1 : 0;
...
static void canvas3d_fill_deferred_draw(rt_canvas3d *c,
    ...
    dd->stable_sort_id = (uintptr_t)cmd->geometry_key ^ (uintptr_t)cmd->texture ^
                         ((uintptr_t)cmd->texture_asset << 1) ^ ((uintptr_t)instance_matrices << 2);
...
static void canvas3d_queue_instanced_fallback(const canvas3d_instanced_batch_t *b,
    ...
    for (int32_t i = 0; i < instance_count; i++) {
        ...
        }
        {
            ...
            queued->occlusion_key =
                queued->stable_sort_id ? queued->stable_sort_id : (uintptr_t)per_instance.vertices;
            queued->occlusion_key = canvas3d_occlusion_draw_key(c, queued, c->draw_count - 1);
...
void rt_canvas3d_queue_instanced_batch_bounds(void *canvas_obj,
    ...
    if (rt_heap_is_payload(mesh_obj)) {
        if (!canvas3d_track_draw_resource(c, mesh_obj, batch.new_refs, &batch.new_ref_count, 16))
            return;
    }
    if (!canvas3d_track_draw_resource(c, material_obj, batch.new_refs, &batch.new_ref_count, 16)) {
        canvas3d_instanced_release_refs(&batch);
        return;
    }
    if (!canvas3d_track_material_resources(c, mat, batch.new_refs, &batch.new_ref_count, 16)) {
        canvas3d_instanced_release_refs(&batch);
        return;
    }
...
/// @brief Render the directional/cascaded shadow-map pass for the queued 3D draws.
...
static void canvas3d_render_shadow_pass(rt_canvas3d *c, deferred_draw_t *cmds) {
    ...
    if (c->shadows_enabled && c->backend->shadow_begin && c->backend->shadow_draw &&
        ...
        int32_t selected_shadow_count = canvas3d_select_shadow_directional_lights_from_draws(
            cmds, c->draw_count, selected_lights, VGFX3D_MAX_SHADOW_LIGHTS);
        float shadow_world_min[3];
        float shadow_world_max[3];
        int has_shadow_bounds = canvas3d_build_shadow_world_bounds(
            cmds, c->draw_count, shadow_world_min, shadow_world_max);

        if (has_shadow_bounds && selected_shadow_count > 0 && c->shadow_cascade_count > 1) {
            vgfx3d_light_params_t primary_light = selected_lights[0];
            int32_t cascade_count = c->shadow_cascade_count;
            float cascade_splits[VGFX3D_MAX_SHADOW_LIGHTS];
            float cascade_near = 0.0f;
            float cascade_far = 0.0f;

            if (cascade_count > VGFX3D_MAX_SHADOW_LIGHTS)
                cascade_count = VGFX3D_MAX_SHADOW_LIGHTS;
