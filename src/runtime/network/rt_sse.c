...
static void rt_sse_finalize(void *obj)
{
    ...
    if (sse->tcp) rt_tcp_close(sse->tcp);
...
{
    *host = NULL; *port = 80; *path = NULL;
    if (strncmp(url, "http://", 7) == 0) url += 7;
    else if (strncmp(url, "https://", 8) == 0) { url += 8; *port = 443; }
    else return 0;
    ...
    *host = (char *)malloc(hlen + 1);
    ...
    if (*end == ':') { *port = atoi(end + 1); while (*end && *end != '/') end++; }
    *path = strdup(*end == '/' ? end : "/");
    return 1;
}
...
void *rt_sse_connect(rt_string url)
{
    ...
    if (!url_str) rt_trap("SSE: NULL URL");
    ...
    while (1)
    {
        ...
        if (!l || *l == '\0') { rt_string_unref(line); break; }
        rt_string_unref(line);
    }

    rt_sse_impl *sse = (rt_sse_impl *)rt_obj_new_i64(0, (int64_t)sizeof(rt_sse_impl));
    if (!sse) { rt_tcp_close(tcp); rt_trap("SSE: OOM"); return NULL; }
...
rt_string rt_sse_recv(void *obj)
{
    ...
    rt_sse_impl *sse = (rt_sse_impl *)obj;
    if (!sse->is_open || !sse->tcp) return rt_string_from_bytes("", 0);

    // Accumulate "data:" lines until a blank line (event boundary)
    size_t cap = 4096, len = 0;
    char *data_buf = (char *)malloc(cap);
    if (!data_buf) return rt_string_from_bytes("", 0);

    while (rt_tcp_is_open(sse->tcp))
    {
        rt_string line = rt_tcp_recv_line(sse->tcp);
        if (!line) { sse->is_open = false; break; }
        const char *l = rt_string_cstr(line);
        if (!l) { rt_string_unref(line); break; }
        ...
        if (strncmp(l, "data:", 5) == 0)
        {
            ...
            if (len + vlen + 2 > cap)
            {
                ...
                if (!nb) { rt_string_unref(line); break; }
                data_buf = nb;
            }
            if (len > 0) data_buf[len++] = '\n'; // Multi-line data separated by \n
...
rt_string rt_sse_recv_for(void *obj, int64_t timeout_ms)
{
    if (!obj) return rt_string_from_bytes("", 0);
    rt_sse_impl *sse = (rt_sse_impl *)obj;
    if (!sse->is_open) return rt_string_from_bytes("", 0);
...
void rt_sse_close(void *obj)
{
    ...
    rt_sse_impl *sse = (rt_sse_impl *)obj;
    sse->is_open = false;
    if (sse->tcp) { rt_tcp_close(sse->tcp); sse->tcp = NULL; }
}
