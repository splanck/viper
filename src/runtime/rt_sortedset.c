...
int8_t rt_sortedset_put(void *obj, rt_string str)
{
    ...
    if (found) return 0;  // Already present
...
{
    rt_sortedset set = (rt_sortedset)obj;
    if (!set || set->len == 0) return rt_const_cstr("");
    return set->data[0];
}
...
rt_string rt_sortedset_floor(void *obj, rt_string str)
{
    ...
    if (found) return set->data[idx];
    if (idx == 0) return rt_const_cstr("");
...
rt_string rt_sortedset_ceil(void *obj, rt_string str)
{
    ...
    if (found) return set->data[idx];
    if (idx >= set->len) return rt_const_cstr("");
    return set->data[idx];
}
...
{
    rt_sortedset set = (rt_sortedset)obj;
    if (!set || index < 0 || index >= set->len) return rt_const_cstr("");
...
void *rt_sortedset_range(void *obj, rt_string from, rt_string to)
{
    ...
    for (int64_t i = start; i < set->len; i++) {
        if (compare_strings(set->data[i], to) >= 0) break;
...
int8_t rt_sortedset_is_subset(void *obj, void *other)
{
    ...
    if (!a || a->len == 0) return 1;  // Empty is subset of everything
    if (!b) return 0;  // Non-empty can't be subset of empty
