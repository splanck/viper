...
//===----------------------------------------------------------------------===//
//
// File: src/frontends/basic/parse/ParseStmt_Decl.cpp
// Purpose: Implements declaration and core statement parselets for the BASIC
//          parser, including LET and procedure forms.
// Key invariants: Maintains procedure registry for known call targets and
//                 honours BASIC type suffix semantics when inferring types.
// Ownership/Lifetime: Parser owns the produced AST nodes through unique_ptr.
...
il::support::SourceLoc Parser::parseProcedureBody(TokenKind endKind, std::vector<StmtPtr> &body)
{
    ...
    auto info = ctx.collectStatements(
        ...
        [&](int, il::support::SourceLoc, StatementSequencer::TerminatorInfo &info) {
            consume();
            consume();
        },
        body);
