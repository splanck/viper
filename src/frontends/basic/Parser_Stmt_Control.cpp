...
// Purpose: Implements control-flow statement parsing helpers for the BASIC parser.
// Key invariants: Keeps parser token stream and separator handling in sync with StatementSequencer.
// Ownership/Lifetime: Parser retains ownership of AST nodes built by these helpers.
...
StmtPtr Parser::parseIfStatement(int line)
{
    ...
    if (at(TokenKind::EndOfLine))
    {
            ...
            -> std::pair<StmtPtr, BlockTerminator>
        {
            ...
            ctxIf.collectStatements(predicate, consumer, stmts);
...
StmtPtr Parser::parseSelectCaseStatement()
{
    ...
    while (!at(TokenKind::EndOfFile))
    {
        ...
        if (peek(1).kind == TokenKind::KeywordElse)
        {
            ...
            bodyCtx.collectStatements(predicate, consumer, elseBody);
            if (!sawCaseElse)
            {
