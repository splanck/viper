...
///
/// @details The rule-driven path first checks whether the builtin is part of
///          the dedicated string registry.  If so, it delegates to
///          `builtins::LowerCtx`.  Otherwise, it evaluates each lowering variant
///          in priority order, matching on presence, absence, or types of
///          arguments.  Once a variant is chosen the helper applies argument
///          transforms (coercions, constant adjustments, etc.), emits the IL for
///          the variant kind, and records any runtime feature usage.  Missing
///          variants result in a conservative zero literal.
///
