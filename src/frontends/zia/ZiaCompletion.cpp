...
static CompletionKind kindFromSymbol(const Symbol &sym)
{
    switch (sym.kind)
    {
    case Symbol::Kind::Variable:  return CompletionKind::Variable;
    case Symbol::Kind::Parameter: return CompletionKind::Parameter;
    case Symbol::Kind::Function:  return CompletionKind::Function;
    case Symbol::Kind::Method:    return CompletionKind::Method;
    ...
    case Symbol::Kind::Type:      return CompletionKind::Entity;
    case Symbol::Kind::Module:    return CompletionKind::Module;
    }
...
static const char *const kKeywords[] = {
    // Statement keywords
    "var", "func", "if", "else", "while", "for", "in", "return",
    "break", "continue", "and", "or", "not", "is", "as", "new",
    "true", "false", "null", "match",
    // Declaration keywords
    "entity", "interface", "value", "expose", "module", "bind",
    // Built-in types
    "Integer", "Number", "Boolean", "String", "Byte", "Bytes",
    "List", "Map", "Set", "Object",
    nullptr,
};
                                                            ...
                                                            int              col) const
{
    ...
    for (int i = static_cast<int>(lineUpToCursor.size()) - 1;
         i >= 0 && isIdentChar(lineUpToCursor[i]); --i)
    {
        ++prefixLen;
    }
...
CompletionEngine::provideKeywords(const std::string &prefix) const
{
...
std::vector<CompletionItem>
CompletionEngine::provideSnippets(const std::string &prefix) const
{
    std::vector<CompletionItem> items;
...
std::vector<CompletionItem>
CompletionEngine::provideScopeSymbols(const Sema        &sema,
...
std::vector<CompletionItem>
CompletionEngine::provideMemberCompletions(const Sema    &sema,
...
std::vector<CompletionItem>
CompletionEngine::provideTypeNames(const Sema        &sema,
                                    const std::string &prefix) const
{
    std::vector<CompletionItem> items;
...
std::vector<CompletionItem>
CompletionEngine::provideModuleMembers(const Sema        &sema,
...
std::vector<CompletionItem>
CompletionEngine::provideRuntimeMembers(const Sema        &sema,
...
std::vector<CompletionItem>
CompletionEngine::provideNamespaceMembers(const Sema        &sema,
...
void CompletionEngine::rank(std::vector<CompletionItem> &items,
                             const std::string           &prefix) const
{
    if (prefix.empty())
    {
    ...
    auto score = [&](const CompletionItem &item) -> int {
        if (item.label == prefix)
            return 0;
        if (item.label.size() >= prefix.size() &&
            item.label.substr(0, prefix.size()) == prefix)
            return 1;
        return 2;
    };
    ...
    case TriggerKind::AfterColon:
    {
        ...
        for (auto &kw : kws)
        {
            ...
            static const char *const typeKws[] = {
                "Integer", "Number", "Boolean", "String", "Byte", "Bytes",
                "List", "Map", "Set", "Object", nullptr,
            };
            bool isType = false;
            for (int i = 0; typeKws[i]; ++i)
                if (kw.label == typeKws[i]) { isType = true; break; }
            if (isType)
