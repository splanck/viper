...
class LowererSymbolTable
{
  public:
    ...
    void defineLocal(const std::string &name, Value value) { locals_[name] = value; }
    Value *lookupLocal(const std::string &name)
    {
        auto it = locals_.find(name);
        return it != locals_.end() ? &it->second : nullptr;
    }
    const std::unordered_map<std::string, Value> &locals() const { return locals_; }
    std::unordered_map<std::string, Value> &locals() { return locals_; }
    void clearLocals() { locals_.clear(); }

    void setLocalType(const std::string &name, TypeRef type) { localTypes_[name] = type; }
    ...
    const std::unordered_map<std::string, TypeRef> &localTypes() const { return localTypes_; }
    std::unordered_map<std::string, TypeRef> &localTypes() { return localTypes_; }
    void clearLocalTypes() { localTypes_.clear(); }

    void registerSlot(const std::string &name, Value slot) { slots_[name] = slot; }
    Value *lookupSlot(const std::string &name)
    {
        auto it = slots_.find(name);
        return it != slots_.end() ? &it->second : nullptr;
    }
    void removeSlot(const std::string &name) { slots_.erase(name); }
    const std::unordered_map<std::string, Value> &slots() const { return slots_; }
    std::unordered_map<std::string, Value> &slots() { return slots_; }
    void clearSlots() { slots_.clear(); }

    void defineGlobalConstant(const std::string &name, Value value) { globalConstants_[name] = value; }
    ...
    bool hasGlobalConstant(const std::string &name) const { return globalConstants_.count(name) > 0; }
    std::unordered_map<std::string, Value> &globalConstants() { return globalConstants_; }
    const std::unordered_map<std::string, Value> &globalConstants() const { return globalConstants_; }

    void defineGlobalVariable(const std::string &name, TypeRef type) { globalVariables_[name] = type; }
    ...
    std::unordered_map<std::string, TypeRef> &globalVariables() { return globalVariables_; }
    const std::unordered_map<std::string, TypeRef> &globalVariables() const { return globalVariables_; }

    void defineGlobalInitializer(const std::string &name, Value value) { globalInitializers_[name] = value; }
    ...
    std::unordered_map<std::string, Value> &globalInitializers() { return globalInitializers_; }
    const std::unordered_map<std::string, Value> &globalInitializers() const { return globalInitializers_; }
