...
This document covers how to use and develop ViperIDE as it exists today.

ViperIDE is frame-driven and project-oriented. Most workflows begin with a
...
When a project opens, ViperIDE does not immediately index every source file for
...
If the project has a `viper.project` file, ViperIDE reads project name, entry,
...
ViperIDE tracks one `Document` object per open path. The editor widget displays
...
Build and run use `Viper.System.Process` and argument vectors.

Default project behavior:

```text
viper build --diagnostic-format=json <project-root>
viper run --diagnostic-format=json <project-root>
```

Default single-file behavior:

```text
viper build --diagnostic-format=json <active-file>
viper run --diagnostic-format=json <active-file>
```

The IDE resolves the `viper` binary in this order:
...
Before launching build/debug flows, ViperIDE can save modified files according
...
```text
viper run --debug-adapter <active-file>
```
...
### Build says it cannot find `viper`

Set `VIPER_BINARY` to the full path of the desired `viper` executable or add the
binary to `PATH`.
...
This is expected. ViperIDE tracks the scene document kind but does not currently
include a visual scene editor.

### Terminal is unavailable

The runtime PTY layer may be unavailable on the platform or unable to launch the
configured shell. The terminal panel reports `Viper.System.Pty.LastError()` when
startup fails.
