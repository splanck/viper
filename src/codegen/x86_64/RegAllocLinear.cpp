...
public:
  LinearScanAllocator(MFunction& func, const TargetInfo& target)
      : func_(func), target_(target) {
    buildPools();
  }
...
private:
  ...
  void buildPools() {
    ...
    freeGPR_.erase(std::remove_if(freeGPR_.begin(), freeGPR_.end(),
  ...
  void handleOperand(Operand& operand, const OperandRole& role,
    ...
    std::visit(Overload{[&](OpReg& reg) {
                         ...
                         [&](OpMem& mem) {
                           ...
                           processRegOperand(mem.base, baseRole, prefix, suffix,
                                             scratch);
                         },
                         [](auto&) {}},
               operand);
  ...
  void processRegOperand(OpReg& reg, const OperandRole& role,
                         std::vector<MInstr>& prefix,
                         std::vector<MInstr>& suffix,
                         std::vector<ScratchRelease>& scratch) {
    if (reg.isPhys) {
  ...
  void handleParallelCopy(const MInstr& instr, std::vector<MInstr>& out) {
    ...
    while (!tasks.empty()) {
      ...
      for (auto& pending : tasks) {
        if (pending.src.kind == CopySource::Kind::Reg &&
            pending.src.reg == srcReg) {
