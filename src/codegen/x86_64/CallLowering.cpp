...
[[nodiscard]] bool isI1Value(const MBasicBlock& block, std::size_t searchLimit,
                             uint16_t vreg) {
...
void lowerCall(MBasicBlock& block, std::size_t insertIdx,
  ...
  for (const auto& arg : plan.args) {
    const auto currentIdx = static_cast<std::size_t>(
        std::distance(block.instructions.begin(), insertIt));
      ...
      if (gprUsed < kGprArgLimit) {
        ...
        if (arg.isImm) {
          insertInstr(MInstr::make(MOpcode::MOVri,
                                   {makePhysOperand(RegClass::GPR, destReg),
                                    makeImmOperand(arg.imm)}));
        } else {
    ...
    case CallArg::XMM: {
      if (xmmUsed < kXmmArgLimit) {
        ...
        if (arg.isImm) {
          // Phase A backend does not yet support immediate materialisation into XMM args.
          const Operand scratchGpr = makePhysOperand(RegClass::GPR, kScratchGPR);
          insertInstr(MInstr::make(MOpcode::MOVri,
                                   {scratchGpr, makeImmOperand(arg.imm)}));
