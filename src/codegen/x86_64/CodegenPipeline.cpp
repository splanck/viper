...
// Key invariants: Pipeline stages run sequentially with early exits on error, ensuring diagnostics
//                 are recorded deterministically and no partial artefacts leak on failure.
// Ownership/Lifetime: The pipeline borrows IL modules and writes assembly/binaries to the
//                     caller-specified locations without assuming ownership of external resources.
...
PipelineResult CodegenPipeline::run()
{
    ...
    const ILModule adapted = convertToAdapterModule(module);
    const CodegenResult cgResult = emitModuleToAssembly(adapted, {});
    if (!cgResult.errors.empty())
    {
        err << "error: x64 codegen failed:\n" << cgResult.errors << "\n";
        result.exit_code = 1;
        result.stdout_text = out.str();
        result.stderr_text = err.str();
        return result;
    ...
    if (!writeAssemblyFile(asmPath, cgResult.asmText, err))
    {
        result.exit_code = 1;
        result.stdout_text = out.str();
        result.stderr_text = err.str();
