...
void insertPrologueEpilogue(MFunction& func, const TargetInfo& target, const FrameInfo& frame) {
  ...
  prologue.push_back(
      MInstr::make(MOpcode::MOVrr, {makeMemOperand(rspBase, 0), rbpOperand}));
  ...
  for (std::size_t idx = 0; idx < frame.usedCalleeSaved.size(); ++idx) {
    ...
    prologue.push_back(MInstr::make(MOpcode::MOVrr,
                                    {makeMemOperand(rbpBase, offset),
                                     makePhysOperand(RegClass::GPR, reg)}));
  }
  ...
  for (std::size_t idx = frame.usedCalleeSaved.size(); idx > 0; --idx) {
    ...
    epilogue.push_back(MInstr::make(MOpcode::MOVrr,
                                    {makePhysOperand(RegClass::GPR, reg),
                                     makeMemOperand(rbpBase, offset)}));
  }
  ...
  epilogue.push_back(
      MInstr::make(MOpcode::MOVrr, {rbpOperand, makeMemOperand(rspBase, 0)}));
