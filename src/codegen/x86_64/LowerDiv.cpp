...
[[nodiscard]] Operand cloneOperand(const Operand& operand) { return operand; }
...
void lowerSignedDivRem(MFunction& fn) {
  ...
  auto ensureTrapBlock = [&]() -> std::size_t {
    ...
    if (auto existing = findBlockIndex(fn, trapLabel)) {
      ...
      const bool hasCall = std::any_of(trapBlock.instructions.begin(),
      ...
      if (!hasCall) {
        trapBlock.append(MInstr::make(
            MOpcode::CALL,
            std::vector<Operand>{makeLabelOperand("rt_trap_div0")}));
      }
      return *trapIndex;
    }
  ...
  for (std::size_t blockIdx = 0; blockIdx < fn.blocks.size(); ++blockIdx) {
    ...
    for (std::size_t instrIdx = 0; instrIdx < block.instructions.size(); ++instrIdx) {
      ...
      if (candidate.opcode != MOpcode::DIVS64rr &&
          candidate.opcode != MOpcode::REMS64rr) {
        continue;
      }
      ...
      const bool dividendSupported =
          std::holds_alternative<OpReg>(dividendOp) ||
      ...
      block.append(MInstr::make(MOpcode::TESTrr,
                                std::vector<Operand>{cloneOperand(divisorClone),
                                                     cloneOperand(divisorClone)}));
      ...
      if (std::holds_alternative<OpImm>(dividendClone)) {
        block.append(MInstr::make(MOpcode::MOVri,
                                  std::vector<Operand>{cloneOperand(raxOp),
      ...
      block.append(MInstr::make(MOpcode::IDIVrm,
                                std::vector<Operand>{cloneOperand(divisorClone)}));

      const Operand resultPhys = isDiv ? raxOp : rdxOp;
      block.append(MInstr::make(MOpcode::MOVrr,
                                std::vector<Operand>{cloneOperand(destOp),
                                                     cloneOperand(resultPhys)}));
