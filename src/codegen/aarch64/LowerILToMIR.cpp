...
MFunction LowerILToMIR::lowerFunction(const il::core::Function &fn) const
{
    ...
    if (!fn.blocks.empty())
    {
        ...
        if (fn.blocks.size() == 1 && bb.instructions.size() >= 2 && !bb.params.empty())
        {
            ...
            const auto &retI = bb.instructions.back();
            // call @callee(args...) feeding ret: marshal params/consts (and one temp) into x0..x7,
            // then bl
                    ...
                    if (nargs <= ti_->intArgOrder.size())
                    {
                        ...
                        else
                        {
                            ...
                            if (nStackArgs > 0)
                            {
                                ...
                                bbMir.instrs.push_back(
                                    MInstr{MOpcode::SubSpImm, {MOperand::immOp(frameBytes)}});
                                ...
                                bbMir.instrs.push_back(
                                    MInstr{MOpcode::AddSpImm, {MOperand::immOp(frameBytes)}});
                                return mf;
                            }
