...
inline std::optional<RtComponent> componentForRuntimeSymbol(std::string_view sym)
{
    ...
    if (starts("rt_obj_") || starts("rt_type_") || starts("rt_cast_") || starts("rt_ns_") ||
        starts("rt_box_") || starts("rt_exc_") || starts("rt_result_") ||
    ...
    if (starts("rt_list_") || starts("rt_map_") || starts("rt_treemap_") || starts("rt_bag_") ||
        ...
        starts("rt_convert_") ||
        starts("rt_statemachine_") || starts("rt_tween_") || starts("rt_buttongroup_") ||
        starts("rt_particle_") || starts("rt_spriteanim_") || starts("rt_collision_") ||
        starts("rt_objpool_") || starts("rt_screenfx_") || starts("rt_pathfollow_") ||
        starts("rt_quadtree_"))
    ...
    if (starts("rt_file_") || starts("rt_dir_") || starts("rt_path_") ||
        ...
        starts("rt_io_file_") || starts("rt_memstream_") || starts("rt_stream_") ||
        starts("rt_watcher_") || starts("rt_compress_") || starts("rt_archive_") ||
        starts("rt_glob_") || starts("rt_tempfile_") ||
        sym == "rt_eof_ch" || sym == "rt_lof_ch" ||
    ...
    if (starts("rt_canvas_") || starts("rt_color_") || starts("rt_vec2_") ||
        starts("rt_vec3_") || starts("rt_pixels_") || starts("rt_sprite_") ||
        starts("rt_spritebatch_") || starts("rt_tilemap_") || starts("rt_camera_") ||
        starts("rt_scene_") || starts("rt_font_") || starts("rt_gui_") ||
        starts("rt_checkbox_") || starts("rt_codeeditor_") || starts("rt_widget_") ||
        starts("rt_treeview_") || starts("rt_radiobutton_") || starts("rt_menuitem_") ||
        starts("rt_contextmenu_") || starts("rt_statusbar_") || starts("rt_toolbar_") ||
        starts("rt_findbar_") || starts("rt_commandpalette_") || starts("rt_scrollview_") ||
        starts("rt_action_") || starts("rt_input_") || starts("rt_inputmgr_") ||
        starts("rt_mat3_") || starts("rt_mat4_") || starts("rt_graphics_"))
    ...
    if (starts("rt_context_") || starts("rt_crc32_") || starts("rt_error_") ||
        starts("rt_trap_") || starts("rt_fp_") || starts("rt_memory_") ||
...
inline std::string_view archiveNameForComponent(RtComponent comp)
{
    switch (comp)
    {
        case RtComponent::Base:        return "viper_rt_base";
        case RtComponent::Arrays:      return "viper_rt_arrays";
        case RtComponent::Oop:         return "viper_rt_oop";
        case RtComponent::Collections: return "viper_rt_collections";
        case RtComponent::Text:        return "viper_rt_text";
        case RtComponent::IoFs:        return "viper_rt_io_fs";
        case RtComponent::Exec:        return "viper_rt_exec";
        case RtComponent::Threads:     return "viper_rt_threads";
        case RtComponent::Graphics:    return "viper_rt_graphics";
        case RtComponent::Audio:       return "viper_rt_audio";
        case RtComponent::Network:     return "viper_rt_network";
    }
...
inline std::vector<RtComponent> resolveRequiredComponents(const SymbolRange &symbols)
{
    ...
    if (has(RtComponent::Text) || has(RtComponent::IoFs) ||
        has(RtComponent::Exec) || has(RtComponent::Network))
        add(RtComponent::Collections);
    if (has(RtComponent::Collections))
        add(RtComponent::Arrays);
    if (has(RtComponent::Collections) || has(RtComponent::Arrays) ||
        has(RtComponent::Graphics) || has(RtComponent::Threads) ||
        has(RtComponent::Audio) || has(RtComponent::Network))
        add(RtComponent::Oop);
    ...
    static constexpr RtComponent order[] = {
        RtComponent::Oop, RtComponent::Arrays, RtComponent::Collections,
        RtComponent::Text, RtComponent::IoFs, RtComponent::Exec,
        RtComponent::Threads, RtComponent::Graphics, RtComponent::Audio,
        RtComponent::Network,
    };
    for (auto c : order)
    {
