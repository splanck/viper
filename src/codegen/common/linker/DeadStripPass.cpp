...
// Key invariants:
//   - All user .o sections are unconditionally live (roots)
               ...
               std::ostream &err)
{
    ...
    // All user .o sections are roots (not from archives).
    for (size_t oi = 0; oi < userObjCount && oi < allObjects.size(); ++oi)
    {
        for (size_t si = 1; si < allObjects[oi].sections.size(); ++si)
            markLive(oi, si);
    }
    ...
    // Always-live sections in archive objects (ObjC, TLS, init/fini).
    for (size_t oi = userObjCount; oi < allObjects.size(); ++oi)
    {
        const auto &obj = allObjects[oi];
        for (size_t si = 1; si < obj.sections.size(); ++si)
