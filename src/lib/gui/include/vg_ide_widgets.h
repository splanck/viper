...
typedef struct vg_filedialog {
    ...
    void (*on_select)(struct vg_filedialog* dialog,
        char** paths, size_t count, void* user_data); ///< Selection callback
...
typedef struct vg_contextmenu {
    ...
    void (*on_select)(struct vg_contextmenu* menu,
        vg_menu_item_t* item, void* user_data); ///< Selection callback
...
typedef bool (*vg_tree_can_drop_callback_t)(vg_tree_node_t* source, vg_tree_node_t* target,
    vg_tree_drop_position_t position, void* user_data);
typedef void (*vg_tree_on_drop_callback_t)(vg_tree_node_t* source, vg_tree_node_t* target,
    vg_tree_drop_position_t position, void* user_data);

/// @brief Lazy loading callback type
typedef void (*vg_tree_load_children_callback_t)(struct vg_treeview* tree, vg_tree_node_t* node, void* user_data);
...
void vg_tooltip_set_timing(vg_tooltip_t* tooltip, uint32_t show_delay_ms,
    uint32_t hide_delay_ms, uint32_t duration_ms);
...
vg_command_t* vg_commandpalette_add_command(vg_commandpalette_t* palette,
    const char* id, const char* label, const char* shortcut,
    void (*action)(vg_command_t*, void*), void* user_data);
...
void vg_commandpalette_set_callbacks(vg_commandpalette_t* palette,
    void (*on_execute)(vg_commandpalette_t*, vg_command_t*, void*),
    void (*on_dismiss)(vg_commandpalette_t*, void*), void* user_data);
...
typedef enum vg_ansi_color {
    ...
    VG_ANSI_BRIGHT_BLACK = 90, VG_ANSI_BRIGHT_RED, VG_ANSI_BRIGHT_GREEN,
    VG_ANSI_BRIGHT_YELLOW, VG_ANSI_BRIGHT_BLUE, VG_ANSI_BRIGHT_MAGENTA,
    VG_ANSI_BRIGHT_CYAN, VG_ANSI_BRIGHT_WHITE
} vg_ansi_color_t;
...
typedef struct vg_breadcrumb {
    ...
    void (*on_dropdown_select)(struct vg_breadcrumb* bc, int crumb_index,
