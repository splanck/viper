...
static void render_textinput(vgfx_window_t window, vg_textinput_t* input) {
    ...
    if ((input->base.state & VG_STATE_FOCUSED) && input->font) {
        ...
        if (input->text && input->cursor_pos > 0) {
            cursor_x += vg_font_get_cursor_x(input->font, input->font_size,
                                              input->text, (int)input->cursor_pos);
        }
        float cursor_y1 = sy + 4;
        float cursor_y2 = sy + sh - 4;
        vgfx_line(window, (int)cursor_x, (int)cursor_y1, (int)cursor_x, (int)cursor_y2,
...
static void render_checkbox(vgfx_window_t window, vg_checkbox_t* checkbox) {
    ...
    if (checkbox->font && checkbox->text) {
        ...
        vg_font_draw_text(window, checkbox->font, checkbox->font_size, tx, ty,
                          checkbox->text, theme->colors.fg_primary);
    }
}
...
static void render_demo(demo_state_t* demo) {
    ...
    if (!g_debug_printed && demo->font) {
        ...
        if (glyph) {
            printf("Glyph 'T': width=%d height=%d advance=%d bearing_x=%d bearing_y=%d bitmap=%p\n",
                   ...
                   glyph->bearing_x, glyph->bearing_y, (void*)glyph->bitmap);
        } else {
    ...
    if (demo->font) {
        ...
        vg_font_draw_text(window, demo->font, 24.0f, 250, 40,
                          "TEST", 0xFFFFFFFF);

        // Try a simpler approach - just draw at baseline
        vg_font_draw_text(window, demo->font, 16.0f, 250, 80,
                          "Hello", 0xFFFFFFFF);
    }

    // Draw title using theme color
    if (demo->font) {
        vg_font_draw_text(window, demo->font, 24.0f, 20, 130,
                          "ViperGUI Demo", theme->colors.fg_primary);
    }
    ...
    while (vgfx_poll_event(demo->window, &pe)) {
        ...
        if (event.type == VG_EVENT_MOUSE_DOWN) {
            ...
            if (mx >= bx && mx < bx + bw && my >= by && my < by + bh) {
                ...
                on_dark_mode_change(&demo->dark_mode_checkbox->base,
                                   demo->dark_mode_checkbox->checked, demo);
            }
        }
