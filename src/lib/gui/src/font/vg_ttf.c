...
static bool ttf_parse_cmap_format4(vg_font_t* font, const uint8_t* subtable) {
    ...
    if (!font->cmap4_end_codes || !font->cmap4_start_codes ||
        !font->cmap4_id_deltas || !font->cmap4_id_range_offsets) {
        return false;
    }
...
static bool ttf_parse_cmap_format12(vg_font_t* font, const uint8_t* subtable) {
    ...
    if (!font->cmap12_start_codes || !font->cmap12_end_codes ||
        !font->cmap12_start_glyph_ids) {
        return false;
    }
...
bool ttf_parse_name(vg_font_t* font, const uint8_t* data, uint32_t len) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (string_offset + offset + length > len) continue;
...
bool ttf_parse_tables(vg_font_t* font) {
    ...
    if (!table || !ttf_parse_head(font, table, len)) return false;

    table = ttf_find_table(font, TTF_TAG_HHEA, &len);
    if (!table || !ttf_parse_hhea(font, table, len)) return false;

    table = ttf_find_table(font, TTF_TAG_MAXP, &len);
    if (!table || !ttf_parse_maxp(font, table, len)) return false;

    table = ttf_find_table(font, TTF_TAG_CMAP, &len);
    if (!table || !ttf_parse_cmap(font, table, len)) return false;

    // Store offsets for tables we'll need later
    table = ttf_find_table(font, TTF_TAG_GLYF, &len);
    if (table) font->glyf_offset = (uint32_t)(table - font->data);

    table = ttf_find_table(font, TTF_TAG_LOCA, &len);
    if (table) font->loca_offset = (uint32_t)(table - font->data);

    table = ttf_find_table(font, TTF_TAG_HMTX, &len);
    if (table) font->hmtx_offset = (uint32_t)(table - font->data);
...
void ttf_get_h_metrics(vg_font_t* font, uint16_t glyph_id, int* advance_width, int* left_side_bearing) {
    ...
    if (glyph_id < font->hhea.num_h_metrics) {
        ...
        *advance_width = ttf_read_u16(hmtx + (font->hhea.num_h_metrics - 1) * 4);
        ...
        uint32_t lsb_offset = font->hhea.num_h_metrics * 4 +
                              (glyph_id - font->hhea.num_h_metrics) * 2;
...
static bool ttf_get_simple_glyph_outline(vg_font_t* font, const uint8_t* glyph_data,
...
static bool ttf_get_composite_glyph_outline(vg_font_t* font, const uint8_t* glyph_data,
                                             float** out_points_x, float** out_points_y,
                                             uint8_t** out_flags, int** out_contour_ends,
                                             int* out_num_points, int* out_num_contours) {
    ...
    do {
        ...
        if (component_glyph_id < font->maxp.num_glyphs) {
            ...
            if (comp_offset != comp_next) {
                ...
                if (comp_contour_count >= 0) {
                    ttf_get_simple_glyph_outline(font, comp_data, comp_contour_count,
                                                  &comp_x, &comp_y, &comp_flags,
                                                  &comp_contours, &comp_num_points, &comp_num_contours);
                }
        ...
        if (comp_num_points > 0) {
            ...
            all_contour_ends = realloc(all_contour_ends, (total_contours + comp_num_contours) * sizeof(int));
            ...
            if (all_points_x && all_points_y && all_flags && all_contour_ends) {
                ...
                memcpy(all_contour_ends + total_contours, comp_contours, comp_num_contours * sizeof(int));
...
bool ttf_get_glyph_outline(vg_font_t* font, uint16_t glyph_id,
                           float** out_points_x, float** out_points_y,
                           uint8_t** out_flags, int** out_contour_ends,
                           int* out_num_points, int* out_num_contours) {
    if (glyph_id >= font->maxp.num_glyphs) return false;
    ...
    if (num_contours < 0) {
        return ttf_get_composite_glyph_outline(font, glyph,
                                                out_points_x, out_points_y,
                                                out_flags, out_contour_ends,
                                                out_num_points, out_num_contours);
    }

    // Simple glyph - delegate to helper function
    return ttf_get_simple_glyph_outline(font, glyph, num_contours,
                                         out_points_x, out_points_y,
                                         out_flags, out_contour_ends,
                                         out_num_points, out_num_contours);
}
