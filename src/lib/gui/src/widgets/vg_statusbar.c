...
static vg_widget_vtable_t g_statusbar_vtable = {
    ...
    switch (item->type) {
        ...
        case VG_STATUSBAR_ITEM_BUTTON:
            if (sb->font && item->text) {
                ...
                if (item->min_width > 0 && width < item->min_width) width = item->min_width;
                if (item->max_width > 0 && width > item->max_width) width = item->max_width;
                return width;
            }
...
static void statusbar_paint(vg_widget_t* widget, void* canvas) {
    ...
    float text_y = widget->y + (widget->height - font_metrics.line_height) / 2.0f + font_metrics.ascent;
    ...
    for (size_t i = 0; i < sb->left_count; i++) {
        ...
        switch (item->type) {
            ...
            case VG_STATUSBAR_ITEM_BUTTON:
                if (item->text) {
                    vg_font_draw_text(canvas, sb->font, sb->font_size,
                                      x + sb->item_padding, text_y,
                                      item->text, sb->text_color);
                }
                break;
    ...
    for (size_t i = 0; i < sb->center_count; i++) {
        ...
        if (item->type == VG_STATUSBAR_ITEM_SPACER) continue;
...
static vg_statusbar_item_t* find_item_at(vg_statusbar_t* sb, float mouse_x) {
    ...
    for (size_t i = 0; i < sb->center_count; i++) {
        vg_statusbar_item_t* item = sb->center_items[i];
        if (!item->visible || item->type == VG_STATUSBAR_ITEM_SPACER) continue;
