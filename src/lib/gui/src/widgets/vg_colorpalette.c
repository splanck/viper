...
static vg_widget_vtable_t g_colorpalette_vtable = {
    .destroy = colorpalette_destroy,
...
static void colorpalette_measure(vg_widget_t* widget, float available_width, float available_height) {
    ...
    float total_width = palette->columns * palette->swatch_size +
                       (palette->columns - 1) * palette->gap;
    float total_height = rows * palette->swatch_size +
                        (rows - 1) * palette->gap;
