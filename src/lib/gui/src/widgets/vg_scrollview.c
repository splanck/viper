...
static vg_widget_vtable_t g_scrollview_vtable = {
...
static void clamp_scroll(vg_scrollview_t* scroll) {
    ...
    if (scroll->scroll_x > max_scroll_x) scroll->scroll_x = max_scroll_x;
    if (scroll->scroll_y > max_scroll_y) scroll->scroll_y = max_scroll_y;
}
...
static void scrollview_arrange(vg_widget_t* widget, float x, float y, float width, float height) {
    ...
    bool needs_h = (scroll->direction & VG_SCROLL_HORIZONTAL) &&
                   scroll->content_width > width;
    bool needs_v = (scroll->direction & VG_SCROLL_VERTICAL) &&
...
static void scrollview_paint(vg_widget_t* widget, void* canvas) {
    ...
    // Draw scrollbars
    float content_area_width = widget->width - (scroll->show_v_scrollbar ? scroll->scrollbar_width : 0);
    float content_area_height = widget->height - (scroll->show_h_scrollbar ? scroll->scrollbar_width : 0);
    ...
    if (scroll->show_v_scrollbar && scroll->content_height > content_area_height) {
        ...
        if (visible_ratio > 1.0f) visible_ratio = 1.0f;
        ...
        uint32_t thumb_color = scroll->v_scrollbar_hovered || scroll->v_scrollbar_dragging
            ? scroll->thumb_hover_color : scroll->thumb_color;
        // TODO: Use vgfx primitives
        (void)thumb_height;
        (void)thumb_y;
        (void)thumb_color;
...
static bool scrollview_handle_event(vg_widget_t* widget, vg_event_t* event) {
    ...
    if (event->type == VG_EVENT_MOUSE_MOVE) {
        ...
        scroll->v_scrollbar_hovered = scroll->show_v_scrollbar &&
                                      event->mouse.x >= widget->width - scroll->scrollbar_width;
...
void vg_scrollview_get_scroll(vg_scrollview_t* scroll, float* out_x, float* out_y) {
    ...
    if (out_x) *out_x = scroll->scroll_x;
    if (out_y) *out_y = scroll->scroll_y;
}
...
void vg_scrollview_scroll_to_widget(vg_scrollview_t* scroll, vg_widget_t* child) {
    ...
    while (p && p != base) p = p->parent;
    if (p != base) return;  // Not a descendant

    float content_area_width = base->width - (scroll->show_v_scrollbar ? scroll->scrollbar_width : 0);
    float content_area_height = base->height - (scroll->show_h_scrollbar ? scroll->scrollbar_width : 0);
