...
static void commandpalette_measure(vg_widget_t* widget, float available_width, float available_height);
...
static vg_widget_vtable_t g_commandpalette_vtable = {
    .destroy = commandpalette_destroy,
    ...
    for (size_t i = 0; i < palette->command_count; i++) {
        ...
        if (score > 0 || !query || !query[0]) {
            ...
            if (palette->filtered_count >= palette->filtered_capacity) {
                ...
                vg_command_t** new_filtered = realloc(palette->filtered, new_cap * sizeof(vg_command_t*));
                if (!new_filtered) continue;
...
static void commandpalette_measure(vg_widget_t* widget, float available_width, float available_height) {
    ...
    if (visible > palette->max_visible) visible = palette->max_visible;
...
static void commandpalette_paint(vg_widget_t* widget, void* canvas) {
    ...
    for (size_t i = 0; i < visible; i++) {
        ...
        if (palette->font && cmd->label) {
            vg_font_draw_text(canvas, palette->font, palette->font_size,
                widget->x + 12, y + 8, cmd->label, palette->text_color);

            if (cmd->shortcut) {
                // Draw shortcut right-aligned
                vg_font_draw_text(canvas, palette->font, palette->font_size,
                    widget->x + palette->width - 100, y + 8,
                    cmd->shortcut, palette->shortcut_color);
            }
        }
...
static bool commandpalette_handle_event(vg_widget_t* widget, vg_event_t* event) {
    ...
    if (!palette->is_visible) return false;
...
vg_command_t* vg_commandpalette_add_command(vg_commandpalette_t* palette,
    ...
    if (!palette || !id || !label) return NULL;
    ...
    if (shortcut) cmd->shortcut = strdup(shortcut);
    cmd->action = action;
...
void vg_commandpalette_execute_selected(vg_commandpalette_t* palette) {
    if (!palette) return;
    if (palette->selected_index < 0 || palette->selected_index >= (int)palette->filtered_count) return;
