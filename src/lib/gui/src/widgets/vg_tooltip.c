...
static vg_widget_vtable_t g_tooltip_vtable = {
...
static void tooltip_paint(vg_widget_t* widget, void* canvas) {
    ...
    if (!tooltip->is_visible || !tooltip->text) return;
    ...
    if (tooltip->font && tooltip->text[0]) {
        ...
        vg_font_draw_text(canvas, tooltip->font, tooltip->font_size,
            text_x, text_y, tooltip->text, tooltip->text_color);
    }
}
