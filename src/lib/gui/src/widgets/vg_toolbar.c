...
static vg_widget_vtable_t g_toolbar_vtable = {
...
static bool ensure_item_capacity(vg_toolbar_t* tb, size_t needed) {
    if (needed <= tb->item_capacity) return true;
...
static void free_item(vg_toolbar_item_t* item) {
    ...
    if (item->tooltip) free(item->tooltip);
...
static uint32_t get_icon_pixels(vg_toolbar_icon_size_t size) {
    switch (size) {
        case VG_TOOLBAR_ICONS_SMALL:  return ICON_SIZE_SMALL;
        case VG_TOOLBAR_ICONS_MEDIUM: return ICON_SIZE_MEDIUM;
        case VG_TOOLBAR_ICONS_LARGE:  return ICON_SIZE_LARGE;
        default: return ICON_SIZE_MEDIUM;
    }
}
...
static void toolbar_measure(vg_widget_t* widget, float available_width, float available_height) {
    ...
    if (tb->orientation == VG_TOOLBAR_HORIZONTAL) {
        ...
        widget->measured_height = total_height > 0 ? total_height - tb->item_spacing : available_height;
    }
}
        ...
        switch (item->type) {
            ...
            case VG_TOOLBAR_ITEM_DROPDOWN: {
                ...
                switch (item->icon.type) {
                    case VG_ICON_GLYPH:
                        ...
                        if (tb->font) {
                            ...
                            vg_font_draw_text(canvas, tb->font, (float)icon_px,
                                              icon_x, icon_y + icon_px * 0.8f,
                                              buf, tb->text_color);
                        }
                        break;
                ...
                if (item->show_label && item->label && tb->font) {
                    ...
                    vg_font_draw_text(canvas, tb->font, tb->font_size,
                                      label_x, label_y, item->label,
...
static vg_toolbar_item_t* find_item_at(vg_toolbar_t* tb, float px, float py) {
    ...
    for (int i = 0; i < max_index; i++) {
        ...
        if (px >= item_x && px < item_x + item_width &&
            py >= item_y && py < item_y + item_height) {
            if (item->type != VG_TOOLBAR_ITEM_SEPARATOR &&
                item->type != VG_TOOLBAR_ITEM_SPACER) {
                return item;
            }
        }
...
vg_toolbar_item_t* vg_toolbar_add_button(vg_toolbar_t* tb,
                                          ...
                                          void* user_data) {
    if (!tb) return NULL;
    if (!ensure_item_capacity(tb, tb->item_count + 1)) return NULL;
...
void vg_toolbar_remove_item(vg_toolbar_t* tb, const char* id) {
    ...
    for (size_t i = 0; i < tb->item_count; i++) {
        if (tb->items[i]->id && strcmp(tb->items[i]->id, id) == 0) {
            free_item(tb->items[i]);
            memmove(&tb->items[i], &tb->items[i + 1],
