...
static vg_widget_vtable_t g_breadcrumb_vtable = {
...
static void breadcrumb_measure(vg_widget_t* widget, float available_width, float available_height) {
    ...
    for (size_t i = 0; i < bc->item_count; i++) {
        ...
        if (metrics.height > height) height = metrics.height;
...
static void breadcrumb_paint(vg_widget_t* widget, void* canvas) {
    ...
    for (size_t i = 0; i < bc->item_count; i++) {
        ...
        // Draw text
        vg_font_draw_text(canvas, bc->font, bc->font_size,
            x + bc->item_padding, y, item->label, bc->text_color);
        ...
        if (i < bc->item_count - 1 && bc->separator) {
            ...
            vg_font_draw_text(canvas, bc->font, bc->font_size,
                x, y, bc->separator, bc->separator_color);
            vg_font_measure_text(bc->font, bc->font_size, bc->separator, &metrics);
...
void vg_breadcrumb_push(vg_breadcrumb_t* bc, const char* label, void* data) {
    ...
    if (bc->item_count >= bc->item_capacity) {
        ...
        vg_breadcrumb_item_t* new_items = realloc(bc->items, new_cap * sizeof(vg_breadcrumb_item_t));
        if (!new_items) return;
        bc->items = new_items;
...
void vg_breadcrumb_item_add_dropdown(vg_breadcrumb_item_t* item,
    ...
    if (item->dropdown_count >= item->dropdown_capacity) {
        ...
        vg_breadcrumb_dropdown_t* new_items = realloc(item->dropdown_items,
            new_cap * sizeof(vg_breadcrumb_dropdown_t));
        if (!new_items) return;
