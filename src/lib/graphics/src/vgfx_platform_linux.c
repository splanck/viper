...
static vgfx_key_t translate_keysym(KeySym keysym) {
    ...
    /* Special keys */
    switch (keysym) {
        case XK_space:      return VGFX_KEY_SPACE;
        case XK_Return:     return VGFX_KEY_ENTER;
        case XK_KP_Enter:   return VGFX_KEY_ENTER;
        case XK_Escape:     return VGFX_KEY_ESCAPE;
        case XK_Left:       return VGFX_KEY_LEFT;
        case XK_Right:      return VGFX_KEY_RIGHT;
        case XK_Up:         return VGFX_KEY_UP;
        case XK_Down:       return VGFX_KEY_DOWN;
...
int vgfx_platform_init_window(struct vgfx_window* win,
    ...
    x11->window = XCreateWindow(
        x11->display,
        root,
        0, 0,                           /* x, y position (will be overridden) */
...
int vgfx_platform_present(struct vgfx_window* win) {
    ...
    vgfx_x11_data* x11 = (vgfx_x11_data*)win->platform_data;
    if (!x11->display || !x11->window || !x11->ximage) return 0;
