...
int main(void) {
    ...
    if (!fb.pixels || fb.width != 320 || fb.height != 240 || fb.stride != 1280) {
        fprintf(stderr, "FAIL: Invalid framebuffer (pixels=%p, w=%d, h=%d, stride=%d)\n",
                (void*)fb.pixels, fb.width, fb.height, fb.stride);
        return 1;
    }
    ...
    if (pixel[0] != 0xFF || pixel[1] != 0xFF || pixel[2] != 0xFF || pixel[3] != 0xFF) {
        fprintf(stderr, "FAIL: Pixel at (10,10) expected RGBA(255,255,255,255), got (%d,%d,%d,%d)\n",
