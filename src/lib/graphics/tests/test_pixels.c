...
void test_circle_writes_respect_clip(void) {
    TEST_BEGIN("T5c: Circle Writes Respect Clip");
    ...
    vgfx_set_clip(win, 4, 4, 4, 4);
    vgfx_fill_circle(win, 5, 5, 4, 0xFF0000);

    vgfx_color_t color = 0xFFFFFF;
    ASSERT_EQ(vgfx_point(win, 5, 5, &color), 1);
    ASSERT_EQ(color, 0xFF0000);
    ASSERT_EQ(vgfx_point(win, 3, 5, &color), 1);
    ASSERT_EQ(color, 0x000000);
    ASSERT_EQ(vgfx_point(win, 5, 3, &color), 1);
    ASSERT_EQ(color, 0x000000);

    vgfx_cls(win, VGFX_BLACK);
    vgfx_set_clip(win, 4, 4, 4, 4);
    vgfx_circle(win, 5, 5, 1, 0x00FF00);

    ASSERT_EQ(vgfx_point(win, 5, 4, &color), 1);
    ASSERT_EQ(color, 0x00FF00);
    ASSERT_EQ(vgfx_point(win, 5, 3, &color), 1);
    ASSERT_EQ(color, 0x000000);
...
int main(void) {
    ...
    test_circle_writes_respect_clip();
    test_clear_screen();
