...
{
    if (tok == "i64") return Type::Kind::I64;
    if (tok == "f64") return Type::Kind::F64;
    if (tok == "i1") return Type::Kind::I1;
    if (tok == "str" || tok == "string") return Type::Kind::Str;
    if (tok == "obj" || tok == "ptr") return Type::Kind::Ptr;
    if (tok == "void") return Type::Kind::Void;
...
TEST(RuntimeClassCatalogTargets, AllTargetsResolveAndMatchArity)
{
    ...
    for (const auto &d : reg) map.emplace(std::string(d.name), &d);
    ...
    for (const auto &c : classes)
    {
        ...
        auto checkReceiverKind = [&](const RuntimeDescriptor &desc) {
            if (desc.signature.paramTypes.empty()) return; // don't over-report
        ...
        for (const auto &m : c.methods)
        {
            ...
            const auto *d = it->second;
            auto args = parseArgs(m.signature ? std::string_view(m.signature) : std::string_view(""));
