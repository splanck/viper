...
// Purpose: Ensure static calls to Viper.System.IO.File bind to canonical externs.
//===----------------------------------------------------------------------===//
...
TEST(RuntimeClassFileBinding, MethodIndexTargets)
{
    ...
    // Exists(path)
    auto a = midx.find("Viper.System.IO.File", "Exists", 1);
    ASSERT_TRUE(a.has_value());
    EXPECT_EQ(a->target, std::string("Viper.IO.File.Exists"));
    // ReadAllText(path)
    auto b = midx.find("Viper.System.IO.File", "ReadAllText", 1);
    ASSERT_TRUE(b.has_value());
    EXPECT_EQ(b->target, std::string("Viper.IO.File.ReadAllText"));
    // WriteAllText(path,contents)
    auto c = midx.find("Viper.System.IO.File", "WriteAllText", 2);
    ASSERT_TRUE(c.has_value());
    EXPECT_EQ(c->target, std::string("Viper.IO.File.WriteAllText"));
    // Delete(path)
    auto d = midx.find("Viper.System.IO.File", "Delete", 1);
