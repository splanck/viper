...
static void test_tracked_count() {
    ...
    if (rt_obj_release_check0(a)) rt_obj_free(a);
    if (rt_obj_release_check0(b)) rt_obj_free(b);
}
...
static void test_double_track() {
    ...
    rt_gc_untrack(obj);
    if (rt_obj_release_check0(obj)) rt_obj_free(obj);
}
...
static void test_collect_no_cycle() {
    ...
    if (rt_obj_release_check0(c)) rt_obj_free(c);
    if (rt_obj_release_check0(b)) rt_obj_free(b);
    if (rt_obj_release_check0(a)) rt_obj_free(a);
}
