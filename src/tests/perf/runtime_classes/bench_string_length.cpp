...
struct BenchOut { double ms; int64_t exitCode; };
...
std::string programLen(size_t iters)
{
    ...
    return
        ...
        "20 LET s = \"abcd\"\n"
        "30 LET x = 0\n"
        "40 FOR i = 1 TO " + std::to_string(iters) + "\n"
        "50 LET x = x + LEN(s)\n"
        "60 NEXT\n"
        "70 PRINT x\n";
}
...
int main(int argc, char **argv)
{
    ...
    if (argc >= 2) iters = static_cast<size_t>(std::strtoull(argv[1], nullptr, 10));

    auto a = runBasic(programLen(iters));
    auto b = runBasic(programProp(iters));

    std::cout << "bench_len_ms=" << a.ms << " bench_prop_ms=" << b.ms
              << " iters=" << iters << "\n";
    return 0;
}
