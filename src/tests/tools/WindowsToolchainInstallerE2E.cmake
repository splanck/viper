...
#
# Ownership/Lifetime: Test packages and installs stay under VIPER_BUILD_DIR.
#
...
set(_installer_repeat "${_tmp_root}/viper-toolchain-e2e-repeat.exe")
...
set(_preclean_script "${VIPER_BUILD_DIR}/tests/windows-toolchain-installer-e2e-preclean.ps1")
file(WRITE "${_preclean_script}"
        ...
        "$candidate = if ($arp -and (Test-Path -LiteralPath $arp.ViperMaintenanceCache -PathType Leaf)) { $arp.ViperMaintenanceCache } elseif (Test-Path -LiteralPath ([IO.Path]::Combine($root, 'uninstall.exe')) -PathType Leaf) { [IO.Path]::Combine($root, 'uninstall.exe') } else { $null }\n"
...
execute_process(
        COMMAND "${VIPER_BIN}" install-package --stage-dir "${_stage_dir}"
...
if (NOT _identity_preview_rv EQUAL 0 OR
   ...
   NOT _identity_preview_out MATCHES "Windows identifier: org.viper.toolchain.development" OR
   NOT _identity_preview_out MATCHES "Windows install directory: Viper development" OR
   NOT _identity_preview_out MATCHES "Windows display name: Viper Toolchain \\(development\\)")
    message(FATAL_ERROR
...
execute_process(
        COMMAND "${VIPER_BIN}" install-package --verify-only "${_installer}"
...
if (NOT _inspect_json_text MATCHES "\"schema_version\": 3" OR
   NOT _inspect_json_text MATCHES "\"identifier\": \"org.viper.toolchain.e2e\"" OR
   NOT _inspect_json_text MATCHES "\"display_name\": \"Viper Toolchain \\(e2e\\)\"" OR
   NOT _inspect_json_text MATCHES "\"channel\": \"e2e\"" OR
   NOT _inspect_json_text MATCHES "\"default_install_dir\": \"ViperInstallerE2E\"" OR
...
if (NOT _collision_log MATCHES "unowned file conflicts with the new Viper payload")
    message(FATAL_ERROR
...
_viper_installer_assert_exists("${_unowned_sentinel}" "unowned file during manifest recovery")
file(READ "${_setup_log}" _legacy_upgrade_log)
if (NOT _legacy_upgrade_log MATCHES "Migrating a verified Viper installation")
...
_viper_installer_assert_absent("${_start_menu}" "disabled Start Menu folder")
if (_staged_vsix_files)
...
endforeach ()
_viper_installer_assert_absent(
        "${_install_root}/bin/viper-installer-host.exe" "installer-only native host")
_viper_installer_assert_absent(
        "${_install_root}/bin/viper-installer-cleanup.exe" "installer-only cleanup helper")
...
file(TO_NATIVE_PATH "${_viperide_shortcut}" _viperide_shortcut_native)
...
file(WRITE "${_integration_probe}"
        ...
        "$expectedCommand = '\"' + [IO.Path]::Combine($root, 'bin\\viperide.exe') + '\" \"%1\"'\n"
        ...
        "foreach ($name in @('DisplayName','DisplayVersion','Publisher','UninstallString','QuietUninstallString','ModifyPath','RepairPath','DisplayIcon','ViperArchitecture','ViperChannel','ViperCommit','ViperPackageSha256','ViperComponents','ViperMaintenanceCache','ViperLastInstallerLog')) { if (-not $arp.PSObject.Properties[$name] -or [string]::IsNullOrWhiteSpace([string]$arp.$name)) { throw \"ARP field is missing: $name\" } }\n"
        "if ($arp.ViperChannel -cne 'e2e') { throw \"Wrong ARP channel: $($arp.ViperChannel)\" }\n"
        "if ($arp.ViperPackageSha256 -cne '${_installer_sha256}') { throw \"Wrong ARP package digest\" }\n"
        "if (-not (Test-Path -LiteralPath $arp.ViperMaintenanceCache -PathType Leaf)) { throw \"Maintenance cache is missing\" }\n"
        ...
        "$ide = Get-ShellLink '${_viperide_shortcut_native}'\n"
        "if (-not [string]::Equals($ide.Path, [IO.Path]::Combine($root, 'bin\\viperide.exe'), [StringComparison]::OrdinalIgnoreCase)) { throw \"ViperIDE shortcut target is not destination-aware: $($ide.Path)\" }\n"
        "$prompt = Get-ShellLink '${_developer_prompt_shortcut_native}'\n"
        "$expectedPromptArguments = '/k \"' + [IO.Path]::Combine($root, 'bin\\viper-dev.cmd') + '\"'\n"
        ...
        "Write-Output $arp.ViperMaintenanceCache\n")
execute_process(
        COMMAND powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass
...
file(SHA256 "${_installed_viper}" _installed_viper_sha256)
file(APPEND "${_installed_viper}" "corrupt-owned-bytes")
execute_process(
...
file(SHA256 "${_installed_viper}" _repaired_viper_sha256)
if (NOT _repaired_viper_sha256 STREQUAL _installed_viper_sha256)
    message(FATAL_ERROR "repair did not restore the exact owned viper.exe bytes")
endif ()
_viper_installer_assert_exists("${_unowned_sentinel}" "unowned file after repair")
...
# ViperIDE process only when the caller explicitly authorizes it.
file(TO_NATIVE_PATH "${_installed_viperide}" _installed_viperide_native)
set(_restart_manager_probe "${_tmp_root}/probe-restart-manager.ps1")
file(WRITE "${_restart_manager_probe}"
        "${_utf8_bom}"
        "$ErrorActionPreference = 'Stop'\n"
        "$ide = Start-Process -FilePath '${_installed_viperide_native}' -PassThru\n"
        "try {\n"
        "  Start-Sleep -Seconds 2\n"
        "  if ($ide.HasExited) { throw \"ViperIDE exited before the files-in-use probe\" }\n"
        ...
        "  if ($ide.HasExited) { throw \"Installer closed ViperIDE without authorization\" }\n"
        ...
        "  if (-not $ide.HasExited) { throw \"Restart Manager did not close ViperIDE\" }\n"
...
if (VIPER_INSTALLER_TEST_HOOKS)
    function (_viper_run_installer_hook environment_name environment_value expected_exit)
        execute_process(
    ...
    function (_viper_assert_no_transaction_residue cache_executable context)
        ...
        file(GLOB _transactions LIST_DIRECTORIES TRUE
                "${_install_parent}/.${_install_leaf}.viper-transaction-*")
        if (_transactions)
    ...
    _viper_run_installer_hook(
            VIPER_INSTALLER_TEST_WINDOWS_VERSION 6.3.9600 1603
            /install /quiet /norestart /installDir "${_install_root}" /type minimal
            /log "${_fault_log}")
    _viper_installer_assert_absent("${_install_root}" "Windows-version-preflight install root")

    # Disk exhaustion is rejected before the first filesystem or registry mutation.
    _viper_run_installer_hook(
            VIPER_INSTALLER_TEST_FREE_BYTES 1 1603
            /install /quiet /norestart /installDir "${_install_root}" /type minimal
            /log "${_fault_log}")
    _viper_installer_assert_absent("${_install_root}" "disk-preflight install root")

    # Cooperative cancellation after verified staging rolls back as exit 1602.
    _viper_run_installer_hook(
            VIPER_INSTALLER_TEST_CANCEL_AT after-stage 1602
            /install /quiet /norestart /installDir "${_install_root}" /type minimal
            /log "${_fault_log}")
    _viper_installer_assert_absent("${_install_root}" "cancelled install root")
    ...
    file(SHA256 "${_installed_viper}" _fault_baseline_sha256)
    execute_process(
            COMMAND powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass
                    -Command "(Get-ItemProperty -LiteralPath 'Registry::HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\${_package_identifier}').ViperMaintenanceCache"
    ...
    _viper_run_installer_hook(
            VIPER_INSTALLER_TEST_FAIL_AT after-registry 1603
            /repair /quiet /norestart /log "${_fault_log}")
    file(SHA256 "${_installed_viper}" _fault_recovered_sha256)
    if (NOT _fault_recovered_sha256 STREQUAL _fault_baseline_sha256)
        message(FATAL_ERROR "synchronous fault rollback changed the installed viper.exe")
    endif ()
    _viper_assert_no_transaction_residue("${_fault_cache}" "synchronous fault rollback")
    ...
    foreach (_crash_stage after-old-move after-new-move after-registry)
        _viper_run_installer_hook(
                VIPER_INSTALLER_TEST_CRASH_AT "${_crash_stage}" 1603
                /repair /quiet /norestart /log "${_fault_log}")
        ...
        file(SHA256 "${_installed_viper}" _crash_recovered_sha256)
        if (NOT _crash_recovered_sha256 STREQUAL _fault_baseline_sha256)
            message(FATAL_ERROR "recovery after ${_crash_stage} changed viper.exe")
        endif ()
        _viper_assert_no_transaction_residue(
