' Edge case testing for Math operations
' Looking for crashes, incorrect results, or unexpected behavior

DIM result AS DOUBLE
DIM iresult AS INTEGER

' === Sqrt edge cases ===
PRINT "=== Sqrt Edge Cases ==="

result = Zanna.Math.Sqrt(0)
PRINT "Sqrt(0): "; result

result = Zanna.Math.Sqrt(-1)
PRINT "Sqrt(-1): "; result

result = Zanna.Math.Sqrt(1e308)
PRINT "Sqrt(1e308): "; result

PRINT ""

' === Log edge cases ===
PRINT "=== Log Edge Cases ==="

result = Zanna.Math.Log(0)
PRINT "Log(0): "; result

result = Zanna.Math.Log(-1)
PRINT "Log(-1): "; result

result = Zanna.Math.Log(1)
PRINT "Log(1): "; result

PRINT ""

' === Pow edge cases ===
PRINT "=== Pow Edge Cases ==="

result = Zanna.Math.Pow(0, 0)
PRINT "Pow(0, 0): "; result

result = Zanna.Math.Pow(0, 1)
PRINT "Pow(0, 1): "; result

' These cause traps:
PRINT "Pow(0, -1): [skipped - causes overflow trap]"
PRINT "Pow(-1, 0.5): [skipped - causes DomainError trap]"

result = Zanna.Math.Pow(2, 1000)
PRINT "Pow(2, 1000): "; result

result = Zanna.Math.Pow(2, -1000)
PRINT "Pow(2, -1000): "; result

PRINT ""

' === Trig edge cases ===
PRINT "=== Trig Edge Cases ==="

result = Zanna.Math.Sin(1e308)
PRINT "Sin(1e308): "; result

result = Zanna.Math.Asin(2)
PRINT "Asin(2): "; result

result = Zanna.Math.Acos(2)
PRINT "Acos(2): "; result

result = Zanna.Math.Tan(Zanna.Math.Pi / 2)
PRINT "Tan(Pi/2): "; result

PRINT ""

' === Min/Max edge cases ===
PRINT "=== Min/Max Edge Cases ==="

result = Zanna.Math.Min(1e308, -1e308)
PRINT "Min(1e308, -1e308): "; result

result = Zanna.Math.Max(1e308, -1e308)
PRINT "Max(1e308, -1e308): "; result

PRINT ""

' === Abs edge cases ===
PRINT "=== Abs Edge Cases ==="

result = Zanna.Math.Abs(-1e308)
PRINT "Abs(-1e308): "; result

PRINT ""

' === Floor/Ceil/Round edge cases ===
PRINT "=== Floor/Ceil/Round Edge Cases ==="

result = Zanna.Math.Floor(-0.5)
PRINT "Floor(-0.5): "; result

result = Zanna.Math.Ceil(-0.5)
PRINT "Ceil(-0.5): "; result

result = Zanna.Math.Round(-0.5)
PRINT "Round(-0.5): "; result

result = Zanna.Math.Round(0.5)
PRINT "Round(0.5): "; result

result = Zanna.Math.Round(1.5)
PRINT "Round(1.5): "; result

result = Zanna.Math.Round(2.5)
PRINT "Round(2.5): "; result

PRINT ""

' === Clamp edge cases ===
PRINT "=== Clamp Edge Cases ==="

result = Zanna.Math.Clamp(5, 10, 1)
PRINT "Clamp(5, 10, 1) [min > max]: "; result

result = Zanna.Math.Clamp(5, 5, 5)
PRINT "Clamp(5, 5, 5) [all same]: "; result

PRINT ""

' === Bits edge cases ===
PRINT "=== Bits Edge Cases ==="

iresult = Zanna.Math.Bits.Shl(1, 63)
PRINT "Shl(1, 63): "; iresult

iresult = Zanna.Math.Bits.Shl(1, 64)
PRINT "Shl(1, 64): "; iresult

iresult = Zanna.Math.Bits.Shl(1, -1)
PRINT "Shl(1, -1): "; iresult

iresult = Zanna.Math.Bits.Shr(-1, 1)
PRINT "Shr(-1, 1): "; iresult

iresult = Zanna.Math.Bits.CountOnes(0)
PRINT "Count(0): "; iresult

iresult = Zanna.Math.Bits.CountOnes(-1)
PRINT "Count(-1): "; iresult

PRINT ""
PRINT "=== Math Edge Case Tests Complete ==="
END
