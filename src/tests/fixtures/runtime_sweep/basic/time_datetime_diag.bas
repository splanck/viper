...
' COVER: Viper.Time.DateTime.FromParts
...
' COVER: Viper.Time.DateTime.ToIso8601
...
' COVER: Viper.Time.Clock.NowMs
' COVER: Viper.Time.Clock.NowMicros
' COVER: Viper.Time.Countdown.New
' COVER: Viper.Time.Countdown.Elapsed
' COVER: Viper.Time.Countdown.IsExpired
...
DIM ts AS INTEGER
ts = Viper.Time.DateTime.FromParts(2000, 1, 2, 3, 4, 5)
...
DIM iso AS STRING
iso = Viper.Time.DateTime.ToIso8601(ts)
...
DIM t1 AS INTEGER
DIM t2 AS INTEGER
t1 = Viper.Time.Clock.NowMs()
t2 = Viper.Time.Clock.NowMicros()
...
cd.Wait()
Viper.Core.Diagnostics.Assert(cd.IsExpired, "cd.expired")
cd.Reset()
Viper.Core.Diagnostics.Assert(cd.Elapsed = 0, "cd.reset")
cd.Start()
cd.Stop()
Viper.Core.Diagnostics.Assert(cd.IsRunning = FALSE, "cd.stop")
Viper.Core.Diagnostics.Assert(cd.Remaining >= 0, "cd.remaining")

DIM sw AS OBJECT
sw = Viper.Time.Stopwatch.New()
Viper.Core.Diagnostics.Assert(sw.IsRunning = FALSE, "sw.new")
sw.Start()
Viper.Core.Diagnostics.Assert(sw.IsRunning, "sw.start")
Viper.Time.Clock.Sleep(1)
sw.Stop()
Viper.Core.Diagnostics.Assert(sw.IsRunning = FALSE, "sw.stop")
