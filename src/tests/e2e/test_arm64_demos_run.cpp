...
static RunResult run_process(const std::vector<std::string>& args) {
    ...
    for (const auto& arg : args) {
        if (!cmd.empty()) cmd += " ";
        if (arg.find(' ') != std::string::npos) cmd += '"' + arg + '"';
        else cmd += arg;
    }
    ...
    if (result.exit_code != -1) result.exit_code = WEXITSTATUS(result.exit_code);
    std::ifstream outStream(outFile), errStream(errFile);
    std::stringstream ob, eb; ob << outStream.rdbuf(); eb << errStream.rdbuf();
    result.out = ob.str(); result.err = eb.str();
    std::remove(outFile.c_str()); std::remove(errFile.c_str());
    return result;
}
...
TEST(ARM64E2E, Frogger_RunNative_OptIn) {
    if (!onMacArm64() || !optInRun()) return; // opt-in only
    ...
    if (!exists(vbasic) || !exists(ilc) || !exists(froggerBas)) return;
...
TEST(ARM64E2E, Vtris_RunNative_OptIn) {
    ...
    if (!exists(vbasic) || !exists(ilc) || !exists(vtrisBas)) return;
