...
void TarWriter::addFile(const std::string &path, const uint8_t *data,
                        size_t size, uint32_t mode, uint32_t mtime)
{
    Entry e;
    e.path = path;
...
void TarWriter::addFileString(const std::string &path,
                              ...
                              uint32_t mode, uint32_t mtime)
{
    addFile(path, reinterpret_cast<const uint8_t *>(content.data()),
            content.size(), mode, mtime);
}
...
void TarWriter::addDirectory(const std::string &path, uint32_t mode,
                             uint32_t mtime)
{
    Entry e;
    e.path = path;
...
void TarWriter::addSymlink(const std::string &path, const std::string &target,
                           uint32_t mtime)
{
    Entry e;
    e.path = path;
...
std::vector<uint8_t> TarWriter::finish() const
{
    ...
    for (const auto &e : entries_) {
        ...
        // size[12]
        writeOctal(hdr + 124, 12,
                   (e.typeflag == '0') ? e.data.size() : 0);
