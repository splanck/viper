...
//===----------------------------------------------------------------------===//
//
// File: src/tools/viper/cmd_asset.cpp
// Purpose: `viper asset` subcommands — offline 3D asset conditioning. `bake`
...
void printAssetUsage(std::FILE *out) {
    std::fprintf(out,
                 "usage: viper asset <bake|validate> ...\n"
                 "  viper asset bake <input> <output.vscn> [--force-tangents]\n"
...
int cmdAsset(int argc, char **argv) {
    ...
    (void)argv;
    std::fprintf(stderr, "viper asset: requires a graphics-enabled runtime build\n");
    return 2;
#else
    ...
    if (sub == "validate") {
        ...
        std::printf("%s\n", text && *text ? text : "{}");
        if (!model) {
            std::fprintf(stderr, "viper asset validate: failed to load '%s'\n", argv[1]);
            return 2;
        }
    ...
    if (sub == "bake") {
        ...
        for (int i = 3; i < argc; i++) {
            ...
            if (arg == "--force-tangents") {
                ...
                if (lods < 0 || lods > 8) {
                    std::fprintf(stderr, "viper asset bake: --lods expects 0..8\n");
                    return 1;
                }
            } else {
                std::fprintf(stderr, "viper asset bake: unknown option '%s'\n", arg.c_str());
                printAssetUsage(stderr);
                return 1;
            }
        }
        ...
        if (!model) {
            std::fprintf(stderr, "viper asset bake: failed to load '%s'\n", input);
            return 2;
        }
        if (lods > 0)
        ...
        if (!scene) {
            std::fprintf(stderr, "viper asset bake: failed to instantiate '%s'\n", input);
            if (rt_obj_release_check0(model))
                rt_obj_free(model);
            return 2;
        }
        ...
        if (rt_obj_release_check0(model))
            rt_obj_free(model);
        if (!saved) {
            std::fprintf(stderr, "viper asset bake: failed to save '%s'\n", output);
            return 2;
        }
