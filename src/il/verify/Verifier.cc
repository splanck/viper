    ...
    for (const auto &bb : fn.blocks) {
      ...
      for (const auto &in : bb.instructions) {
        ...
        if (isTerminator(in.op)) {
          if (seenTerm) {
            err << fn.name << ":" << bb.label << ": " << snippet(in) << ": multiple terminators\n";
            ok = false;
            break;
          }
          seenTerm = true;
        ...
        case Opcode::Alloca: {
          if (in.operands.size() != 1) {
            err << fn.name << ":" << bb.label << ": " << snippet(in) << ": expected 1 operand\n";
            ok = false;
          }
        ...
        case Opcode::AShr: {
          if (in.operands.size() != 2) {
            err << fn.name << ":" << bb.label << ": " << snippet(in) << ": expected 2 operands\n";
            ok = false;
          }
          for (const auto &op : in.operands)
        ...
        case Opcode::Load: {
          ...
          [[maybe_unused]] size_t sz = typeSize(in.type.kind); // TODO: natural alignment
          if (in.result) {
        ...
        case Opcode::AddrOf: {
          if (in.operands.size() != 1 || in.operands[0].kind != Value::Kind::GlobalAddr) {
            ...
            err << fn.name << ":" << bb.label << ": " << snippet(in) << ": unknown global @"
                << in.operands[0].str << "\n";
            ok = false;
          }
          if (in.result) {
        ...
        case Opcode::Call: {
          ...
          if (sig->retType.kind == Type::Kind::Void) {
            ...
            if (!in.result) {
              err << fn.name << ":" << bb.label << ": " << snippet(in) << ": call missing result\n";
              ok = false;
            } else {
        ...
        case Opcode::Br: {
          if (in.labels.size() != 1) {
            err << fn.name << ":" << bb.label << ": " << snippet(in) << ": expected 1 label\n";
            ok = false;
          }
          break;
        }
