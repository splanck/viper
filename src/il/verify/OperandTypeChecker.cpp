...
OperandTypeChecker::OperandTypeChecker(const VerifyCtx &ctx, const spec::InstructionSpec &spec)
    : ctx_(ctx), spec_(spec)
{
}
...
Expected<void> OperandTypeChecker::run() const
{
    ...
    for (size_t index = 0; index < instr.operands.size() && index < spec_.operands.types.size();
         ++index)
    {
        const TypeCategory category = spec_.operands.types[index];
