...
| GATE-003 | Frame finalization and final screenshot contract proven before ergonomic API | `runtime-changes.md` §1, `roadmap.md` Phase 0B | partial | C contract tests added; `walk_min` visual baseline added; `ctest --test-dir build -L graphics3d` 44/44 passed | Cross-platform build/test proof still pending |
| GATE-004 | Synthetic input/clock proven before deterministic gameplay tests | `runtime-changes.md` §7, `roadmap.md` Phase 0C | partial | `test_rt_canvas3d_gpu_paths`; `g3d_test_canvas3d_synthetic_input`; `g3d_test_game3d_runframes_probe`; graphics3d 44/44 | Canvas3D contract and Game3D `runFramesOnly` timing proven; full gameplay-state replay remains later |
...
| PHASE-0C | Deterministic test input, clock, and backend-safe quality | done | PHASE-0B for final-frame probes | Synthetic input/clock and quality ctests passed; graphics3d 44/44 | Game3D `runFrames` and Debug3D overlay wrappers remain later-phase work |
...
| PHASE-1 | Core Game3D world/entity/input/masks | partial | Phase 0 contracts | `test_rt_game3d`; `g3d_test_game3d_world_probe`; `g3d_test_game3d_runframes_probe`; callback rejection probe; graphics3d 44/44 | Core C runtime layer is implemented; docs-snippet and richer negative diagnostics remain |
| PHASE-2 | Cameras and walkable movement | partial | Core world/input | `test_rt_game3d`; `g3d_test_game3d_camera_controllers_probe`; `g3d_test_game3d_character_controller_probe` | Controllers and character runtime implemented; `walk_min` migration still pending |
...
| AC-012 | Each subsystem ships a `*_probe.zia` ctest and verifies escape hatches | partial | `g3d_test_game3d_camera_controllers_probe`; `g3d_test_game3d_character_controller_probe` | Cameras/character covered; presets, assets wrapper, physics events, animation, audio, VFX, environment remain |
