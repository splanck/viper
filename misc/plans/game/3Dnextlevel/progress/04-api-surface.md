...
| API-AUDIO-004 | `load(path)` / `loadAsset(assetPath)` | `api-spec.md` 3D audio | done | Game3D wrappers return `Viper.Sound.Sound`; lower `Sound.LoadAsset` covers mounted/embedded bytes | `test_rt_game3d`, `g3d_test_game3d_audio_probe`, lower audio tests | Game3D docs, `docs/viperlib/audio.md` | focused ctests passed |  |
