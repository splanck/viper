...
| API-WORLD-003 | `World3D.WithCamera(...)` | `api-spec.md` World3D | done | runtime method | `test_rt_game3d` | Game3D docs | focused ctest passed |  |
...
| API-WORLD-027 | Optional `world.runFixed(step, update)` | `api-spec.md` World3D | waived | native callback-loop runtime method | callback-boundary coverage | Game3D callback-boundary docs | W-001 | Native function pointers only; interpreted Zia uses manual fixed-step loops |
| API-WORLD-028 | Optional `world.runFixedWithOverlay(step, update, overlay)` | `api-spec.md` World3D | waived | native callback-loop runtime method | callback-boundary coverage | Game3D callback-boundary docs | W-001 | Native function pointers only |
...
| API-AUDIO-001 | `Audio3D.listener` | `api-spec.md` 3D audio | done | world-owned listener property | `test_rt_game3d`, `g3d_test_game3d_audio_probe` | Game3D docs | focused ctests passed |  |
| API-AUDIO-002 | `listenerFollowCamera(enabled)` | `api-spec.md` 3D audio | done | toggles listener camera binding and keeps listener active | `test_rt_game3d`, `g3d_test_game3d_audio_probe` | Game3D docs | focused ctests passed |  |
| API-AUDIO-003 | `setListenerPose(pos,forward,up)` | `api-spec.md` 3D audio | done | sets manual listener position/forward and disables camera follow | `test_rt_game3d`, `g3d_test_game3d_audio_probe` | Game3D docs | focused ctests passed | `up` reserved until low-level listener stores full orientation |
| API-AUDIO-004 | `load(path)` / `loadAsset(assetPath)` | `api-spec.md` 3D audio | done | Game3D wrappers return `Viper.Audio.Sound`; lower `Sound.LoadAsset` covers mounted/embedded bytes | `test_rt_game3d`, `g3d_test_game3d_audio_probe`, lower audio tests | Game3D docs, `docs/viperlib/audio.md` | focused ctests passed |  |
| API-AUDIO-005 | `playAt` / `playAttached` / `play2D` | `api-spec.md` 3D audio | done | positional/attached helpers create `AudioSource3D`; 2D helper returns voice id | `test_rt_game3d`, `g3d_test_game3d_audio_probe` | Game3D docs | focused ctests passed | Playback assertions conditional on audio backend availability |
| API-AUDIO-006 | `setAttenuation(refDist,maxDist)` | `api-spec.md` 3D audio | done | stores ref/max defaults and applies max distance to created sources | `test_rt_game3d`, `g3d_test_game3d_audio_probe` | Game3D docs | focused ctests passed | Low-level spatial audio currently uses max-distance linear falloff |
| API-AUDIO-007 | Audio binding sync after scene/body sync | `api-spec.md` 3D audio | done | `World3D.stepSimulation` calls `rt_audio3d_sync_bindings` after scene sync | `test_rt_game3d`, `g3d_test_game3d_audio_probe` | Game3D docs | focused ctests passed |  |
| API-VFX-001 | `Effects3D.Explosion/Sparks/Dust/Smoke/ImpactDecal` | `api-spec.md` VFX | done | static runtime class creates configured `Particles3D`/`Decal3D` presets | `test_rt_game3d`, `g3d_test_game3d_effects_probe` | Game3D docs | focused ctests passed |  |
| API-VFX-002 | `EffectRegistry3D.addParticles(p,lifetime)` | `api-spec.md` VFX | done | retains particle emitters and expires them by registry lifetime | `test_rt_game3d`, `g3d_test_game3d_effects_probe` | Game3D docs | focused ctests passed |  |
| API-VFX-003 | `EffectRegistry3D.addDecal(d)` | `api-spec.md` VFX | done | retains decals and removes them when `Decal3D.Expired` reports true | `test_rt_game3d`, `g3d_test_game3d_effects_probe` | Game3D docs | focused ctests passed | Also fixed Decal3D overshoot expiry |
| API-VFX-004 | `EffectRegistry3D.update(dt)` / `draw(canvas,camera)` / `clear()` | `api-spec.md` VFX | done | registry updates, draws particles/decals, and releases retained effects | `test_rt_game3d`, `g3d_test_game3d_effects_probe` | Game3D docs | focused ctests passed |  |
| API-VFX-005 | Effects draw after scene and before final overlay | `api-spec.md` VFX | done | `World3D.drawEffects()` draws registry after scene draw and before end/final overlay | `test_rt_game3d`, `g3d_test_game3d_effects_probe` | Game3D docs | focused ctests passed |  |
...
| API-EX-003 | Richer physics/collision/audio/VFX/follow-camera example compiles or is updated | `api-spec.md` Richer example | done | showcase and docs snippet use event-buffer version | `g3d_game3d_showcase`, `g3d_test_game3d_docs_snippets` | Game3D docs, showcase README | graphics3d 57/57 | Callback sugar replaced with event polling |
