...
| R-FRAME-006 | Preserve legacy `DrawRect2D` / `DrawText2D` auto-frame behavior | `runtime-changes.md` §1 | done | legacy helpers unchanged | `ctest -L graphics3d` |  | 45/45 graphics3d passed | Document as legacy/convenience |
...
| R-FRAME-021 | Regression: legacy overlay helpers still work | `runtime-changes.md` §1 Tests | done |  | `ctest -L graphics3d` |  | 45/45 graphics3d passed |  |
...
| R-METAL-007 | Existing software/D3D/OpenGL tests stay green | `runtime-changes.md` §4 Tests | done |  | `ctest --test-dir build -L graphics3d` |  | 45/45 graphics3d passed | Includes shared D3D/OpenGL tests and software-backend Zia probes |
...
| R-VAL-003 | `ctest --test-dir build -L graphics3d` passes | `runtime-changes.md` Validation | done | 45/45 graphics3d passed | Includes Phase 1 and Phase 2 Game3D C/Zia probes plus migrated `walk_min` |
