...
| R-FRAME-006 | Preserve legacy `DrawRect2D` / `DrawText2D` auto-frame behavior | `runtime-changes.md` §1 | done | legacy helpers unchanged | `ctest -L graphics3d` |  | 51/51 graphics3d passed | Document as legacy/convenience |
...
| R-FRAME-021 | Regression: legacy overlay helpers still work | `runtime-changes.md` §1 Tests | done |  | `ctest -L graphics3d` |  | 51/51 graphics3d passed |  |
...
| R-GET-018 | `AnimController3D.get_StateTime -> f64` | `runtime-changes.md` §3 | done | lower-level runtime getter plus `Game3D.Animator3D.stateTime()` wrapper | `test_rt_game3d`, `g3d_test_game3d_anim_probe` | Game3D docs | focused tests passed | Do not duplicate `get_CurrentState` |
...
| R-METAL-007 | Existing software/D3D/OpenGL tests stay green | `runtime-changes.md` §4 Tests | done |  | `ctest --test-dir build -L graphics3d` |  | 51/51 graphics3d passed | Includes shared D3D/OpenGL tests and software-backend Zia probes |
...
| R-QUAL-005 | `PostFX.Cinematic` enables GPU-only effects only when supported | `runtime-changes.md` §6 | done | `Canvas3D.SetQuality(2)` / `PostFX3D.NewQuality(canvas,2)` are capability-gated runtime APIs; Game3D cinematic preset uses CPU-safe effects directly | `test_rt_canvas3d_gpu_paths`, `g3d_test_canvas3d_quality_profiles`, `test_rt_game3d` | docs updated | focused ctests passed | GPU-only cinematic extras remain owned by lower-level quality profiles |
| R-QUAL-006 | Quality fallback is recorded for `Debug3D` | `runtime-changes.md` §6 | done | `QualityFallback` and `QualityFallbackReason` exposed on Canvas3D; Debug3D reads requested/active/fallback state | `test_rt_canvas3d_gpu_paths`, `g3d_test_canvas3d_quality_profiles`, `g3d_test_game3d_presets_probe` | docs updated | focused ctests passed |  |
...
| R-QUAL-009 | Debug overlay reports active profile and fallback | `runtime-changes.md` §6 Tests | done | Debug3D overlay reports requested/active quality and fallback flag | `g3d_test_game3d_presets_probe` | Game3D docs | focused ctest passed |  |
...
| R-SYN-009 | Probe: scripted WASD + mouse-look deterministic across two runs | `runtime-changes.md` §7 Tests | partial | Canvas3D probe compares two identical camera updates; Game3D controller probes and `walk_min` sample consume synthetic movement input | `g3d_test_canvas3d_synthetic_input`; `g3d_test_game3d_camera_controllers_probe`; `g3d_test_game3d_character_controller_probe`; `g3d_walk_min_visual_probe` |  | graphics3d 51/51 passed | Full showcase gameplay replay pending |
...
| R-ASSET-008 | Test `Assets3D.LoadModelTemplate` cache without retained asset leaks | `runtime-changes.md` §8 Tests | done | `Assets3D.LoadModelTemplate` / `LoadModelTemplateAsset` cache `ModelTemplate` wrappers and release cache entries on `ClearCache` | `test_rt_game3d`, `g3d_test_game3d_assets_probe` | Game3D docs | focused ctests passed | C test verifies cache identity; Zia probe verifies reuse path remains usable |
...
| R-LIFE-002 | Define `World3D` ownership of collision/animation event buffers | `runtime-changes.md` §10 | done | collision buffers are owned by `Physics3DWorld`; `Animator3D` owns latest-frame animation event names drained from its controller | `test_rt_game3d`, `g3d_test_game3d_collision_probe`, `g3d_test_game3d_anim_probe` | Game3D docs | focused tests passed | Callback sugar remains deferred; buffers are authoritative |
| R-LIFE-003 | Define `World3D` ownership of effects, controllers, model templates, audio/effect objects | `runtime-changes.md` §10 | done | world owns input/audio/effects/camera controller refs; Audio3D tracks created sources; EffectRegistry3D retains/releases particles and decals; global Assets3D cache owns retained model templates until `ClearCache` | `test_rt_game3d`, `g3d_test_game3d_assets_probe`, `g3d_test_game3d_audio_probe`, `g3d_test_game3d_effects_probe` | Game3D docs | focused ctests passed |  |
...
| R-LIFE-007 | Define imported model-root group despawn semantics | `runtime-changes.md` §10 | done | `Assets3D` returns group entities whose root owns the raw imported node subtree | `test_rt_game3d`, `g3d_test_game3d_assets_probe` | Game3D docs | focused ctests passed | Raw imported children are scene nodes, not Game3D child entities |
...
| R-LIFE-009 | Define raw imported `SceneNode3D` child semantics | `runtime-changes.md` §10 | done | raw nodes remain under imported root by design and are removed with the group root | `test_rt_game3d`, `g3d_test_game3d_assets_probe` | Game3D docs | focused ctests passed |  |
| R-LIFE-010 | Define attached body/audio/effect/animation cleanup semantics | `runtime-changes.md` §10 | done | attached BodyDef/raw bodies are removed from physics on despawn/replacement; Audio3D attached sources bind to entity nodes and can be cleared; EffectRegistry3D expires/releases effects; entity animators are retained by entity and bind raw controller to node | `test_rt_game3d`, `g3d_test_game3d_physics_probe`, `g3d_test_game3d_anim_probe`, `g3d_test_game3d_audio_probe`, `g3d_test_game3d_effects_probe` | Game3D docs | focused tests passed |  |
...
| R-VAL-003 | `ctest --test-dir build -L graphics3d` passes | `runtime-changes.md` Validation | done | 51/51 graphics3d passed | Includes Phase 1 through Phase 6 Game3D C/Zia probes plus migrated `walk_min` |
| R-VAL-004 | Focused C tests exist for getters/default lighting/finalization/screenshots/synthetic input/assets/lifetime | `runtime-changes.md` Validation | partial | Finalization/screenshot/capability/synthetic-input/assets/Game3D Phase 1-6 lifetime C tests added | Starter and showcase areas pending |
| R-VAL-005 | Zia probes exist for final screenshots/overlay/default lighting/quality/package load/determinism | `runtime-changes.md` Validation | partial | `g3d_walk_min_visual_probe`; `g3d_test_canvas3d_synthetic_input`; `g3d_test_canvas3d_quality_profiles`; `g3d_test_model3d_load_asset`; Game3D probes through audio/effects | `walk_min` now covers grounded movement; rich showcase gameplay-state replay pending |
| R-VAL-006 | Every new public runtime function has success-path ctest or waiver | `runtime-changes.md` Validation | partial | Phase 0B/0C and Phase 1-6 Game3D functions covered by focused tests | Starter and snippet coverage pending; callback sugar has explicit waiver |
