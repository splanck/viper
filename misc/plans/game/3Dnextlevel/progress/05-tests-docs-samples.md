...
| TEST-015 | Regression | `ctest --test-dir build -L graphics3d` stays green | `roadmap.md` Testing strategy | done | `ctest --test-dir build -L graphics3d --output-on-failure` 48/48 passed |  | Includes Phase 1-4 Game3D probes, synthetic input, quality profiles, and model asset loading probes |
...
|---|---|---|---|---|---|
| TEST-RULE-001 | Tests follow `src/tests/CMakeLists.txt` registration patterns | `roadmap.md` Testing strategy | partial | Phase 0A, `walk_min`, synthetic-input, quality, Game3D world/controller/preset/assets/physics/collision probes registered; Phase 0B-4 C coverage added to existing unit tests | Future animation/audio/VFX/starter/showcase probes still need registration |
| TEST-RULE-002 | Display-dependent tests carry `requires_display` labels | `roadmap.md` Testing strategy | partial | `g3d_game3d_surface_probe`, `g3d_test_canvas3d_synthetic_input`, `g3d_test_canvas3d_quality_profiles`, `g3d_walk_min_visual_probe`, Game3D world/controller/preset/assets/physics/collision probes | Existing graphics tests still need audit before marking done |
...
| DOC-011 | API reference | Every new runtime function and Game3D API area | `roadmap.md` Phase 7 | partial | pending | `docs/viperlib/graphics/game3d.md` | Phase 1 through Phase 4 Game3D surfaces documented; docs snippet ctest and later API areas pending |
