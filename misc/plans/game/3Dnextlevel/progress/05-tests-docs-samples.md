...
| TEST-003 | Synthetic input/clock | Deterministic camera/gameplay state probe | `roadmap.md` Required ctest inventory | partial | C: `test_rt_canvas3d_gpu_paths`; Zia: `g3d_test_canvas3d_synthetic_input`, `g3d_test_game3d_camera_controllers_probe`, `g3d_test_game3d_character_controller_probe`, `g3d_walk_min_visual_probe` |  | Canvas3D, Game3D controllers, and the `walk_min` grounded movement path covered; full showcase gameplay replay pending |
...
| TEST-015 | Regression | `ctest --test-dir build -L graphics3d` stays green | `roadmap.md` Testing strategy | done | `ctest --test-dir build -L graphics3d --output-on-failure` 51/51 passed |  | Includes Phase 1-6 Game3D probes, synthetic input, quality profiles, model asset loading, and `walk_min` |
...
|---|---|---|---|---|---|
| TEST-RULE-001 | Tests follow `src/tests/CMakeLists.txt` registration patterns | `roadmap.md` Testing strategy | partial | Phase 0A, `walk_min`, synthetic-input, quality, Game3D world/controller/preset/assets/physics/collision/animation/audio/effects probes registered; Phase 0B-6 C coverage added to existing unit tests | Future starter/showcase probes still need registration |
| TEST-RULE-002 | Display-dependent tests carry `requires_display` labels | `roadmap.md` Testing strategy | partial | `g3d_game3d_surface_probe`, `g3d_test_canvas3d_synthetic_input`, `g3d_test_canvas3d_quality_profiles`, `g3d_walk_min_visual_probe`, Game3D world/controller/preset/assets/physics/collision/animation/audio/effects probes | Existing graphics tests still need audit before marking done |
...
| DOC-003 | Deterministic tests | Synthetic input/clock, `runFrames`, replay assertions | `roadmap.md` Phase 7 | partial | `g3d_test_game3d_runframes_probe`, controller probes | `docs/viperlib/graphics/game3d.md`, rendering docs | Game3D docs explain `runFramesOnly` and controller phase order; full replay tutorial pending |
...
| DOC-006 | Assets and packages | GLB, glTF external deps, audio assets, package layout | `roadmap.md` Phase 7 | partial | `g3d_test_model3d_load_asset`, `g3d_test_game3d_assets_probe` cover API import/use | `docs/graphics3d-guide.md`, `docs/viperlib/io/assets.md`, `docs/viperlib/audio.md`, `docs/viperlib/graphics/game3d.md` | Game3D `Assets3D` docs done; starter package layout still pending |
...
| DOC-011 | API reference | Every new runtime function and Game3D API area | `roadmap.md` Phase 7 | partial | pending | `docs/viperlib/graphics/game3d.md` | Phase 1 through Phase 6 Game3D surfaces documented; docs snippet ctest and later starter/showcase docs pending |
...
| SHOW-003 | Final-frame post-FX plus crisp HUD/debug overlay | `roadmap.md` Phase 7 | partial | `g3d_test_game3d_presets_probe` | Showcase integration pending |
| SHOW-004 | Lighting/material/post-FX/quality/environment preset toggles | `roadmap.md` Phase 7 | partial | `g3d_test_game3d_presets_probe` | Toggle UI/showcase integration pending |
| SHOW-005 | Terrain/ground, water or fog | `roadmap.md` Phase 7 | partial | `g3d_test_game3d_presets_probe` | Showcase integration pending |
| SHOW-006 | Prefabs and imported packaged GLB/glTF props | `roadmap.md` Phase 7 | partial | `g3d_test_game3d_presets_probe` covers prefabs; `g3d_test_model3d_load_asset` covers model assets | Combined showcase path pending |
| SHOW-007 | First-person camera mode | `roadmap.md` Phase 7 | partial | `g3d_test_game3d_character_controller_probe` | Showcase integration pending |
| SHOW-008 | Follow or orbit camera mode | `roadmap.md` Phase 7 | partial | `g3d_test_game3d_camera_controllers_probe` | Showcase integration pending |
| SHOW-009 | Character controller or player-controlled physics body | `roadmap.md` Phase 7 | partial | `g3d_test_game3d_character_controller_probe` | Showcase integration pending |
| SHOW-010 | `LayerMask`, `BodyDef`, triggers, collision event handling | `roadmap.md` Phase 7 | partial | `g3d_test_game3d_physics_probe`, `g3d_test_game3d_collision_probe` | Runtime and probes covered; showcase integration pending |
| SHOW-011 | Animated skinned model with animation events | `roadmap.md` Phase 7 | partial | `g3d_test_game3d_anim_probe` | Runtime animation events/root motion covered; showcase needs real imported skinned model |
...
| SHOW-016 | Package-aware model/audio loading | `roadmap.md` Phase 7 | partial | `test_rt_gltf`, `test_rt_audio_integration`, `g3d_test_game3d_assets_probe` | Runtime and Game3D Assets3D covered; showcase/starter package workflow pending |
