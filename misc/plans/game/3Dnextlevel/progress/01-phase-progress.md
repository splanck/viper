...
| P4-010 | Model loads return group entities with root subtree and populated `entity.anim` when skinned | `roadmap.md` Phase 4 | done | instantiated model roots become group Entity3D values; root skeletal animator is wrapped into `Game3D.Animator3D` when present | `test_rt_game3d`, `g3d_test_game3d_assets_probe`, `g3d_test_game3d_anim_probe` | Game3D docs | focused ctests passed | Phase 5 owns playback/event/root-motion helpers |
...
| ID | Item | Source | Status | Impl | Tests | Docs | Proof / link | Notes |
|---|---|---|---|---|---|---|---|---|
| P5-001 | Implement runtime-backed `Animator3D` over `AnimController3D`/`Skeleton3D` | `roadmap.md` Phase 5 | done | `Game3D.Animator3D` wraps a raw `AnimController3D` and exposes controller escape hatch | `test_rt_game3d`, `g3d_test_game3d_anim_probe` | Game3D docs | focused tests passed | Normal C runtime files, not a Zia helper library |
| P5-002 | Expose `entity.anim` | `roadmap.md` Phase 5 | done | `Entity3D.anim` is typed as `Game3D.Animator3D`; `attachAnimator` accepts wrapper or raw controller | `test_rt_game3d`, `g3d_test_game3d_anim_probe` | Game3D docs | focused tests passed | Model template instantiation wraps imported root controller |
| P5-003 | Advance animation in documented pre-physics phase | `roadmap.md` Phase 5 | done | `World3D.stepSimulation` advances spawned entity animators after controller update and before physics | `test_rt_game3d`, `g3d_test_game3d_anim_probe` | Game3D docs | focused tests passed | Manual `Animator3D.update` remains for off-world animators |
| P5-004 | `SyncMode.NodeFromAnimRootMotion` drives node transforms deterministically | `roadmap.md` Phase 5 | done | entity node binds raw controller; existing scene sync consumes root-motion delta | `test_rt_game3d`, `g3d_test_game3d_anim_probe` | Game3D docs | focused tests passed | Probe asserts root-motion displacement after one deterministic frame |
| P5-005 | Add `stateTime()` backed by runtime getter | `roadmap.md` Phase 5 | done | `AnimController3D.get_StateTime` plus `Game3D.Animator3D.stateTime()` | `test_rt_game3d`, `g3d_test_game3d_anim_probe` | Game3D docs | focused tests passed | Also adds `AnimController3D.IsStatePlaying` helper |
| P5-006 | Skinned glTF plays and crossfades animations | `roadmap.md` Phase 5 Exit | partial | `Animator3D.play`/`crossfade` works over imported/root controllers; direct committed skinned glTF visual probe remains later | `test_rt_game3d`, `g3d_test_game3d_anim_probe`, lower `AnimController3D` tests | Game3D docs | focused tests passed | Showcase should add real skinned asset proof |
| P5-007 | Runtime animation event accessors report named events at right times | `roadmap.md` Phase 5 Exit | done | `Animator3D.eventCount` / `eventName` drain controller events per play/update frame | `test_rt_game3d`, `g3d_test_game3d_anim_probe` | Game3D docs | focused tests passed |  |
| P5-008 | Optional `onAnimEvent` sugar dispatches if approved | `roadmap.md` Phase 5 Exit | deferred | not shipped because interpreted-Zia callbacks are not C-callable yet | callback rejection policy | Game3D docs | explicit waiver | Event-buffer polling is supported path |
| P5-009 | Root motion moves entity node deterministically | `roadmap.md` Phase 5 Exit | done | `World3D` animation update plus scene root-motion sync moves the entity node | `test_rt_game3d`, `g3d_test_game3d_anim_probe` | Game3D docs | focused tests passed |  |
| P5-010 | `anim_probe.zia` passes under `runFrames` | `roadmap.md` Phase 5 Exit | done | `tests/runtime/test_game3d_anim_probe.zia` registered under ctest | `g3d_test_game3d_anim_probe` | Game3D docs | direct probe passed | Uses deterministic `runFramesOnly` because interpreted callbacks are deferred |
