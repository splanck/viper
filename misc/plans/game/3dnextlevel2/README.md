...
|---|---|---|
| Concurrency | Deterministic main-thread simulation with worker substrate, ordered merge, main-thread commit queue, asset-worker coverage, focused TSan race lane, and VM/native determinism gate | Future simulation-touching changes must rerun the NL3-031 gate and add ADR evidence when IL/VM/native semantics change |
...
| World size | Manifest-driven streamed terrain/cell baseline with stitched tile seams and named traversal proof | Streamed nav bake/export tooling depth |
