...
Update **immediately** whenever a Zanna runtime or Zia issue is hit.
...
- **What:** `Zanna.Graphics.Canvas` has `Text`, `TextBg`, `TextCentered`, `TextCenteredScaled`, `TextRight`, `TextWidth`, `TextHeight`, but `Zanna.Graphics.Pixels` exposes **no** text method (no `DrawText`). You cannot rasterize text directly into an offscreen `Pixels` layer buffer.
...
- **Repro:** Survey of `docs/zannalib/graphics/pixels.md` + `src/runtime/graphics/2d/rt_pixels_*.c` — no text entry point.
...
- **What:** `docs/zannalib/graphics/rendering2d.md` documents `Gradient2D.Sample(t)` with `t` a Number 0.0–1.0, but calling `g.Sample(0.5)` from Zia fails to compile: `error[V-ZIA-TYPE-MISMATCH]: expected Integer, got Number`. The binding appears to expect an Integer (possibly a step index 0..steps-1), contradicting the docs.
...
- **`zanna bench`** may hang on some IL files (`tooling`).
...
- **What:** Building Xenoscape emits `warning[V3001]` because
  `Zanna.Math.Lerp` conflicts with `Zanna.Graphics.Color.Lerp` when both modules
  are bound.
...
- **What:** `examples/games/xenoscape/save.zia` and `lore.zia` used `Int(...)`
  without binding `Zanna.Text.Fmt`. A whole-project build through
...
- **What:** `docs/zannalib/graphics/canvas.md` demonstrates fullscreen toggling
  with `Zanna.Input.Keyboard.Pressed(300)`, but the live runtime API exposed to
...
- **Impact:** Users following the fullscreen example get a compile-time
  `Runtime class 'Zanna.Input.Keyboard' has no method 'Pressed'` error.
...
- **Workaround:** Use `Zanna.Input.Keyboard.WasPressed(KEY_F11)` for one-shot
  fullscreen toggles.

### ISSUE-010 — ✅ runtime: macOS 2D Canvas fullscreen could fail or keep a window-sized drawable
- **Component:** runtime / graphics (ZannaGFX macOS backend)
- **What:** Xenoscape's `canvas.Fullscreen()` path still did not enter
  ...
  input API. The 2D Canvas path delegates to ZannaGFX; on macOS the backend used
...
- **Coverage:** Incremental `zannagfx`/runtime/`zanna` build, focused
  `test_window` and `test_input` executables, Xenoscape project build, plus a
  real macOS ZannaGFX fullscreen probe that grew a fixed `960x640` window to
