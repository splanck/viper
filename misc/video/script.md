# Zanna — Public Introduction Video (Flagship Deep-Dive)
...
**SCREEN:** Title card — **ZANNA**.

## ACT 1 — It's a real language (0:20–2:00)
**VO:** "Zanna's flagship language is called Zia — statically typed, with classes,
generics, pattern matching. But let's start small."
**SCREEN:** `zanna repl` → `Say("Hello from Zanna")` → `Say(Fmt.Int(2 + 3))` → `5`.
...
lowers to Zanna IL — a typed, SSA-based intermediate language you can read."
**SCREEN:** `zanna build demo.zia -O0 -o demo_O0.il`; scroll to `func @square`.
...
**SCREEN:** `zanna build demo.zia -o demo.il`; show `square` collapsed to
`imul.ovf %t1, %t1 / ret`.
...
**VO:** "The IL can run on Zanna's virtual machine. Or it can become a real
executable — and here's where it gets serious."
**SCREEN:** `zanna build demo.zia -o demo_native` → `file demo_native` →
`Mach-O 64-bit executable arm64` → run it.
**VO:** "That's a native Mach-O binary. And here's the part I want to be precise
about: Zanna did not call out to LLVM, or gcc, or clang, or the system linker. It
...
**SCREEN:** Paint (layers, undo). ZannasSQL (a query running).
...
**VO (honesty):** "One thing I want to be straight about: Zanna is pre-alpha. The
...
**SCREEN:** GitHub repo, GPLv3 badge, `git clone` + `zanna run` quickstart.
...
Thanks for watching."
**SCREEN:** End card — repo URL, "Zanna · an IL-first toolchain & VM".
