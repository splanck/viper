...
{
    {
        ...
        if (prefix.find("%xmm1") == std::string::npos) return 4;
    }
    {
        const auto text = buildAsmWithCallee("rt_sb_printf");
        const auto callPos = text.find("call");
        if (callPos == std::string::npos) return 5;
        const auto mov2 = text.rfind("movq $2, %rax", callPos);
        if (mov2 == std::string::npos) return 6;
        const auto prefix = text.substr(0, callPos);
        if (prefix.find("%xmm0") == std::string::npos) return 7;
        if (prefix.find("%xmm1") == std::string::npos) return 8;
    }
    // 0 f64s
    {
        ILValue buf = makeParam(0, ILValue::Kind::PTR);
        ILValue size = makeParam(1, ILValue::Kind::I64);
        ILInstr call{}; call.opcode = "call"; call.resultId = 3; call.resultKind = ILValue::Kind::I64;
        call.ops = {makeLabel("rt_snprintf"), buf, size};
    ...
    // 1 f64
    {
        ...
        ILValue size = makeParam(1, ILValue::Kind::I64);
        ILValue f0 = makeParam(2, ILValue::Kind::F64);
        ILInstr call{}; call.opcode = "call"; call.resultId = 4; call.resultKind = ILValue::Kind::I64;
        call.ops = {makeLabel("rt_snprintf"), buf, size, f0};
    ...
    {
        ILValue x = makeParam(0, ILValue::Kind::F64);
        ILInstr call{}; call.opcode = "call"; call.resultId = 1; call.resultKind = ILValue::Kind::I64;
        call.ops = {makeLabel("rt_print_f64"), x};
