...
Viper.Diagnostics.Assert(host <> "", "dns.localhost")

DIM addrs AS Viper.Collections.Seq
addrs = Viper.Network.Dns.LocalAddrs()
Viper.Diagnostics.Assert(addrs.Len >= 1, "dns.localaddrs")

DIM resolved AS STRING
resolved = Viper.Network.Dns.Resolve("localhost")
Viper.Diagnostics.Assert(Viper.Network.Dns.IsIPv4(resolved), "dns.resolve")

DIM resolved4 AS STRING
resolved4 = Viper.Network.Dns.Resolve4(ip4)
Viper.Diagnostics.Assert(Viper.Network.Dns.IsIPv4(resolved4), "dns.resolve4")

DIM resolved6 AS STRING
resolved6 = Viper.Network.Dns.Resolve6(ip6)
Viper.Diagnostics.Assert(Viper.Network.Dns.IsIPv6(resolved6), "dns.resolve6")

DIM all AS Viper.Collections.Seq
all = Viper.Network.Dns.ResolveAll("localhost")
Viper.Diagnostics.Assert(all.Len >= 1, "dns.resolveall")

DIM rev AS STRING
rev = Viper.Network.Dns.Reverse(ip4)
Viper.Diagnostics.Assert(rev <> "", "dns.reverse")
