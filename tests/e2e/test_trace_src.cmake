if(NOT DEFINED ILC)
  message(FATAL_ERROR "ILC not set")
endif()
if(NOT DEFINED BAS_FILE)
  message(FATAL_ERROR "BAS_FILE not set")
endif()
if(NOT DEFINED GOLDEN)
  message(FATAL_ERROR "GOLDEN not set")
endif()
execute_process(COMMAND ${ILC} front basic -run ${BAS_FILE} --trace=src
                OUTPUT_FILE out.txt ERROR_FILE trace.txt RESULT_VARIABLE r)
if(NOT r EQUAL 0)
  message(FATAL_ERROR "execution failed")
endif()
file(READ trace.txt TRACE)
file(READ ${GOLDEN} EXP)
if(NOT TRACE STREQUAL EXP)
  message(FATAL_ERROR "trace mismatch")
endif()
