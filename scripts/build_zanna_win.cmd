...
REM ===----------------------------------------------------------------------===
REM
REM File: scripts/build_zanna_win.cmd
REM Purpose: Configure, build, test, audit, and install Zanna on Windows.
REM
REM Key invariants:
...
REM
REM Ownership/Lifetime: Build outputs remain in ZANNA_BUILD_DIR.
REM
...
set "ZANNA_NORMALIZED_PATH=%PATH%"
set "Path="
set "PATH="
set "PATH=%ZANNA_NORMALIZED_PATH%"
set "ZANNA_NORMALIZED_PATH="

setlocal EnableDelayedExpansion
set TESTS_FAILED=0

REM build_zanna.cmd - Windows Zanna build + test + install script.
REM Matches robustness and features of the Unix build scripts.

echo ==========================================
echo Building Zanna on Windows
echo ==========================================
echo.

REM --- Parallel job detection -------------------------------------------------
REM MSVC can exhaust compiler heap when a large clean build fans out across every
REM logical CPU. Keep the automatic default conservative; ZANNA_JOBS remains an
...
if not "%ZANNA_CTEST_JOBS%"=="" (
    set "CTEST_JOBS=%ZANNA_CTEST_JOBS%"
) else (
    set "CTEST_JOBS=%JOBS%"
)
...
if "%ZANNA_BUILD_DIR%"=="" set "ZANNA_BUILD_DIR=build"
if "%ZANNA_BUILD_TYPE%"=="" set "ZANNA_BUILD_TYPE=Debug"
if "%ZANNA_SKIP_INSTALL%"=="" set "ZANNA_SKIP_INSTALL=0"
if "%ZANNA_SKIP_TESTS%"=="" set "ZANNA_SKIP_TESTS=0"
if "%ZANNA_SKIP_LINT%"=="" set "ZANNA_SKIP_LINT=0"
if "%ZANNA_SKIP_AUDIT%"=="" set "ZANNA_SKIP_AUDIT=0"
if "%ZANNA_LINT_CHANGED_ONLY%"=="" set "ZANNA_LINT_CHANGED_ONLY=1"
if "%ZANNA_SKIP_SMOKE%"=="" set "ZANNA_SKIP_SMOKE=0"
if "%ZANNA_SKIP_CLEAN%"=="" set "ZANNA_SKIP_CLEAN=0"
if "%ZANNA_RUN_SLOW_TESTS%"=="" set "ZANNA_RUN_SLOW_TESTS=0"
if "%ZANNA_FAST_DEBUG%"=="" set "ZANNA_FAST_DEBUG=1"
set "BASH_BUILD_DIR=%ZANNA_BUILD_DIR:\=/%"
echo Build type: %ZANNA_BUILD_TYPE%
echo Fast Debug: %ZANNA_FAST_DEBUG%
...
set "CONFIG_ARGS=-DZANNA_FAST_DEBUG=%ZANNA_FAST_DEBUG%"
if defined BASH_EXE (
    set "CONFIG_ARGS=%CONFIG_ARGS% -DZANNA_BASH_EXECUTABLE:FILEPATH="%BASH_EXE%""
)
if not "%ZANNA_CMAKE_GENERATOR%"=="" (
    set "CONFIG_ARGS=%CONFIG_ARGS% -G "%ZANNA_CMAKE_GENERATOR%""
)
if not "%ZANNA_EXTRA_CMAKE_ARGS%"=="" (
    set "CONFIG_ARGS=%CONFIG_ARGS% %ZANNA_EXTRA_CMAKE_ARGS%"
)
if "%ZANNA_WARN_AS_ERROR%"=="" (
    if /I "%PROCESSOR_ARCHITECTURE%"=="ARM64" (
        set "CONFIG_ARGS=%CONFIG_ARGS% -DIL_WARN_AS_ERROR=OFF"
    )
) else (
    set "CONFIG_ARGS=%CONFIG_ARGS% -DIL_WARN_AS_ERROR=%ZANNA_WARN_AS_ERROR%"
)

REM --- Compiler selection -----------------------------------------------------
REM Default to MSVC on Windows. Opt into clang-cl with ZANNA_WINDOWS_COMPILER=clang-cl.
set COMPILER_FLAGS=
set "REQUESTED_WINDOWS_COMPILER=msvc"
if /I "%ZANNA_WINDOWS_COMPILER%"=="clang-cl" (
    where clang-cl >nul 2>&1
    if errorlevel 1 (
        echo ERROR: ZANNA_WINDOWS_COMPILER=clang-cl but clang-cl was not found in PATH
        exit /b 1
    )
    echo Using clang-cl ^(ZANNA_WINDOWS_COMPILER=clang-cl^)
...
if "%ZANNA_SKIP_CLEAN%"=="1" (
    echo Skipping clean ^(ZANNA_SKIP_CLEAN=1^); incremental rebuild
) else if "%CACHE_RESET%"=="0" (
    cmake --build "%ZANNA_BUILD_DIR%" --target clean-all 2>nul
    if errorlevel 1 (
...
cmake -S . -B "%ZANNA_BUILD_DIR%" %COMPILER_FLAGS% -DCMAKE_BUILD_TYPE=%ZANNA_BUILD_TYPE% %CONFIG_ARGS%
if errorlevel 1 (
...
cmake --build "%ZANNA_BUILD_DIR%" --config %ZANNA_BUILD_TYPE% -j %JOBS%
if errorlevel 1 (
...
if exist "%ZANNA_BUILD_DIR%\Testing" rmdir /s /q "%ZANNA_BUILD_DIR%\Testing"

REM --- Run tests --------------------------------------------------------------
if "%ZANNA_SKIP_TESTS%"=="1" (
    echo Skipping tests ^(ZANNA_SKIP_TESTS=1^)
    goto :after_tests
)

echo Running tests...
set "CTEST_LABEL_ARGS="
if not "%ZANNA_TEST_LABEL%"=="" (
    echo Running only tests labeled "%ZANNA_TEST_LABEL%" ^(ZANNA_TEST_LABEL^)
    set "CTEST_LABEL_ARGS=-L %ZANNA_TEST_LABEL%"
)
if not "%ZANNA_RUN_SLOW_TESTS%"=="1" (
    echo Skipping tests labeled slow ^(set ZANNA_RUN_SLOW_TESTS=1 to include them^)
...
if "%CTEST_USE_PRETTY%"=="1" (
    powershell -NoProfile -ExecutionPolicy Bypass -File "%CTEST_PRETTY_SCRIPT%" --test-dir "%ZANNA_BUILD_DIR%" -C %ZANNA_BUILD_TYPE% --output-on-failure -j %CTEST_JOBS% %CTEST_LABEL_ARGS%
) else (
    ctest --test-dir "%ZANNA_BUILD_DIR%" -C %ZANNA_BUILD_TYPE% --output-on-failure -j %CTEST_JOBS% %CTEST_LABEL_ARGS%
)
if errorlevel 1 (
    set TESTS_FAILED=1
    echo.
...
if "%ZANNA_SKIP_LINT%"=="0" (
    if defined BASH_EXE (
        echo Running platform policy lint...
        if "%ZANNA_LINT_CHANGED_ONLY%"=="1" (
...
if "%ZANNA_SKIP_AUDIT%"=="0" (
    if defined BASH_EXE (
        echo Running runtime surface audit...
        "%BASH_EXE%" --login scripts/audit_runtime_surface.sh --build-dir="%BASH_BUILD_DIR%" --config="%ZANNA_BUILD_TYPE%"
        if errorlevel 1 set TESTS_FAILED=1
    ) else (
...
if "%ZANNA_SKIP_SMOKE%"=="0" (
    if defined BASH_EXE (
        echo Running cross-platform smoke tests...
        "%BASH_EXE%" --login scripts/run_cross_platform_smoke.sh --build-dir "%BASH_BUILD_DIR%" --config "%ZANNA_BUILD_TYPE%"
        if errorlevel 1 set TESTS_FAILED=1
    ) else (
...
REM --- Install ----------------------------------------------------------------
echo.
if "%ZANNA_SKIP_INSTALL%"=="1" (
    echo Skipping install ^(ZANNA_SKIP_INSTALL=1^)
    goto :build_done
)

echo Installing Zanna...
if defined LOCALAPPDATA (
    set INSTALL_PREFIX=%LOCALAPPDATA%\zanna
) else (
    set INSTALL_PREFIX=%USERPROFILE%\zanna
)
if not "%ZANNA_INSTALL_PREFIX%"=="" set INSTALL_PREFIX=%ZANNA_INSTALL_PREFIX%
cmake --install "%ZANNA_BUILD_DIR%" --prefix "%INSTALL_PREFIX%" --config %ZANNA_BUILD_TYPE%
if errorlevel 1 (
...
set "CACHE_FILE=%ZANNA_BUILD_DIR%\CMakeCache.txt"
set "STALE_CACHE=0"
...
echo Detected stale CMake compiler cache in "%ZANNA_BUILD_DIR%".
...
if exist "%ZANNA_BUILD_DIR%\CMakeFiles" (
    rmdir /s /q "%ZANNA_BUILD_DIR%\CMakeFiles"
    if exist "%ZANNA_BUILD_DIR%\CMakeFiles" (
        echo ERROR: Failed to remove "%ZANNA_BUILD_DIR%\CMakeFiles"
        exit /b 1
    )
)
set "CACHE_RESET=1"
...
:find_usable_bash
set "BASH_EXE="
if defined ZANNA_BASH_EXECUTABLE (
    call :try_bash "%ZANNA_BASH_EXECUTABLE%"
    if defined BASH_EXE exit /b 0
    echo WARNING: ZANNA_BASH_EXECUTABLE is not usable: %ZANNA_BASH_EXECUTABLE%
)
