...
#         canonical Zanna.* names. Use this script in CI to catch regressions.
#
# Behavior:
...
#
# Enable strict mode by setting ZANNA_STRICT_NO_RT_NAMES=1 in CI.
...
if [[ "${ZANNA_STRICT_NO_RT_NAMES:-0}" == "1" ]]; then
  echo "FAIL: rt_* usage detected in BASIC frontend (strict mode)." >&2
  exit 1
else
  echo "WARN: rt_* usage detected; set ZANNA_STRICT_NO_RT_NAMES=1 to make this fatal." >&2
  exit 0
fi
