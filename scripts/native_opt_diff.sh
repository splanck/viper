...
#   <target>  A .zia/.bas file, project directory, or viper.project path.
#
# Environment:
#   VIPER_BIN   Override the viper binary (default: build/src/tools/viper/viper)
...
VIPER_BIN="${VIPER_BIN:-build/src/tools/viper/viper}"
OPT_A="${OPT_A:--O0}"
OPT_B="${OPT_B:--O2}"

if [ ! -x "$VIPER_BIN" ]; then
    echo "ERROR: viper binary not found at $VIPER_BIN (set VIPER_BIN or build first)" >&2
    exit 2
fi

WORKDIR="$(mktemp -d "${TMPDIR:-/tmp}/viper_opt_diff.XXXXXX")" || exit 2
...
build_one() {
    ...
    if ! "$VIPER_BIN" build "$TARGET" -o "$2" "$1" --quiet-warnings \
