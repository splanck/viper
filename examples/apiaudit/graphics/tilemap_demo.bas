PRINT "=== API Audit: Zanna.Graphics2D.Tilemap ==="
PRINT "NOTE: Graphics classes require a canvas/window. API surface test only."
PRINT "Constructor: New(width, height, tileW, tileH)"
PRINT "Properties: get_Width, get_Height, get_TileWidth, get_TileHeight"
PRINT "Methods: SetTile(x, y, tileId), GetTile(x, y)"
PRINT "Methods: SetLayer(layerIdx), GetLayer()"
PRINT "Methods: Clear()"
PRINT "=== Tilemap Audit Complete (API surface only) ==="
END
