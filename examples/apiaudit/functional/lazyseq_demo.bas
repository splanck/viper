' API Audit: Viper.Functional.LazySeq (BASIC)
...
PRINT "=== API Audit: Viper.Functional.LazySeq ==="

' --- Range creation ---
PRINT "--- Range ---"
DIM s1 AS OBJECT = Viper.Functional.LazySeq.Range(1, 10, 1)
PRINT s1.Index
PRINT s1.IsExhausted

' --- Count ---
PRINT "--- Count ---"
DIM s4 AS OBJECT = Viper.Functional.LazySeq.Range(1, 5, 1)
PRINT s4.Count()
...
DIM rep AS OBJECT = Viper.Functional.LazySeq.Repeat(box42, 3)
PRINT rep.Count()

' --- Next with Repeat (returns proper box) ---
PRINT "--- Next (Repeat) ---"
DIM box7 AS OBJECT = Viper.Core.Box.I64(7)
DIM rep2 AS OBJECT = Viper.Functional.LazySeq.Repeat(box7, 5)
...
' --- Take ---
PRINT "--- Take ---"
DIM s2 AS OBJECT = Viper.Functional.LazySeq.Range(1, 100, 1)
DIM taken AS OBJECT = s2.Take(5)
PRINT taken.Count()

' --- Drop ---
PRINT "--- Drop ---"
DIM s3 AS OBJECT = Viper.Functional.LazySeq.Range(1, 10, 1)
...
' --- Concat ---
PRINT "--- Concat ---"
DIM sa AS OBJECT = Viper.Functional.LazySeq.Range(1, 3, 1)
DIM sb AS OBJECT = Viper.Functional.LazySeq.Range(10, 12, 1)
DIM sc AS OBJECT = sa.Concat(sb)
PRINT sc.Count()

' --- Reset ---
PRINT "--- Reset ---"
DIM s6 AS OBJECT = Viper.Functional.LazySeq.Range(1, 3, 1)
PRINT s6.Count()
s6.Reset()
PRINT s6.Index
