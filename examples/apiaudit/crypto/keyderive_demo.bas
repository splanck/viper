...
PRINT "--- Pbkdf2SHA256 ---"
DIM derived AS OBJECT = Zanna.Crypto.KeyDerive.Pbkdf2Sha256("mypassword", salt, 1000, 32)
...
DIM derivedStr AS STRING = Zanna.Crypto.KeyDerive.Pbkdf2Sha256Encoded("mypassword", salt, 1000, 32)
PRINT "Derived key (hex): "; derivedStr

' --- Same password + salt => same result ---
PRINT "--- Determinism check ---"
DIM derivedStr2 AS STRING = Zanna.Crypto.KeyDerive.Pbkdf2Sha256Encoded("mypassword", salt, 1000, 32)
PRINT "Same inputs produce same output: "; derivedStr = derivedStr2

' --- Different password => different result ---
PRINT "--- Different password ---"
DIM derivedStr3 AS STRING = Zanna.Crypto.KeyDerive.Pbkdf2Sha256Encoded("otherpassword", salt, 1000, 32)
...
DIM derivedStr4 AS STRING = Zanna.Crypto.KeyDerive.Pbkdf2Sha256Encoded("mypassword", salt2, 1000, 32)
PRINT "Different salt: "; derivedStr4
PRINT "Different from first: "; derivedStr <> derivedStr4

' --- Different key length ---
PRINT "--- Different key length (64 bytes) ---"
DIM derived64 AS OBJECT = Zanna.Crypto.KeyDerive.Pbkdf2Sha256("mypassword", salt, 1000, 64)
PRINT "Key length: "; Zanna.Collections.Bytes.get_Length(derived64)

' --- Different iterations ---
PRINT "--- Different iterations (2000) ---"
DIM derivedStr5 AS STRING = Zanna.Crypto.KeyDerive.Pbkdf2Sha256Encoded("mypassword", salt, 2000, 32)
