...
DIM derivedStr AS STRING = Viper.Crypto.KeyDerive.Pbkdf2Sha256Encoded("mypassword", salt, 1000, 32)
PRINT "Derived key (hex): "; derivedStr

' --- Same password + salt => same result ---
PRINT "--- Determinism check ---"
DIM derivedStr2 AS STRING = Viper.Crypto.KeyDerive.Pbkdf2Sha256Encoded("mypassword", salt, 1000, 32)
PRINT "Same inputs produce same output: "; derivedStr = derivedStr2

' --- Different password => different result ---
PRINT "--- Different password ---"
DIM derivedStr3 AS STRING = Viper.Crypto.KeyDerive.Pbkdf2Sha256Encoded("otherpassword", salt, 1000, 32)
...
DIM salt2 AS OBJECT = Viper.Crypto.SecureRandom.Bytes(16)
DIM derivedStr4 AS STRING = Viper.Crypto.KeyDerive.Pbkdf2Sha256Encoded("mypassword", salt2, 1000, 32)
...
DIM derivedStr5 AS STRING = Viper.Crypto.KeyDerive.Pbkdf2Sha256Encoded("mypassword", salt, 2000, 32)
