...
Start the ViperSQL database server:
...
```bash
viper run examples/apps/vipersql/client/
```

```
vsql v0.2.0 (ViperSQL interactive client)
...
```
ViperSQL v0.2.0 — Standalone REPL (in-memory)
...
```bash
viper build examples/apps/vipersql/main.zia -o vipersql
./vipersql
```
...
The ViperSQL server accepts these command-line options:
...
```bash
viper run examples/apps/vipersql/ -- --port 5433 --log-queries
```
