...
The chess demo has been upgraded from a polished single-screen demo into a release-style game shell while staying fully in Zia and `Zanna.Graphics.Canvas`.
...
- Persistent settings through `Zanna.IO.SaveData`.
...
- Persistent stats and achievements through `Zanna.Game.AchievementTracker`.
...
```sh
zanna check examples/games/chess --diagnostic-format=json
