...
**Description:**  
Global variables are completely isolated from SUB/FUNCTION procedures. Each procedure sees a separate zero-initialized copy instead of the actual global.
...
**Description:**  
The NEW operator for instantiating classes causes a runtime error. The automatically-generated constructor (`__ctor`) doesn't properly return a value, causing a "ret: missing result" error.
...
**Description:**  
When a CLASS defines an explicit `SUB NEW()` constructor, the compiler generates code that causes a "ret: missing result" error at runtime.
...
**Description:**  
String comparison using `>` and `<` operators causes runtime errors in certain contexts, particularly in sorting functions. The runtime function `@rt_str_gt` appears to be missing or not properly linked.
...
**Context:**  
The `basic_audit.md` claims "Full lexicographic string comparison now supported for all relational operators", but this appears to be incomplete or broken in certain contexts (arrays, complex expressions).
...
**Description:**  
The parser does not recognize the underscore (`_`) character for line continuation in function/subroutine parameter lists or other multi-line statements.
