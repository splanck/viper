...
ViperOS implements a complete UEFI boot infrastructure with a custom bootloader (VBoot) and a two-disk architecture for clean separation of system and user content.
...
VBoot is a custom UEFI bootloader that loads the ViperOS kernel on AArch64 systems. It's implemented as a freestanding UEFI application with no external dependencies.
...
3. **Load Kernel** - Reads `\viperos\kernel.sys` into 4MB buffer
...
ViperOS uses separate disks for system and user content to enable:
...
mmd -i esp.img ::EFI ::EFI/BOOT ::viperos
mcopy -i esp.img BOOTAA64.EFI ::EFI/BOOT/
mcopy -i esp.img kernel.sys ::viperos/

# Create system disk
mkfs.viperfs -s 2097152 sys.img
# Copy servers...

# Create user disk
mkfs.viperfs -s 8388608 user.img
# Copy programs...
```
...
`scripts/build_viperos.sh` (691 lines)
...
```bash
./scripts/build_viperos.sh [options]
...
4. **Create Disk Images**:
   ...
   - Create sys.img with mkfs.viperfs
   - Create user.img with mkfs.viperfs
...
| `scripts/build_viperos.sh` | Build orchestration |
