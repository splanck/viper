# ViperOS — Technical Specification v1.4
...
| # | Section                                           | Description                                 |
|---|---------------------------------------------------|---------------------------------------------|
| 1 | [Overview](#1-overview)                           | What ViperOS is, target platform, goals     |
...
ViperOS is the native operating system for the Viper Platform. Its purpose is to provide a complete runtime environment
where Viper applications execute directly on hardware, without requiring a host operating system.

Today, Viper applications run on Linux, Windows, and macOS through the Viper runtime. ViperOS eliminates this
dependency. Applications written in ViperLang, Viper BASIC, or Viper Pascal compile to Viper IL and execute natively on
ViperOS, with the kernel providing memory management, scheduling, I/O, and graphics services directly.

```
┌────────────────────────────────────────────────────────────────┐
│  Viper Application (.vpr)                                      │
│  (ViperLang / Viper BASIC / Viper Pascal)                     │
├────────────────────────────────────────────────────────────────┤
...
The ultimate goal of ViperOS is to fully host the Viper Platform:

- **Self-hosting:** ViperIDE, the Viper compiler, and all development tools run natively on ViperOS
...
ViperOS is a single-architecture operating system. This deliberate constraint enables:
...
The Viper Platform is not a guest on ViperOS—it is the reason ViperOS exists. Viper IL is the syscall boundary.
...
EL1 - Kernel (ViperOS microkernel)
...
ViperOS parses the DTB provided by firmware/vboot to discover:
...
This section defines the exact files present on a freshly installed ViperOS system.
...
Shell scripts are written in Viper BASIC, the native scripting language of ViperOS.
...
```basic
' ViperOS Startup Script
Print "ViperOS starting..."
...
ViperOS development uses QEMU's `virt` machine for AArch64:
...
- SDL window showing ViperOS display
...
**Goal:** ViperOS fully hosts the Viper Platform.

The ultimate vision:

- **ViperIDE** runs natively on ViperOS
- **Viper compiler** builds applications on ViperOS itself
- **Full runtime library** — all Viper.* namespaces functional
- **Development workflow** — edit, compile, run, debug without leaving ViperOS
- **Viper Computer** — ViperOS ships as the default OS on Viper hardware

At this point, ViperOS becomes self-sustaining: you can develop Viper applications on ViperOS, for ViperOS, using tools
