...
// Client input channel (for sending keyboard events to connected client)
static int32_t g_client_input_channel = -1;
                           ...
                           uint32_t *handles, uint32_t handle_count)
{
    ...
    switch (msg_type)
    {
        ...
        case CON_CONNECT:
        {
            ...
            // Store the client's input channel (handle[1] - client's recv endpoint stays with client)
            // handles[0] is the reply channel, handles[1] (if present) is the input channel
            if (handle_count > 1 && handles[1] != 0xFFFFFFFF)
            {
                // Close old input channel if any
                if (g_client_input_channel >= 0)
                {
                    sys::channel_close(g_client_input_channel);
                }
...
extern "C" void _start()
{
    ...
    while (true)
    {
        ...
        while (gui_poll_event(g_window, &event) == 0)
        {
            ...
            {
                // Forward keyboard events to connected client
                if (g_client_input_channel >= 0)
                {
                    InputEvent input_ev;
                    input_ev.type = CON_INPUT;
                    input_ev.ch = keycode_to_ascii(event.key.keycode, event.key.modifiers);
                    input_ev.pressed = 1;
                    input_ev.keycode = event.key.keycode;
                    input_ev.modifiers = event.key.modifiers;
                    input_ev._pad[0] = 0;
                    input_ev._pad[1] = 0;
                    input_ev._pad[2] = 0;
