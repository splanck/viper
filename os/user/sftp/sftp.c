...
static void cmd_put(const char *local, const char *remote)
{
    ...
    const char *remote_path = remote ? join_path(g_cwd, remote)
                                     : join_path(g_cwd, basename_of(local));
    ...
    sftp_file_t *rf = sftp_open(g_sftp, remote_path,
                                 SFTP_WRITE | SFTP_CREAT | SFTP_TRUNC, 0644);
    if (!rf) {
...
static void cmd_rename(const char *oldpath, const char *newpath)
{
    ...
    int rc = sftp_rename(g_sftp,
                         join_path(g_cwd, oldpath),
                         join_path(g_cwd, newpath));
    if (rc != SFTP_OK) {
