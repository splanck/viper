...
static int add_inst(struct re_compiled *rc, int op, int arg) {
    if (rc->ninsts >= rc->capacity) {
        ...
        struct re_inst *new_insts = realloc(rc->insts,
                                             new_cap * sizeof(struct re_inst));
...
int regcomp(regex_t *preg, const char *regex, int cflags) {
    ...
    while (*p) {
        if (*p == '^') {
            if (add_inst(rc, OP_BOL, 0) < 0) goto nomem;
            p++;
        } else if (*p == '$') {
            if (add_inst(rc, OP_EOL, 0) < 0) goto nomem;
            p++;
        } else if (*p == '.') {
            if (add_inst(rc, OP_ANY, 0) < 0) goto nomem;
            p++;
        } else if (*p == '*') {
            ...
            if (add_inst(rc, negated ? OP_NCLASS : OP_CLASS, 0) < 0) goto nomem;
            ...
            if (add_inst(rc, OP_GROUP_START, group_num++) < 0) goto nomem;
            preg->re_nsub++;
            p++;
        } else if (extended && *p == ')') {
            if (add_inst(rc, OP_GROUP_END, 0) < 0) goto nomem;
            p++;
        } else if (*p == '\\') {
            p++;
            if (*p == 0) {
            ...
            if (cflags & REG_ICASE) c = tolower(c);
            if (add_inst(rc, OP_CHAR, c) < 0) goto nomem;
        } else {
            int c = (unsigned char)*p++;
            if (cflags & REG_ICASE) c = tolower(c);
            if (add_inst(rc, OP_CHAR, c) < 0) goto nomem;
    ...
    if (add_inst(rc, OP_END, 0) < 0) goto nomem;
    return 0;
...
static int match_here(const struct re_inst *insts, const char *s,
                      const char *text, size_t nmatch, regmatch_t *pmatch,
                      int cflags) {
    if (insts->op == OP_END) return 1;
        ...
        do {
            if (match_here(insts + 1, s, text, nmatch, pmatch, cflags))
                return 1;
                       ...
                       ((cflags & REG_ICASE) ?
                        tolower((unsigned char)*s) == insts->arg :
                        (unsigned char)*s == insts->arg) ||
                       (insts[-1].op == OP_ANY) ||
                       (insts[-1].op == OP_CLASS &&
                        get_class_bit(insts[-1].cclass, (unsigned char)*s))) &&
                 s++);
        return 0;
    }
...
int regexec(const regex_t *preg, const char *string,
    ...
    if (!preg || !string || !preg->__re_comp) return REG_BADPAT;

    struct re_compiled *rc = preg->__re_comp;
    if (rc->ninsts == 0) return REG_BADPAT;
    ...
    while (*s || s == string) {
        if (match_here(rc->insts, s, text, nmatch, pmatch, cflags)) {
            if (!(cflags & REG_NOSUB) && nmatch > 0 && pmatch) {
                ...
                /* Find end of match */
                const char *e = s;
                while (*e) e++;  /* Simplified: match to end */
