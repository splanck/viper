...
- context creation contains asynchronous GLX errors, requires an actual OpenGL 3.3 core profile, validates draw-buffer, attachment, texture-unit, vertex-attribute, UBO, and texture-buffer limits, and verifies required shader uniforms/blocks after linking; bloom, TAA, and SSR programs degrade independently when unsupported, while full compile/link logs and optional `KHR_debug` messages remain available through `VIPER_OPENGL_DEBUG=1`
- the clip-depth convention is context-local, normal rendering entry points enforce the creating thread as context owner, and teardown attempts an explicit ownership transfer before releasing host/context storage
...
- offscreen ViperGFX presentation uses fenced double pixel-pack-buffer readback, reuses the last completed frame rather than stalling on a busy transfer, checks update errors before counting a present, and preserves framebuffer, viewport, pixel-store, scissor, color-mask, and sRGB state around helper passes
...
- terrain splat layer 3 retains its own sampler; environment lighting uses the shader's analytic BRDF approximation instead of aliasing the terrain texture as a lookup table
