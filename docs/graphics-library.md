...
The clipboard helpers operate on UTF-8 text and back the higher-level `Viper.GUI.Clipboard` and `Viper.System.Clipboard` runtime surfaces. Linux/X11 uses the modern `CLIPBOARD` selection with `UTF8_STRING` conversion rather than legacy cut buffers.
