...
| `--break-src <file>:<line>` | halt before executing the instruction at source line; path is normalized (`\\`→`/`, collapse `./` and `dir/../`) and matched against full path or basename; may be repeated. |
...
The file path is normalized and compared to both the full path and the basename recorded in the IL.
