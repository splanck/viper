...
- [x] Regalloc liveness builds CFG successors from real block terminators, ignoring in-block select/local labels.
...
- **Liveness CFG:** local labels generated inside a block, such as branchy f64 select expansions, do not create
  inter-block successors. Only the terminating `JCC` / `JMP` / `RET` sequence defines block CFG.
...
- **Target-aware DCE:** peephole DCE uses the selected `TargetInfo`, so SysV and Win64 call argument live-ins differ
  for both GPR and XMM registers. EFLAGS-producing instructions are preserved only when a later conditional branch,
  setcc, or cmov consumes the flags.
- **Frame memory peepholes:** simple RBP-relative stores and reloads are forwarded to register moves when no barrier,
  ...
  later store fully overwrites them before any load or memory barrier.
