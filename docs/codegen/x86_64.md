...
- **Checked FP casts:** `cast.fp_to_si.rte.chk` and `cast.fp_to_ui.rte.chk` call `rt_round_even`, perform explicit NaN
  and range checks, then report `InvalidCast` through `rt_trap_raise_error`.
- **Structured traps:** `idx.chk` and checked-cast failures now use `rt_trap_raise_error` instead of raw `ud2`.
