...
- Can't be a keyword (like `var`, `func`, `if`)
...
var canEnter = age >= 18 && hasTicket;  // true (both conditions met)
```
...
If you try to change a `final` variable later, Viper will give you an error. This prevents accidental bugs and makes your intentions clear.

By convention, constants are named in `UPPER_CASE` to distinguish them from mutable variables.
...
func start() {
    ...
    var ageText = Viper.Terminal.ReadLine();
    var age = Viper.Parse.Int(ageText);
...
- **Variables** are names attached to values, created with `var`
- **Constants** are unchangeable values, created with `final`
