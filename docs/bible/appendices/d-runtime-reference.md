...
For human-oriented documentation, see `docs/viperlib/README.md` and the topic pages under `docs/viperlib/`.
...
```bash
build/src/tools/viper/viper check path/to/file.zia --diagnostic-format=json
build/src/tools/viper/viper run path/to/file.zia --diagnostic-format=json
build/src/tools/viper/viper --dump-runtime-api
build/src/tools/viper/viper --dump-opcodes
build/src/tools/viper/viper explain V-ZIA-UNDEFINED --json
```

`viper run` currently accepts `.zia`, `.bas`, project directories, and `viper.project` targets. IL snippets in the Bible are illustrative unless they are wrapped in a runnable frontend example.
