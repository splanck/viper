...
AppImage artifacts are Viper's FUSE-less self-extracting runtime format. They support `--appimage-help`, `--appimage-extract` (extracts to `./squashfs-root`), and `VIPER_APPIMAGE_CLEAN_CACHE=1` to force cache refresh before launch.
...
| Option | Description |
|--------|-------------|
| `--target windows|macos|linux-deb|linux-rpm|appimage|tarball|all|all-available|macos-dmg` | Select artifact format(s); `macos-dmg` aliases `--target macos --macos-dmg` when building |
...
Staged toolchain packaging accepts `x64` and `arm64` architecture names, and also accepts `universal` for detected macOS fat32 and fat64 Mach-O toolchains. It requires a package version from `lib/cmake/Viper/ViperConfigVersion.cmake` or `include/viper/version.hpp`; CMake package path validation is case-insensitive for staged filesystems that vary directory casing. Linux `.deb` output maps architectures to `amd64` and `arm64`; RPM output maps them to `x86_64` and `aarch64`. RPM generation requires `rpmbuild`; Linux `--target all` includes `.deb`, `.rpm`, `.AppImage`, and portable tarball output and fails with an actionable diagnostic if `rpmbuild` is missing, so release builders do not silently skip an expected artifact. Linux `--target all-available` emits the same set but skips RPM when `rpmbuild` is absent. For Windows and macOS stages, `--target all` and `--target all-available` emit the staged platform's native package type plus a portable tarball. When the staged `viper` binary has a recognizable Mach-O, ELF, or PE header, `install-package` derives both manifest platform and architecture from that binary and rejects a conflicting `--arch` override. An explicitly incompatible target such as `--target windows` for an ELF stage fails before writing an artifact. Portable toolchain tarball top directories use the same filesystem-safe version component as app tarballs. Linux-platform tarballs include `install.sh`, `uninstall.sh`, `README.install`, hicolor app icons, and `share/viper/install_manifest.txt`; by default the scripts install under `/usr/local`, honor `PREFIX` and `DESTDIR`, preserve staged modes/symlinks through `tar`, remove stale files from the prior tarball manifest before copying the new payload, refresh man/MIME/desktop caches when installing directly on a host, and uninstall only the files listed in the manifest. `-o <path>` is treated as an output directory when it already exists as a directory or when multiple targets are selected; otherwise the parent directory of the requested output file is created before writing.

Staged toolchain packaging rejects symlinks whose resolved targets leave the staged prefix, including when `--stage-dir` itself is a symlink and CMake's install manifest records paths through that alias. Relative internal symlink targets are preserved as written in tar-based artifacts and in macOS package roots; absolute internal symlinks are converted to archive-relative targets. Linux `.deb`, `.rpm`, `.AppImage`, and Linux-platform tarball outputs preserve staged Unix permission bits, map root-level docs such as `LICENSE` to `/usr/share/doc/viper/`, include a visible `viperide.desktop` launcher, include hicolor PNG icons, include hidden desktop/MIME metadata for the default Viper file associations, and refresh MIME, desktop, and manpage caches from post-removal maintainer hooks. Installed Linux packages open source associations through `/usr/bin/viper run %f` and IL associations through `/usr/bin/viper -run %f`; portable Linux tarballs use `viper run %f` and `viper -run %f` so the entries do not hard-code `/usr/bin`. File associations intentionally stay with the CLI runner; ViperIDE is shipped as a launcher and command until it supports file-open arguments. The toolchain AppImage launches `viperide` when run without arguments, dispatches arguments to the CLI, and validates a payload hash stamp before reusing its XDG cache extraction. Debian toolchain packages use a valid maintainer field and declare hard dependencies for libc/C++/libgcc, CMake, `make`, a C++ compiler, `desktop-file-utils`, `shared-mime-info`, and `man-db`, plus X11 or ALSA when the staged support libraries require them. RPM specs declare matching hard `Requires`, including `cmake`, `gcc-c++`, `make`, `desktop-file-utils`, `shared-mime-info`, and `man-db`, so package-manager installs set up the developer workflow instead of leaving compiler/CMake setup as a weak recommendation. RPM `%install` copies the full staged source tree, including top-level dotfiles, and `%files` entries are quoted and escaped for paths that contain spaces, quotes, backslashes, or percent signs. RPM output discovery accepts distribution release suffixes added by `%{?dist}`.

Windows toolchain installers dereference only symlinks to regular files and reject directory symlinks with a diagnostic because Windows installer payloads do not carry POSIX symlink metadata. The toolchain installer installs a compressed payload archive, expands it into the selected install root, writes `.viper-install-manifest.txt`, compares that manifest against the previous install manifest, and removes only Viper-owned stale files after the new payload expands successfully. It adds the installed `bin` directory to the selected user or machine `Path`, records the previous value in uninstall metadata only when it actually appends the entry, skips re-adding an existing Viper-owned PATH entry on reinstall, removes that PATH token during uninstall while preserving unrelated user edits, broadcasts an environment change, creates Start Menu shortcuts for the developer prompt, ViperIDE, and the VS Code extension installer only when a `.vsix` is staged, and registers default Viper file associations by default. `.zia` and `.bas` files open through `"bin\viper.exe" run "%1"`; `.il` files open through `"bin\viper.exe" -run "%1"`. The wizard reflects the selected install scope instead of presenting mutable scope choices, uses the packaged Viper icon in shortcuts and Add/Remove Programs, and tells users to open a fresh terminal after PATH changes. The uninstaller schedules only the running `uninstall.exe` for reboot-time deletion when necessary and leaves any non-empty install root in place after removing owned files. Generated toolchain installers include `share/viper/README.windows-prerequisites.txt`; Release builds that depend on the MSVC dynamic runtime require the Microsoft Visual C++ Redistributable for Visual Studio 2015-2022 unless the staged `bin` directory includes the required runtime DLLs or the toolchain was built with the static MSVC runtime. Optional staged support libraries such as `vipergfx`, `vipergui`, and `viperaud` are required only when the staged CMake metadata references them.

For manual clean-VM Windows validation, copy the generated installer to the VM and run `scripts/validate-windows-toolchain-installer.ps1 -Installer <installer.exe>`. The script installs the package, checks every required binary including `viperide.exe`, runs `viper --version`, runs `viperide --version`, verifies `viper run`, compiles and runs a native codegen smoke outside the build tree, projects the registry PATH into a fresh process, and uninstalls unless `-KeepInstalled` is supplied.

macOS toolchain packages are generated without `pkgbuild` or `productbuild`: Viper writes the component `Payload` as gzip-compressed portable ASCII CPIO, assembles component/product XAR archives natively, and uses `mkbom` only for the component bill of materials on macOS hosts. The package installs the staged tree under `/usr/local/viper`, includes receipt-owned `/usr/local/bin` command symlinks for every binary tool including `viperide`, includes `/usr/local/share/man` manpage symlinks, and installs `/usr/local/lib/cmake/Viper` wrapper configs so `find_package(Viper CONFIG REQUIRED)` works from CMake's default `/usr/local` prefix. It also installs `/Applications/Viper Toolchain.app`, a LaunchServices document handler for `.zia`, `.bas`, and `.il` files that opens the installed CLI in Terminal and carries the Viper icon; this app is not ViperIDE. Preinstall/postinstall scripts remove stale files from the prior Viper manifest, refresh command/manpage symlinks, register the file-handler app, and update manpage caches when available. `/usr/local/viper/share/viper/uninstall.sh` unregisters the handler app, removes receipt-created symlinks and CMake wrappers, removes files listed in the Viper install manifest, and forgets the package receipt for local cleanup. Use `--macos-pkg-version` when the Viper version contains suffixes such as `+build` or `~rc1`; the generator rejects lossy automatic truncation. `--macos-sign-identity` signs the finished product package with `productsign`; `--macos-notary-profile` submits it with `notarytool --wait`, and `--macos-staple` staples the resulting ticket. When `--macos-dmg` is used, the generated `.dmg` is verified with `hdiutil` during build and can be submitted and stapled with the same notary options after the `.pkg` is produced. `VIPER_MACOS_SIGN_IDENTITY` and `VIPER_MACOS_NOTARY_PROFILE` can provide the signing identity and notary profile when the corresponding workflow is requested.
