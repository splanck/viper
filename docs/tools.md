...
Windows installers register ProgIDs under `HKLM\Software\Classes` and advertise them through `OpenWithProgids` without overwriting the existing default handler for an extension. Existing extension `Content Type` values are preserved; new MIME `Content Type` values are tagged with a VAPS owner marker and only that owned value is removed during uninstall. `OpenWithProgids` entries are written as zero-length `REG_NONE` values, and uninstall cleanup removes VAPS-owned OpenWith, ProgID, MIME `Content Type`, uninstall metadata, shortcuts, and installed files. ProgID cleanup checks the stored VAPS owner marker value before deleting the tree. Desktop and Start Menu shortcuts are created from package overlay `.lnk` files that use `%ProgramFiles%` expansion instead of a fixed `C:\Program Files` path; when `package-icon` is set, a generated `.ico` is installed next to the app and used by shortcuts. Windows installers and uninstallers request elevation; `min-os-windows` adds supported-OS compatibility manifest entries. Generated installer stubs reserve long-path buffers, bounds-check composed registry commands, stream overlay extraction in fixed-size chunks, check file and directory removal failures, and do not schedule the installation directory itself for deletion on reboot.
...
Staged toolchain packaging accepts only `x64` and `arm64` architecture names. Linux `.deb` output maps them to `amd64` and `arm64`; RPM output maps them to `x86_64` and `aarch64`. When the staged `viper` binary has a recognizable Mach-O, ELF, or PE header, `install-package` derives the manifest architecture from that binary and rejects a conflicting `--arch` override. Staged toolchain packaging rejects symlinks whose resolved targets leave the staged prefix. Relative internal symlink targets are preserved as written in tar-based artifacts and in macOS package roots; absolute internal symlinks are converted to archive-relative targets. Windows toolchain installers dereference only symlinks to regular files and reject directory symlinks with a diagnostic because Windows installer payloads do not carry POSIX symlink metadata. Optional staged support libraries such as `vipergfx`, `vipergui`, and `viperaud` are required only when the staged CMake metadata references them. RPM `%files` entries are emitted with quoting and escaping for paths that contain spaces, quotes, backslashes, or percent signs.

`install-package --verify-only` infers formats only from supported extensions: `.exe`, `.pkg`, `.deb`, `.rpm`, `.tar.gz`, and `.tgz`. Unknown extensions fail unless a supported `--target` is provided. Post-build verification for generated Windows, Debian, and tarball toolchain artifacts checks every staged manifest path in the emitted payload. `.pkg` verification checks the XAR header, inflates and validates the TOC, and requires a package payload entry. `.rpm` verification checks the RPM lead, signature header, main header bounds, and that a non-empty payload follows the headers.
