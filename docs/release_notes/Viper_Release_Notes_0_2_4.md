...
- **Metal / D3D11 / OpenGL Shared Backend Helpers** — Extracted `vgfx3d_backend_metal_shared.{c,h}` (190 + 98 LOC), `vgfx3d_backend_d3d11_shared.{c,h}` (180 + 117 LOC), and `vgfx3d_backend_opengl_shared.{c,h}` (126 + 77 LOC), factoring out per-object/per-material/per-scene constant buffer layouts, packed morph weight encoding, blend mode and target kind enums, color format enums, and frame-history tracking (`vgfx3d_{metal,opengl}_update_frame_history` for motion blur and temporal effects). The main `vgfx3d_backend_metal.m`, `vgfx3d_backend_d3d11.c`, and `vgfx3d_backend_opengl.c` files were rewritten against these shared headers (~900–1400 lines changed each). New unit tests (`test_vgfx3d_backend_{metal,d3d11,opengl}_shared.{c,m}`) directly exercise the shared helpers without requiring a live GPU context.
- **Metal Direct vs GPU-Postfx Window Paths + Cached RTT** — Metal now splits window rendering into a direct path (draws straight into the `CAMetalLayer` drawable when GPU postfx is disabled) and a postfx path (renders the scene into an HDR `RGBA16F` target, records overlays into a separate UNORM color/motion/depth triplet, and composites through the Metal postfx pass at `Flip`). `VGFXMetalRenderTargetCacheEntry` caches offscreen textures plus in-flight command buffers per `vgfx3d_rendertarget_t` with lazy CPU sync, eliminating the per-frame `getBytes` stall at `End`. `VGFXMetalMorphCacheEntry` caches packed delta / normal-delta buffers keyed by `(morph_key, morph_revision, shape_count, vertex_count)` with age-based pruning alongside the texture and cubemap caches; morph normal deltas are now applied in the MSL vertex path.
- **Metal Skybox Fullscreen-Triangle Rewrite** — Replaced the explicit unit-cube vertex buffer with a three-vertex fullscreen triangle generated from `vertex_id`. The fragment shader reconstructs the world-space view direction via `inverseProjection * clip` → normalize → `inverseViewRotation` and samples the cubemap. `mtl_skybox_params_t` now carries `inverseProjection` / `inverseViewRotation` instead of forward matrices, the `create_skybox_vertex_descriptor()` helper is gone, and the pipeline's `vertexDescriptor` is now `nil`. Avoids the legacy cube-vertex upload path and its rear-face depth-fighting.
...
- **Windows x86_64 Codegen Hardening** — CoffWriter cross-section symbol resolution, X64BinaryEncoder runtime symbol mapping, operand materialisation for TESTrr/call.indirect, SETcc REX prefix for byte registers, SSE RIP-relative MOVSD encoding, unsafe spill slot reuse disabled, and process isolation hang fix. Windows native executables now assemble, link, and run correctly. Follow-up pass: MSVC is now the default Windows compiler (clang-cl is explicit opt-in via `VIPER_WINDOWS_COMPILER=clang-cl`), the `scripts/build_viper_win.cmd` CMake generator argument is properly quoted, public headers install under `${includedir}` for both the main library and TUI targets, Windows native link planning + runtime packaging cover the additional runtime components with safer manifest path handling, COFF relocation resolution is tightened, and the Windows import planner names are expanded.
- **D3D11 Bowling Stack Fixes** — Canvas3D and the D3D11 backend now size the swapchain and render targets from the physical framebuffer size so DPI-scaled Windows displays no longer upscale/misalign. Win32 background erases no longer repaint GPU-backed windows black between frames. Shared shadow orthographic projection Z sign corrected so the D3D11 shadow map receives valid depth values. D3D11 shader matrix multiplication order, shadow sampling, fullscreen postfx raster state, overlay clearing, readback source selection, and post-processing parity with Metal/OpenGL all fixed in a single pass (+109 LOC in `vgfx3d_backend_d3d11.c`). `IDXGISwapChain::Present` failures are now logged instead of silently ignored, and the missing `direct.h` include is restored for tempfile support on Windows.
...
- **Demos & Documentation** — XENOSCAPE sidescroller (17K LOC), 3D bowling (3.1K LOC), new 3D world starter demo (~760 LOC — procedural terrain with layered grass/dirt/rock, generated gradient skybox, third-person follow camera with terrain-clamped height, WASD + mouse look + Q/E orbit, four ridge beacons, 2D HUD), ViperSQL (10 SQL features, runtime API migration), ViperIDE professional IDE (live diagnostics, hover, go-to-def, search, symbol outline, 21 files / 7 dirs), Chess (pre-rendered sprites, core/engine/ui), 8 Graphics3D API demos, 6 app/game smoke probes; 3D bowling pixel-packing fix (`effects/particles.zia` and `engine/renderer.zia` were packing colors via `Color.RGBA()` which returns `0xAARRGGBB`, but `Pixels.Set()` expects `0xRRGGBBAA` — both files now use a local `packPixel` helper emitting the correct byte order, with `RTPixelsDrawTests::test_color_rgba_packing_differs_from_pixels_storage` as a regression test); 8 new D3D11 diagnostic probes (`examples/apiaudit/graphics3d/minimal_unlit_probe.zia`, plus `color_probe`, `scene_nopostfx_probe`, `scene_nopostfx_smoke`, `scene_postfx_probe`, `title_nopostfx_probe`, `title_nopostfx_smoke`, `title_postfx_probe` in `examples/games/3dbowling/`) exercising minimal unlit rendering and the bowling title/scene postfx vs no-postfx paths; comprehensive 186-file docs audit (130 with frontmatter date-bumped to 2026-04-09; ~15 substantive content fixes including arithmetic-semantics / il-{guide,reference,quickstart,passes} / frontend-howto opcode policy corrections — plain `add`/`sub`/`mul`/`sdiv`/`udiv`/`srem`/`urem` are verifier-rejected for signed integers per `SpecTables.cpp:55-152` and only `iadd.ovf` / `sdiv.chk0` / etc. are legal; BASIC `DISPOSE`→`DELETE`, RESUME 0 description, TIMER return type, frontend-howto post-`std::move` Instr UB, missing IRBuilder methods, debugging.md `eh.push`/`eh.pop` model corrections, dependencies.md runtime path reorganization); 700+ Doxygen comments, prior 70+ factual error pass; full per-file worklist at `misc/plans/docs-audit-20260409.md`.
