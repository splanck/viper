...
handling, compound assignments, and an 18-code warning system. A new `zanna package` command
...
#### `zanna package` — Cross-Platform Application Packaging (ZAPS)

New subcommand generates platform-native installers with zero external dependencies:

```bash
zanna package myapp.zanna.project --target macos
zanna package myapp.zanna.project --target linux-deb
zanna package myapp.zanna.project --target windows
zanna package myapp.zanna.project --target tarball
```
...
**Manifest directives** in `zanna.project`:

```ini
package-name = MyApp
package-author = Jane Developer
package-description = A cross-platform Zanna application
...
- `build_zanna.sh` rewritten as cross-platform bash script with OS/compiler auto-detection
...
### Adversarial Testing — ZannaSQL

Comprehensive adversarial testing of the ZannaSQL showcase uncovered and fixed 15 bugs:
...
- **ZANNA_TRAP_ASSERT**: Now checks conditions in release builds (was `((void)0)`)
...
- **Test dialog suppression**: `WinDialogSuppress.c` CRT initializer + `_CrtSetReportMode`
  enhancements + `ZANNA_DISABLE_ABORT_DIALOG()` macro + CMake injection into all test targets
...
- `docs/release_notes/Zanna_Release_Notes_0_2_1.md` — v0.2.1 release notes (870 lines)
...
- Updated zannalib docs: BigInt, Mat3, Mat4, Xml, Yaml, Diagnostics, Parse, GC, Channel, Pool, Aes
...
| Application Packaging     | No                   | zanna package (.app/.deb/.exe)   |
...
*Zanna Compiler Platform v0.2.2 (Pre-Alpha)*
*Released: March 2026*
