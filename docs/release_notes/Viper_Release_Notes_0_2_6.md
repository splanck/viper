...
| Production SLOC | 552K | 629K | +77K |
| Test SLOC | 228K | 263K | +35K |
| Demo SLOC | 188K | 189K | +1K |

Counts via `scripts/count_sloc.sh` (production 629,354 / test 262,719 / demo 189,454 / source files 3,058).
...
- Canvas3D/Game3D fixes align shading-model constants, promote `SetShadingModel(2)` to PBR, add `Camera3D.ScreenToRayOrigin`, present GPU post-FX frames through the backend compositor even when final overlays are recorded, composite screen-space overlays through a depth-test-bypass sub-pass that draws over the scene without disturbing scene depth and sizes them in logical coordinates so HUD layout stays stable on HiDPI displays, snapshot Game3D input per frame, reverse winding for mirrored mesh transforms, stream exact binary STL files, clamp synthetic mouse/delta-time extremes, and tighten backend-light/material/draw-queue validation while preserving opaque front-to-back sorting.
- `examples/games/game3d-showcase` now uses a backend-stable balanced daylight/fill rig, higher-contrast terrain and water materials, wrapped HUD controls, GPU post-FX gamma/color-grade parity, and smoke coverage that catches magenta frames, missing final overlays, unreadably dark or overexposed foregrounds, and HUD text overflow.
...
- A software-rasterizer and shared-math performance pass: CPU skinning precomputes each bone's normal matrix once rather than per vertex per influence, the software terrain-splat setup and unbound per-pixel UV interpolation are hoisted out of the inner pixel loop, environment reflections reuse the already-perturbed shading normal, and frustum culling validates its planes once per extraction instead of per object; paired with correctness fixes for a morphed-draw use-after-free (the original mesh is now retained for the deferred draw) and an over-aggressive normal-matrix singularity threshold that dropped the rotation of very small-scaled objects.
- A Game3D/Graphics3D correctness pass tightens callback validation, body/scene transform sync, BodyDef static/kinematic semantics, collision layer masks, fixed-step drop diagnostics, and Sound3D listener orientation; asset importers now walk deep glTF trees iteratively, accept percent-encoded/data-URI buffers, and avoid huge binary-STL upfront reserves. Physics3D query paths reuse the broadphase, expose CCD cap diagnostics, scale collider queries with entity transforms, cap sweep sampling, and Particles3D reuses per-frame draw buffers without mutating queued materials.
- The 3D hardening pass also makes scene transform invalidation allocation-free with parent revision tracking, replaces duplicate SceneNode3D LOD thresholds instead of accumulating stale meshes, preserves Mesh3D appendability after public validation traps, closes STL geometry batches on failure, lazily allocates GPU render-target CPU mirrors, bounds instanced submission counts, caches Physics3D query broadphases and collision event objects, purges contacts on body removal, and lets split GPU post-FX backends draw final overlays after tonemapping before normal present.
- A follow-up Game3D/Graphics3D pass fixes the `Canvas3D.Poll()` open/closed contract and adds queued `PollEvent()`, keeps Game3D fixed-frame callbacks under synthetic input/time for deterministic callback+simulation+render frames, syncs native resize state during `World3D.tick()`, gates body-from-node transform pushes by sync mode, and hardens `Assets3D` model-template caching against duplicate concurrent loads without polling waits. Physics3D gains `World3D.TryAdd`/`ContainsBody`, Scene3D gains boolean `TryAdd`/`TryAddChild` parenting APIs, query hit-list truncation diagnostics, analytic primitive sphere sweeps, mesh-raycast BVH reuse, adaptive heightfield ray steps, stricter quaternion/layer validation, saturated finite motion state, and failure paths that preserve contact buffers and clear accumulated forces.
...
See `git log v0.2.5-dev..HEAD -- .` for the full 216-commit history since v0.2.5.
