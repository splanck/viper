...
- **D3D11 lifetimes.** Target / overlay / shadow / postfx lifetimes corrected with focused SRV unbinds before resource release. Complete resource-set validation before binding, readback, or RTT dirty-state decisions; target fallback on allocation failure; readback / RTT-sync paths `CopyResource` through unbound output resources and restore the prior binding.
- **D3D11 shaders + uploads.** Shaders: `safeNormalize3` + NDC/UV helpers, weight-normalising skinning, identity-pad bone palettes. Uploads: checked size multiplication on dynamic and instanced buffers; 16-byte / 64 KiB cbuffer bounds; float-SRV updates use a bounded `D3D11_BOX` covering only live elements; morph-cache reuse keyed by normal-delta presence; sampler descriptors initialised from valid defaults; contiguous shadow-slot advertisement sanitises light indices.
...
- **Runtime API conformance — typed alternatives.** New entries close raw-pointer ABI escape valves: `Box.ToI64Option` / `ToF64Option` / `ToI1Option` / `ToStrOption`; `Canvas.PolylinePath` / `PolygonPath` / `PolygonFramePath` accepting a `Path2D`; `Viper.IO.Dir.Dirs` / `Files` / `List` returning `seq<str>`; `File.ReadBytes` / `WriteBytes` / `ReadAllBytes` / `WriteAllBytes` / `ReadLines` / `WriteLines`; `String.SplitFieldsSeq`; `Game.ParticleEmitter.ParticleAt` returning `Optional`; new `Game.ParticleSnapshot` value-type. Sema owner-return inference keeps borrowed accessors (`Keys`, `Values`, `Get*`, `Find`, `Fold`) from over-retaining.
