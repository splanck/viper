...
- **Process / Zia tooling / ViperIDE primitives** — boxed-arg streaming exec; ProjectIndex definition/reference/rename; and focused CTests for workspace index/watcher, asset resolution, manifest parsing, transactional edits, fuzzy match, editable scene flows, scaled tilemap hit-testing, GUI automation, and the headless debug protocol.
