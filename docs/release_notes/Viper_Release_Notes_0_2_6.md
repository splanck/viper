...
| Production SLOC | 552K | 634K | +82K |
| Test SLOC | 228K | 264K | +36K |
| Demo SLOC | 188K | 190K | +2K |

Counts via `scripts/count_sloc.sh` (production 634,314 / test 264,363 / demo 189,784 / source files 3,058).
...
- **Physics3D and queries.** Oriented-box SAT, GJK/EPA convex-hull contacts against hulls and simple primitives, BVH-pruned triangle-mesh contacts against spheres, capsules, and boxes, shape-accurate mesh/convex raycasts and collisions, and adaptive sphere/capsule sweeps for thin geometry; heightfield contacts honour signed, non-uniform vertical scale; correct static/kinematic body semantics and collision-layer masks; enter/stay/exit matching moved from an O(n²) scan to a hash table; cached query broadphases and collision-event objects, mesh-raycast BVH reuse, contact purging on body removal, and a fixed quaternion vector-rotation aliasing bug.
...
- **Performance.** CPU skinning precomputes each bone's normal matrix once instead of per vertex per influence; software terrain-splat setup and per-pixel UV interpolation are hoisted out of the inner loop; environment reflections reuse the perturbed shading normal; frustum planes validate once per extraction; GPU render-target CPU mirrors allocate lazily; `Particles3D` batches alpha and additive billboards as one sorted mesh and reuses per-frame draw buffers; camera controllers update through raw-component math rather than allocating a `Vec3` per frame; Canvas3D motion-history and temp-object tracking move from linear scans to O(1) hash lookups with per-draw blend classification cached once; Scene3D queries and draw culling use an internal spatial index with flat-path parity and generated 10k-node candidate-reduction coverage; and a morphed-draw use-after-free is fixed by retaining the source mesh for the deferred draw.
- **New API surface.** `Canvas3D.Resize`; a queued `Canvas3D.PollEvent()` with a defined open/closed `Poll()` contract that also advances `DeltaTime`, so manual loops can time off `Poll()` rather than `Flip()`; `WindowWidth`/`WindowHeight` and `ActiveOutputWidth`/`ActiveOutputHeight` to distinguish the backing window from a bound render target; `Camera3D.ScreenToRayOrigin`; boolean `World3D.TryAdd`/`ContainsBody` and `Scene3D.TryAdd`/`TryAddChild`; `SceneNode3D.SetAutoLOD` / `SetImpostor` plus `Canvas3D.SetOcclusionCulling`, `DrawCount`, and `OccludedDrawCount` for the CPU visibility path; `Canvas3D.SetClusteredLighting` and `MaxActiveLights` with a software >16-light correctness baseline and 16-light fallback on unsupported backends; `Light3D.CastsShadows` for per-light shadow pass selection; `Material3D.Has*` texture-slot inspection; `TextureAsset3D.LoadKTX2`/`LoadKTX2Asset` with KTX2 metadata, mip residency telemetry through `SetResidentMipRange`, active RGBA8 material fallback selection, and `BackendSupports("bc7"/"astc"/"etc2")` gates; `NavMesh3D.Bake`/`BakeTiled` for scene-geometry navmesh baking, `World3D.bakeNavMesh`/`bakeTiledNavMesh` as Game3D editor hooks over the owned scene, `RebuildTile` as the current whole-mesh refilter baseline, `NavMesh3D.AddOffMeshLink`/`OffMeshLinkCount` for authored traversal edges, `NavMesh3D.AddObstacle`/`RemoveObstacle`/`UpdateObstacle`/`ObstacleCount` for coarse AABB carving edits, `agentRadius` shared-portal gating, plus `NavAgent3D.AvoidanceEnabled`/`AvoidanceRadius` for opt-in same-navmesh local separation; `BlendTree3D.New1D`/`New2D` blendspaces accepted by `Canvas3D.DrawMeshBlended` and controller-bound through `AnimController3D.SetBlendTree` / `Game3D.Animator3D.setBlendTree`; `IKSolver3D.TwoBone`/`LookAt`/`FABRIK` with `AnimController3D.SetIKSolver` and `Game3D.Animator3D.setIKSolver`; `Animation3D.Retarget` for name-first/index-fallback clip remapping; `AnimController3D.PlayLayerAdditive`/`CrossfadeLayerAdditive`, `Game3D.Animator3D.playLayerAdditive`/`crossfadeLayerAdditive`, and `AnimController3D.SetAnimationLOD` for true bind-pose-delta overlays and deterministic animation-rate throttling; `Model3D.SceneCount`, `GetCameraCount`, `GetCamera`, `GetSceneName`, and `InstantiateSceneAt` for immutable scene-indexed asset loading with glTF scene-local camera extraction and secondary scene roots; `Game3D.AssetHandle3D`, `Assets3D.LoadModel*Async` deferred/cancellable/error handles with missing-path, unsupported-extension, and malformed glTF JSON terminal states, template-cache residency controls, `World3D` entity/body/draw/visibility/stream counters, and `WorldStream3D` telemetry plus manifest-driven VSCN scene-cell load/unload, deterministic per-update stream-load budgeting, heightmapped terrain tile payload residency, matching static heightfield collider residency, and hidden terrain nav-bake sources; an `Entity3D`-targeting `OrbitController`; `SetShadingModel(2)` promoted to PBR; exact binary-STL streaming; and normal-map tangent-revision caching.
...
- `examples/3d/openworld_slice/` adds a tested streaming vertical-slice project with cell and heightmapped terrain manifests, rendered resident `Terrain3D` payload access, per-tile static heightfield collider residency, streamed-terrain nav-bake sources, staged `WorldStream3D.update` load-budget telemetry, all-four-quadrant bounded-residency traversal plus repeated long-traversal replay, RGBA8/BC7 KTX2 `TextureAsset3D` usage, a committed synthetic skinned glTF play/crossfade fixture with LookAt IK binding, committed GLB and WAV package-asset fixtures, a terrain-sampled TwoBone foot IK proof, WorldStream3D cell/tile inspection hooks, World3D runtime counter reads and world-scoped navmesh bake, async `AssetHandle3D` model completion, character/physics/nav stepping, software final-frame baseline comparison, capability-gated GPU smoke with a degenerate-basis normal-map robustness pass, deterministic replay, deterministic software perf-probe plus reusable perf-harness CTest coverage, a named local macOS Release software/Metal perf baseline, and package dry-run coverage.
...
See `git log v0.2.5-dev..HEAD -- .` for the full 234-commit history since v0.2.5.
