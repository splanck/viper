...
**Camera3D.** Input sanitizers for aspect, near/far clip, FOV, and ortho size catch pathological values before they hit the projection builder. `look_at` with degenerate eye==target preserves the camera position instead of returning identity. `Canvas3D.Begin` uses the active output aspect when building that frame's projection, and `ScreenToRay` uses the shaken render pose during camera shake so picking matches the visible image. A second sanitization pass extended the same `finite_or` / `sanitize_nonnegative` / `sanitize_vec3` helpers across every `LookAt` / `SetPerspective` / `SetOrthographic` entry point: NaN / infinity / zero-length eye/target/up inputs now fall back to identity values (eye=origin, target=-Z, up=+Y) instead of quietly producing a degenerate view matrix that poisons downstream projection, frustum-culling, and shadow-pose derivation.

**Canvas3D.** `Begin2D` caches the full V·P (matched to `Begin3D`) so overlay unprojection has consistent semantics. Camera params now carry world-space forward and an `is_ortho` flag — backends specialize fog / rim / specular per mode. Cleanup paths on `Canvas3D.New` / `RenderTarget3D.New` release partially initialized wrappers on failure. New IL-bound capability-introspection surface — `Canvas3D.BackendCapabilities` returns an i64 bitmask and `Canvas3D.BackendSupports(str)` returns i1 — covering nine capability bits (SOFTWARE, GPU, RENDER_TARGET, WINDOW_READBACK, SHADOWS, SKYBOX, HARDWARE_INSTANCING, POSTFX, GPU_POSTFX). Capabilities are derived from backend-vtable hook presence (e.g. `shadow_begin`+`shadow_draw`+`shadow_end` implies SHADOWS; `submit_draw_instanced` implies HARDWARE_INSTANCING) so a new backend automatically reports exactly what it actually implements. `canvas3d_capability_from_name` accepts aliased strings (`postfx`/`post_fx`, `readback`/`screenshot`/`window_readback`, `instancing`/`hardware_instancing`, etc.) so frontends can query in whichever form reads naturally. The `SetOcclusionCulling` entry — which historically only did coarse frustum rejection + front-to-back opaque ordering — has been renamed to `SetFrustumCulling` (with `SetOcclusionCulling` kept as a thin forwarding alias for backwards compatibility); the old name overpromised Hi-Z / query-based occlusion culling that the runtime never actually did. Normal-map draws now pick up tangents automatically: `rt_canvas3d_draw_mesh_matrix_keyed` and `rt_canvas3d_queue_instanced_batch` both call `canvas3d_ensure_normal_map_tangents` before submit, which scans the mesh for a non-degenerate tangent vector and, when none exists and a normal map is bound, derives tangents from positions/UVs via `rt_mesh3d_calc_tangents` — so glTF/OBJ meshes that omit tangent attributes no longer render with the normal map silently inactive. Canvas input sanitization: `set_fog` clamps RGB to [0, 1], forces non-negative near/far, and guarantees `far > near + 1e-4` so fog-attenuation math can't produce NaN or divide-by-near-zero; `set_shadow_bias` falls back to 0.005 on NaN input and clamps negatives; `rt_canvas3d_poll` mouse updates in both the relative-motion and non-captured branches route through `rt_canvas3d_update_mouse_from_physical` so HiDPI physical→logical scaling stays consistent across the input path.
...
**glTF / Model3D.** glTF mesh extraction now handles `COLOR_0`, `TANGENT`, `JOINTS_0`, and `WEIGHTS_0` in addition to positions, normals, and UVs. Triangle strips/fans are triangulated correctly. Matrix-authored node transforms are decoded in the right column-major order. Tangents are synthesized when a normal-mapped primitive omits them. `test_rt_model3d` is back in CTest; the demo-FBX texture case self-skips when that optional asset is not present in the checkout. The loader also grew a full skin + animation pipeline: a new `gltf_skin_t` scratch table walks every `skins[]` entry, builds a `Skeleton3D` from the joint hierarchy (two-pass — root joints first, orphans promoted on a second pass), and transposes the column-major `inverseBindMatrices` accessor into the engine's row-major bone storage. `gltf_apply_skin_to_mesh` retargets per-vertex glTF joint indices through `joint_to_bone[]` onto engine bone indices, drops influences whose bone mapping failed, and renormalizes the surviving weights. `gltf_parse_animations` flattens the channel/sampler graph into bone-oriented `Animation3D` keyframes: collects unique de-duplicated time stamps per bone (1e-6 tolerance), seeds T/R/S from the node's bind pose so bones missing a channel keep their rest value, and samples LINEAR and STEP interpolations natively. CUBICSPLINE samplers are sampled as LINEAR between the value members of each `[in-tangent, value, out-tangent]` triple (full Hermite support is a future improvement); rotation curves are renormalized after blend so linear quaternion interpolation doesn't drift below unit length on long playback.
...
**Mesh3D.** New `build_failed` flag on `rt_mesh3d` lets callers bail out cleanly after a load or procedural-build failure instead of silently accepting a degenerate mesh. Every `mesh_mark_build_failed` site now traps with a descriptive message rather than silent return. `AddVertex` rejects non-finite position/normal/UV attributes; `AddVertex` and `AddTriangle` trap on capacity overflow and realloc failure; `AddTriangle` rejects degenerate triangles (`v0==v1 || v1==v2 || v0==v2`). The procedural builders gained matching validation: `NewBox` requires each half-extent finite and > 0; `NewSphere` requires radius finite > 0 and segments ≤ `MESH_MAX_SPHERE_SEGMENTS` (512, ~260K-vertex ceiling); `NewPlane` requires each size finite > 0; `NewCylinder` requires radius/height finite > 0 and segments ≤ `MESH_MAX_CYLINDER_SEGMENTS` (8192). All limits trap with `"segments exceeds safe limit"` instead of wrapping into uint32 overflow territory. The OBJ loader grew a hash-based `obj_vertex_cache_t` that de-duplicates shared `(position, UV, normal)` index triplets across faces — previously a mesh with lots of shared verts (typical glTF-exported-to-OBJ) emitted one vertex per face corner, inflating the vertex buffer 3–6×. The cache uses the same FNV-1a + golden-ratio mixer standardized across the runtime's other content-addressable caches and grows on demand via power-of-two doubling. `rt_mesh3d_clone` copies `build_failed` alongside the rest of mesh state so clones preserve the parent's validity marker.
...
**FBX / Model3D.** The FBX loader now exports a real scene-node root instead of only loose mesh/material arrays, and `Model3D.Load(.fbx)` clones that authored `Model` hierarchy into its template tree. Local TRS, parent/child grouping, mesh attachments, and material bindings now survive FBX import. The FBX asset finalizer also releases owned meshes, materials, skeletons, animations, morph targets, and the scene root correctly instead of leaking them. Matching the new glTF path, FBX also picks up real per-vertex skinning: `fbx_skin_influence_t` selects the top-4 `(bone, weight)` pairs per control point (merging duplicate entries, replacing the weakest when full); `fbx_mesh_remap_t` carries the control-point-to-mesh-vertex fan-out so hard-edge split vertices inherit the same influences as the control point they share; and `fbx_apply_control_skin_to_vertex` renormalizes the surviving weights and updates the mesh's running `bone_count` high-water mark so the skinning palette is sized correctly. `fbx_find_skin_geometry` walks the FBX connection table OO-up from a `Deformer(Skin)` to its owning `Geometry(Mesh)`.

**Backends.** Morph-handoff surface unified across D3D11, Metal, OpenGL, and the software rasterizer — backends consume the shared packed-delta buffer directly. PBR-aware alpha-blend gate replaces the old "always opaque" default for sub-1.0 alpha materials. OpenGL texture and cubemap uploads flip top-left-origin pixel rows before upload to match the other backends. Metal texture readback (render-target sync, framebuffer present, GPU `Screenshot` / postfx readback) now uses a `BlitCommandEncoder` copy into a shared-storage `MTLBuffer` instead of the synchronous `[MTLTexture getBytes:]` call; the staging row pitch is padded up to the 256-byte Apple Silicon alignment requirement and the command buffer is waited-on with a `MTLCommandBufferStatusError` check so a GPU encoder failure returns `0` instead of producing uninitialized pixels. BGRA8Unorm and RGBA16F go through the same path, so the HDR readback no longer needs a separate malloc'd intermediate row buffer. Size arithmetic guards (`w * 4`, `stride * h`, `copy_w * bpp`, `copy_h * bytes_per_row`) reject pathological inputs before they can wrap `SIZE_MAX`. The Metal postfx chain gained a dedicated `postfxScratchTexture` (`MTLStorageModePrivate`, same format/usage as `postfxColorTexture`); the ping-pong loop now alternates between `postfxColorTexture` and the scratch target instead of aliasing `offscreenColor` as the scratch — `offscreenColor` is the scene-input texture and must not be simultaneously bound as a render target while the first pass is sampling it. The "no active postfx" branch takes a single identity pass (source → scratch) so the downstream present/readback path can always read from a known non-scene target instead of nil, and preconditions are tightened to require the new scratch handle alongside the existing targets. The software rasterizer got three independent upgrades: `sw_sample_shadow_visibility` now performs a 3×3 percentage-closer filter (nine taps clamped to the shadow-map bounds) and averages visibility — the old binary `{0.15, 1.0}` decision produced hard-edged aliased silhouettes; `pipe_vert_t` was hoisted into the context and `sw_acquire_pipe_vertices` hands out a growable scratch buffer shared across draws, eliminating per-draw malloc noise and a failure point; and new `sw_srgb_to_linear` + `sample_texture_srgb` helpers sRGB-decode sampled texels before shading math so albedo-weighted lighting produces correct HDR-before-tonemap intensities, matching how GPU backends already treat sampled textures.

**Lights / PostFX.** Color/intensity init and finalizer plumbing so lights are safely retained / released through scene-graph mutations. A final sanitization pass hardened every `Light3D` setter against NaN/inf inputs — color channels go through `clamp01`, position components go through `finite_or_zero`, and spot angles are clamped to `[0, 89°]` via `sanitize_spot_angle` so degenerate cone math can't reach the shader. `PostFX3D` got the matching treatment: `sanitize_f32` / `sanitize_nonnegative_f32` / `clamp_i64_to_i32` across bloom (threshold / intensity / blur-pass count), tonemap (mode / exposure), color grade (brightness / contrast / saturation), vignette (radius / softness), and SSAO (radius / intensity / sample count). Caller code that passes along an unchecked `Vec3` / `Float` / `Integer` (from config parsing, scripted camera shake, data-driven UI sliders, etc.) can no longer propagate a NaN into the render state.
