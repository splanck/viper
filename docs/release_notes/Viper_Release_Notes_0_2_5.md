...
| Production SLOC | 450K | 484K | +34K |
| Test SLOC | 183K | 196K | +13K |
...
**glTF / Model3D.** glTF mesh extraction now handles `COLOR_0`, `TANGENT`, `JOINTS_0`, and `WEIGHTS_0` in addition to positions, normals, and UVs. Triangle strips/fans triangulate correctly. Matrix-authored node transforms decode in the right column-major order. Tangents are synthesized when a normal-mapped primitive omits them. `test_rt_model3d` is back in CTest; the demo-FBX texture case self-skips when that optional asset is not present in the checkout.
...
- LINEAR and STEP interpolations are sampled natively. CUBICSPLINE is sampled as LINEAR between the `[in-tangent, value, out-tangent]` middle elements (full Hermite is a future improvement). Quaternion curves are renormalized after blend so long playbacks don't drift below unit length.
