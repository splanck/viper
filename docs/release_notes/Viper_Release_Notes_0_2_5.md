...
**glTF / Model3D.** glTF mesh extraction now handles `COLOR_0`, `TANGENT`, `JOINTS_0`/`WEIGHTS_0`, and `JOINTS_1`/`WEIGHTS_1` in addition to positions, normals, and UVs; secondary joint sets are reduced to the strongest four supported influences. Morph targets now import `TANGENT` deltas and route those shapes through the CPU path until every GPU backend carries tangent-delta payloads. `KHR_lights_punctual` directional, point, and spot lights import onto their authored scene nodes and are included in `Scene3D.Draw` light snapshots after node-world transform; imported directional lights also participate in shadow selection from queued draw snapshots, and VSCN save/load preserves those node-attached lights. `extensionsRequired` is enforced so required compression or exact advanced-material extensions fail load instead of rendering incomplete fallback data. Triangle strips/fans triangulate correctly. Matrix-authored node transforms decode in the right column-major order. Tangents are synthesized when a normal-mapped primitive omits them. `test_rt_model3d` is back in CTest; the demo-FBX texture case self-skips when that optional asset is not present in the checkout.
...
*Graphics3D P0 hardening.* Scene3D culling now extracts its frustum from the canvas's active output projection, so window/RTT aspect changes no longer cull against a stale camera aspect. Deferred draw queue and sort scratch allocation failures degrade to direct unsorted visible submission instead of dropping geometry. HDR render-target readback now preserves a linear RGBA32F CPU mirror alongside the tonemapped `Pixels` mirror, and CPU-supported render-target postfx consumes that HDR mirror before final 8-bit conversion. SSAO, DOF, and motion blur now require GPU window postfx explicitly and raise a clear runtime trap on render-target/software CPU paths instead of silently no-oping.
...
**Graphics3D production-readiness pass.** `Canvas3D.Clear` and `SetAmbient` now clamp RGB inputs before backend submission, `Begin` applies `SetDTMax` to camera shake time steps, default backface culling is enabled, and deferred hardware-instanced draws snapshot their instance matrices so caller-side buffer mutation cannot corrupt queued submissions. `Mesh3D.AddTriangle` now fails fast on negative, out-of-range, or degenerate indices, and degenerate-UV tangent generation produces a normalized tangent orthogonal to the vertex normal. `Physics3DWorld` body/contact/event/joint storage now grows past the old fixed initial capacities, broadphase collision uses sweep-and-prune before narrow-phase tests, CCD substeps allow up to 64 slices for fast bodies, and friction/restitution setters sanitize invalid values.
