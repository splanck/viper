...
- **Zanna.Localization.* (new).** Eleven-class namespace for locale-aware number/date/time/list formatting, translation catalogs, CLDR plural selection, and text-direction utilities. Zero external dependencies.
...
- **Compiler throughput.** Parallel IL optimizer and codegen (hardware-concurrency thread pool), zero-copy codegen pipeline, analytical AArch64 instruction sizing, bitset-backed liveness, fast-link mode. `zanna run` skips optimization by default for fast script invocations.
...
Eleven-class `Zanna.Localization.*` namespace. Zero external dependencies; en-US built in, all other locales load from JSON at runtime.
...
- Windows ARM64: COFF ARM64 relocation constants, SEH stubs, `ExitProcess` startup import; build scripts default to host architecture with `--arch`/`ZANNA_DEMO_ARCH` override.
...
- Build profiles (`debug`/`balanced`/`release`) map to -O0/-O1/-O2; project manifests accept a `profile` directive; `zanna run` defaults to debug profile for fast script invocations while respecting project-level overrides.
...
Six-pass hardening of the `zanna install-package` and `zanna package` subsystems.

- **Cross-platform installer correctness** — Windows uninstaller removes all registry entries it created. Existing `Content Type` values for extensions are preserved; only ZAPS-owned MIME values are tagged on install and removed on uninstall. ProgID trees are tagged with a ZAPS owner marker and only deleted when that marker is present, preventing `zanna install` from silently overwriting another application's file associations. Duplicate `file-assoc` extensions in a project manifest are now rejected at validation time.
...
- **Windows toolchain PATH and file-association management** — The toolchain installer adds the Zanna `bin` directory to the system `Path` environment variable as a `REG_EXPAND_SZ` entry; on reinstall, existing Zanna-owned PATH entries are deduplicated rather than appended. `SendMessageTimeoutW(WM_SETTINGCHANGE)` is broadcast after each PATH modification so running processes pick up the change without a reboot. On uninstall, only the Zanna-owned PATH token is removed. Source (`.zia`) and IL (`.il`) file associations are registered during toolchain install with `zanna run` and `zanna -run` open commands respectively.
...
- **PNG full color-type support in package assets** — Asset packager now handles PNG color types 0 (greyscale), 2 (RGB), 3 (indexed), 4 (greyscale+alpha), and 6 (RGBA); previously only type 6 was decoded correctly for ZPAK asset packs.
- **ZIP header cross-check** — `zanna package` verifies that the central-directory entry for each asset matches the local-file header (compression method, CRC, sizes) and rejects the archive if any field diverges — catches truncated or partially-written asset packs before distribution.
...
- **macOS toolchain package hardening** — `zanna install-package --target macos` validates that the staged tree's platform matches before invoking `pkgbuild`; `--macos-pkg-version` accepts a dotted-numeric override for cases where the Zanna version string has pre-release suffixes that `pkgbuild` cannot accept. `/usr/local/bin` symlinks are now embedded directly in the package root so the postinstall script no longer needs to create them.
- **Mach-O `__LINKEDIT` correctness** — Native Mach-O output no longer pads the final `__LINKEDIT` segment past `LC_CODE_SIGNATURE`, allowing Apple `strip` and `codesign --force` to process Zanna-generated arm64 binaries and enabling bundle-level signing.
...
- **Platform and architecture auto-detection** — `zanna install-package` now inspects the staged `zanna` binary's ELF, PE, or Mach-O header to derive platform and architecture automatically. A `--arch` override that contradicts the detected binary architecture is rejected. `--target all` includes RPM only when `rpmbuild` is available on the host.
...
- **IL, compiler, and audio** — New tests for linker correctness, opcode metadata consistency, optimizer passes (DSE, LICM, LoopRotate, LoopUnroll, IndVarSimplify, MemorySSA, ValueKey), verifier escape-analysis golden fixtures, and IL transform edge cases. Audio tests cover PCM overflow, ALSA write simulation, and WAV format variants. Localization fuzzing harnesses added (opt-in via `ZANNA_ENABLE_FUZZ`).

### Demos & docs

- **Demos** — New text-mode baseball franchise simulator; Crackman rewrite; two new 3D demos; Paint gains layers and undo; ZannaIDE file-watcher and context-menu fixes; XENOSCAPE boss/player fixes and macOS package; Chess drag-vs-click detection fix; three localization examples; Windows ARM64 builds for all 3D demos.
- **Docs** — `zannalib/` coverage extended across all subsystems: 2D graphics split into rendering/effects, tilemaps/layers, shapes/text/UI, and animation/collision/camera; new `zannalib/localization/`; IL Optimizer Correctness Contract; GUI zannalib updated through round-7 APIs. Doxygen/`@brief` coverage pass across all 42 `lib/gui` source files, ~100 runtime files, and every Crypto/Network/Text/Packaging runtime module; all source files received canonical Zanna two-separator file headers.
