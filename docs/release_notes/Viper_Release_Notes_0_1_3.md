...
- `Viper.String` — Added `Split`, `Replace`, `Repeat`, `PadLeft`, `PadRight`, `StartsWith`, `EndsWith`
- `Viper.Math` — Added `Atan2`, `Hypot`, `Log2`, `Log10`, `Clamp`, `Lerp`, `Round`, plus `Pi`, `E`, `Tau` constants
- `Viper.IO.File` — Added `Append`, `Copy`, `Move`, `Size`, `Modified`, `ReadAllLines`, `ReadAllBytes`
- `Viper.Environment` — Added `GetVariable`, `SetVariable`, `HasVariable`, `IsNative`
- `Viper.Collections.List` — Added `Find`, `Has`, `Insert`, `Remove`
...
```cmake
VIPER_RUNTIME_NS_DUAL       # Enable dual namespace mode (rt_* and Viper.*)
...
- **Runtime Library**: See `docs/viperlib/` for API reference
- **Examples**: Browse `demos/` for sample programs
- **License**: See LICENSE file

---

*Viper Compiler Platform v0.1.3 (Pre-Alpha)*  
