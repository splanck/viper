...
# ViperLang — Getting Started

Learn ViperLang by example. For a complete reference, see **[ViperLang Reference](viperlang-reference.md)**.
...
Create a file named `hello.viper`:
...
ViperLang supports generic collections from the runtime library.
...
Import other `.viper` files to use their types and functions:

```viper
module Game;

import "./entities";    // imports entities.viper from same directory
import "./utils";       // imports utils.viper

func start() {
    var player = new Player();  // Player defined in entities.viper
    player.init("Hero");
}
```
...
- `"./foo"` — Relative path, adds `.viper` extension
- `"../bar"` — Parent directory relative path
- `"foo"` — Same directory, adds `.viper` extension

---

## 9. Working with the Runtime

ViperLang programs have access to the full Viper Runtime Library.
...
Here's a complete mini-game demonstrating ViperLang features:
...
For more complete examples, see the `demos/viperlang/` directory:
- `frogger.viper` — Full Frogger game with entities and collision detection
- `entities.viper` — Entity system with multiple types
...
- [ViperLang Reference](viperlang-reference.md) — Complete language specification
- [Runtime Library](viperlib/README.md) — All available classes and methods

**Examples:**

- `demos/viperlang/frogger.viper` — Complete game example
