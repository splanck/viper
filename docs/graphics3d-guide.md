...
The OpenGL backend (Linux) now implements OGL-01 through OGL-16: diffuse/normal/specular/emissive textures, vertex-color modulation, inverse-transpose normal matrices, spot lights, fog, wireframe mode, render-to-texture readback, shadow mapping, GPU post-processing, hardware instancing, GPU skinning + morph payload consumption, terrain splatting, and persistent dynamic buffers. The remaining follow-on gaps are now tracked separately in the backend plan set: backend-owned cubemap skyboxes, material environment reflections, GPU morph normal-delta parity, and depth/history-based GPU postfx additions such as SSAO, DOF, and motion blur.
