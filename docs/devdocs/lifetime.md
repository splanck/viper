...
Viper BASIC uses deterministic lifetimes with reference counting and explicit disposal. This section summarizes the model and best practices.
...
- Use `DISPOSE obj` to deterministically destroy an instance when it goes out of scope or when you finish with it earlier.
...
- A `STATIC DESTRUCTOR` runs once at program shutdown. Use this for module-level cleanup (e.g., releasing global resources).
- Shutdown order is class declaration order within the module.

## Guidance

- Prefer `DISPOSE` when you want timely cleanup of scarce resources (file handles, OS objects) and ordering relative to surrounding code matters.
