# Zia Complete Implementation Plan
...
Zia is a modern, safe programming language targeting the Viper IL and runtime. This document provides a verified implementation plan aligned with actual platform capabilities.
...
|-------|------------------|-----------------|
| 1 | `ex01_hello.zia` | Basic parsing, print |
| 2 | `ex02_point.zia` | Values, fields |
| 3 | `ex03_counter.zia` | Entities, methods |
| 4 | `ex04_fizzbuzz.zia` | Control flow, loops |
| 5 | `ex05_calculator.zia` | Error handling, Result |
| 6 | `ex06_parallel.zia` | Threads (optional) |
| 7 | `ex07_inventory.zia` | Collections |
| 8 | `ex08_shapes.zia` | Interfaces, inheritance |
| 9 | `ex09_fileio.zia` | Standard library |
| 10 | `frogger.zia`, `vtris.zia` | Full demos |
...
The Viper runtime already provides these APIs. Zia should expose them naturally.
...
**List (recommended for Zia `List[T]`):**
...
Before implementing Zia, these runtime components must be created.
...
**Golden Tests:**
```viper
// tests/golden/phase1/hello.zia
func start() {
    print("Hello, Zia!");
}
```
Expected output: `Hello, Zia!`

**Example Program - ex01_hello.zia:**
```viper
// First Zia program
func start() {
    print("Hello, World!");
...
**Example Program - ex02_point.zia:**
```viper
value Point {
    Number x;
    Number y;
...
**Example Program - ex03_counter.zia:**
```viper
entity Counter {
    Integer count = 0;
...
**Example Program - ex04_fizzbuzz.zia:**
```viper
func start() {
    for (i in 1..=100) {
        if (i % 15 == 0) {
...
**Important:** Zia v0.1 uses thread-based concurrency, NOT async/await.
...
| `Mutex` | Zia entity; calls `Monitor.*(self)` | Phase 6 |
...
The final validation phase: implement complete, playable games that demonstrate Zia's full capabilities. These are ports of existing BASIC demos, proving Zia can handle real-world applications.
...
|---------|-------|--------|-----------------|
| `ex01_hello.zia` | 1 | [x] Complete | Print, entry point |
| `ex02_point.zia` | 2 | [ ] | Values, entities, fields |
| `ex03_counter.zia` | 3 | [ ] | Methods, state |
| `ex04_fizzbuzz.zia` | 4 | [ ] | Loops, conditionals |
| `ex05_calculator.zia` | 5 | [ ] | Result, ? operator |
| `ex06_parallel.zia` | 6 | [ ] | Threads (optional) |
| `ex07_inventory.zia` | 7 | [ ] | List, Map operations |
| `ex08_shapes.zia` | 8 | [ ] | Interfaces, polymorphism |
| `ex09_fileio.zia` | 9 | [ ] | File read/write |
| `frogger.zia` | 10 | [ ] | Full game |
| `vtris.zia` | 10 | [ ] | Full game |
...
The Zia entry point is `func start()`:
...
```
func @start() -> void {
    call void @Viper.IO.Print("Hello, Zia!")
    ret
}
```
...
| 2.3 | Dec 2024 | Phase 1 complete: zia CLI tool created, hello.zia runs successfully, 9 compiler unit tests passing |
...
**Final Goal:** Frogger and vTRIS demos running in Zia
