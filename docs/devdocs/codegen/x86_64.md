...
5. Update the selection tests (e.g., `tests/codegen/x86_64/*`) with a focused case that exercises the new path
   end-to-end.
...
- Build with `VIPER_MIR_DEBUG=1` (or equivalent flag) to dump intermediate MIR, including ParallelCopies and allocation
  decisions.
