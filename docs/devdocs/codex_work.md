...
## Objectives
- Complete runtime sweep coverage (Zia + BASIC) and update `bugs/runtime_test.md`.
- Log runtime/Zia bugs discovered during the sweep.
...
## Runtime Sweep Plan (Zia + BASIC)

### Goals
- Exercise every Viper.* runtime class, property, and method.
- Prefer Zia; fall back to BASIC when Zia bindings are missing/buggy.
- Stress test language features and runtime behavior in realistic programs.
- Record all runtime defects in `bugs/runtime_bugs.md` and all Zia defects in `bugs/zia.md`.
- Track coverage in `bugs/runtime_test.md` and update as tests run.

### Plan
1) **Harness + Coverage**
   - Build a runner that executes Zia and BASIC test programs.
   - Parse `COVER:` tags in tests to tick `bugs/runtime_test.md`.
   - Capture stdout/stderr and exit code for report + bug logging.

2) **Zia Tests**
   - Create a suite of Zia programs to stress the language and available runtime bindings.
   - Validate via output checks (no diagnostics/assert runtime in Zia).
   - Log any missing runtime bindings as Zia bugs.

3) **BASIC Tests**
   - Build subsystem-focused BASIC programs to cover runtime classes not reachable from Zia.
...
### 5) Bugs + Report
- [x] Update `bugs/runtime_bugs.md`, `bugs/zia.md`, and `bugs/basic_bugs.md` with findings.
...
- 2025-12-22: Updated runtime/zia/basic bug logs and refreshed runtime test matrix timestamp.
- 2025-12-22: Rebuilt toolchain and reran runtime sweep (Zia + BASIC).
...
- 2025-12-22: Runtime sweep re-run (Zia + BASIC) passed; updated Fmt.Size sweep expectation and utilities docs example output.
...
- 2025-12-21: Updated Zia parser/sema disambiguation (`?` vs ternary, named args, value-type `new`).
- 2025-12-21: Updated docs for Zia reference, viperlib IO/input, and generated-files guidance.
...
- 2025-12-21: Created runtime test matrix (`bugs/runtime_test.md`) and initialized bug logs (`bugs/runtime_bugs.md`, `bugs/zia.md`).
