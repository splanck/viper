...
edge-claiming `DockPanel` layouts. (`Viper.GUI.Grid` is the interactive tabular data control, not a
...
|-------|-----------------------|
| `Viper.GUI.Align` | `Start=0`, `Center=1`, `End=2`, `Stretch=3` |
| `Viper.GUI.Justify` | `Start=0`, `Center=1`, `End=2`, `SpaceBetween=3`, `SpaceAround=4`, `SpaceEvenly=5` |
| `Viper.GUI.FlexDirection` | `Row=0`, `Column=1`, `RowReverse=2`, `ColumnReverse=3` |
| `Viper.GUI.FlexWrap` | `NoWrap=0`, `Wrap=1`, `WrapReverse=2` |
| `Viper.GUI.Dock` | `Left=0`, `Top=1`, `Right=2`, `Bottom=3`, `Fill=4` |

### Flex

`Viper.GUI.Flex.New()` creates a wrapping flexbox-style container. All distances are logical units;
...
`Viper.GUI.LayoutGrid.New()` creates a fixed/auto/fractional two-dimensional layout. Positive track
...
`Viper.GUI.DockPanel.New()` consumes the remaining rectangle one edge at a time. `DockChild` may
...
| `DockChild(child, dock)` | `Boolean(Object, Integer)` | Attach/update one child with a `Viper.GUI.Dock` value |
