...
`Viper.GUI.Theme` selects the active app's appearance. `Viper.GUI.ThemePalette`
...
[Theme](../../generated/runtime/gui.md#viper-gui-theme) and
[ThemePalette](../../generated/runtime/gui.md#viper-gui-themepalette)
references.
...
- font roles are null or live `Viper.GUI.Font` handles; managed role handles are retained by the
...
`Validate()` returns `Viper.Result.Ok(1)` on success or `Err(String)` on
...
bind Viper.GUI.ThemePalette as ThemePalette;
...
if checked.IsOk() {
    if !Theme.SetPalette(palette) {
        Viper.Terminal.Say("The custom theme could not be installed");
    }
}
