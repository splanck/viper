...
ViperIDE toolbar buttons were using single-character labels as implicit icons.
...
IDE code depend on ambiguous glyph literals and prevented other Viper programs
...
- `Viper.GUI.Toolbar.AddNamedButton(name, tooltip)`
- `Viper.GUI.Toolbar.AddNamedButtonWithText(name, text, tooltip)`
- `Viper.GUI.Toolbar.AddNamedToggle(name, tooltip)`
- `Viper.GUI.ToolbarItem.SetNamedIcon(name)`
...
ViperIDE can migrate away from toolbar text glyphs without adding external image
