...
This page walks through the “first seconds” of ViperOS: how the CPU enters the kernel, how `kernel_main()` brings up the
...
ViperOS targets QEMU’s `virt` machine and uses virtio-mmio for devices. `kernel_main()` runs:
...
This is a good example of ViperOS’ bring-up philosophy: simple global tables first, then per-process/per-task
refactoring later.
