...
void check_directory(u64 ino, const std::string &path)
{
    ...
    while (pos < inode.size && pos < BLOCK_SIZE)
    {
        ...
        if (entry->inode != 0)
        {
            ...
            bool is_dot_entry = (strcmp(name, ".") == 0) ||
                               (entry->name_len == 1 && entry->file_type == file_type::DIR && entry->inode == ino);
            bool is_dotdot_entry = (strcmp(name, "..") == 0) ||
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (entry->inode == ino)
                        {
                            report_error("Directory %s has self-referencing entry '%s'",
                                         path.c_str(), name);
                        }
                        else
                        {
                            ...
                            {
                                verbose_log("  File: %s (inode %lu)", child_path.c_str(), entry->inode);
...
bool check_superblock()
{
    ...
    verbose_log("  Bitmap: blocks %lu-%lu", sb.bitmap_start, sb.bitmap_start + sb.bitmap_blocks - 1);
...
bool load_inodes()
{
    ...
    for (u64 i = 0; i < sb.inode_count; i++)
    {
        if (inodes[i].inode_num != 0)
        {
            ...
            {
                report_warning(
                    "Inode at index %lu has inode_num %lu", i, inodes[i].inode_num);
            }
        }
    }

    printf("  Loaded %llu inodes (%llu allocated)\n", (unsigned long long)sb.inode_count, (unsigned long long)allocated);
    return true;
}
...
void check_block_allocation()
{
    ...
    if (mismatch_used > 10)
    {
        fprintf(stderr, "  ... and %llu more used-but-free errors\n", (unsigned long long)(mismatch_used - 10));
    }
    if (mismatch_free > 10)
    {
        fprintf(stderr, "  ... and %llu more unreferenced blocks\n", (unsigned long long)(mismatch_free - 10));
    }
    ...
    {
        report_warning("Superblock free_blocks=%lu but counted %lu free", sb.free_blocks, actual_free);
    }
...
int main(int argc, char **argv)
{
    ...
    printf("Image size: %llu bytes (%llu blocks)\n\n", (unsigned long long)file_size, (unsigned long long)(file_size / BLOCK_SIZE));
