...
- **ID**: VL-001
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:1118` declares `Viper.String.LastIndexOf` but there is no corresponding `RT_FUNC` in `runtime.def`.
...
- **ID**: VL-002
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:1119` declares `Viper.String.Contains`, but runtime binds to `Viper.String.Has` at `runtime.def:StrHas`.
...
- **ID**: VL-003
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:1125-1126` declares `TrimLeft`/`TrimRight`, but runtime binds to `TrimStart`/`TrimEnd`.
...
- **ID**: VL-004
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:1128` declares `ReplaceAll`, but runtime's `Replace` already replaces all occurrences.
...
- **ID**: VL-005
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:1132` declares `Reverse`, but runtime binds to `Flip`.
...
- **ID**: VL-006
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:1137` declares `IsBlank` with no runtime implementation.
...
- **ID**: VL-007
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:1138` declares `Compare`, but runtime binds to `Cmp`.
...
- **ID**: VL-008
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:1112` declares `CharAt` with no runtime implementation.
...
- **ID**: VL-009
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:1135` declares `Format` with no runtime implementation.
...
- **ID**: VL-010
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:1130` declares `Viper.String.Join`, but runtime binds to `Viper.Strings.Join`.
...
- **ID**: VL-011
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:1136` declares `Viper.String.IsEmpty`, but runtime binds property getter to `Viper.String.get_IsEmpty`.
...
- **ID**: VL-012
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:777` declares `Seq.Add`, but runtime binds to `Seq.Push` at `runtime.def:SeqPush`.
...
- **ID**: VL-013
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:796` declares `TreeMap.Remove`, but runtime binds to `TreeMap.Drop` at `runtime.def:TreeMapDrop`.
...
- **ID**: VL-014
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:958-967` declares different names than runtime:
...
- **ID**: VL-015
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:963-965` declares `Path.Exists`, `Path.IsFile`, `Path.IsDir`, but runtime only has `File.Exists` and `Dir.Exists`.
  - **Fix**: Either implement in runtime or remove from sema.

- **ID**: VL-016
  - **Area**: sema (Runtime Function Registry)
  - **Summary**: Multiple `Viper.IO.Dir.*` name mismatches and missing functions.
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:944-953` declares:
...
- **ID**: VL-017
  ...
  - **Root Cause**: `src/frontends/viperlang/Sema.cpp:928-939`:
