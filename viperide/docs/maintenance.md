# ViperIDE Maintenance Guide

This guide explains how to change ViperIDE without making the app harder to
reason about. It is intentionally procedural: use the section that matches the
kind of change you are making.

## Maintenance Principles

ViperIDE is a real application, but it is still young enough that accidental
...
The quickest way to make ViperIDE feel broken is to add visible commands that
...
Scene files are the cautionary example. ViperIDE recognizes `.scene` and
...
## Adding Runtime API Used By ViperIDE
...
6. Add runtime tests.
7. Add ViperIDE probes when the IDE depends on the behavior.
...
- `Viper.System.Pty`: runtime PTY implementation.
- `Viper.GUI.OutputPane`: terminal-mode rendering/input capture.
...
- Run `zia_smoke_viperide_project_compile`.
...
When summarizing ViperIDE changes, be specific:
