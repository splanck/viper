...
| `zia_viperide_scm` | Git Source Control command layer and view model smoke. | `zia;viperide;scm` |
...
| `zia_viperide_console_search` | Output panel helpers, console behavior, search, and Quick Open ranking. | `zia;viperide;console;search;requires_display` |
...
| `scm_probe.zia` | Git command layer and Source Control view. |
...
| `console_search_probe.zia` | Output/console/search/Quick Open helpers. |
...
- Full visual scene editor behavior does not exist yet and therefore has no app
  tests.
- Source Control push/pull and complex path quoting are not deeply covered.
...
- Tool-panel virtualization and huge-output behavior need stronger UI stress
  coverage.
- Rich debugger watch/variable-tree UX is not present and not covered.
