...
The architecture subsystem provides low-level AArch64 support for the ViperDOS microkernel, targeting the QEMU `virt` machine. In microkernel mode, the kernel provides only essential services while user-space servers handle networking, filesystem, and drivers.
