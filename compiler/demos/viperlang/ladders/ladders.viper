...
var _canvas: Viper.Graphics.Canvas;
...
func initColors() {
    COLOR_BLACK = Viper.Graphics.Color.RGB(0, 0, 0);
    COLOR_WHITE = Viper.Graphics.Color.RGB(255, 255, 255);
    COLOR_RED = Viper.Graphics.Color.RGB(255, 0, 0);
    COLOR_GREEN = Viper.Graphics.Color.RGB(0, 255, 0);
    COLOR_YELLOW = Viper.Graphics.Color.RGB(255, 255, 0);
    COLOR_GOLD = Viper.Graphics.Color.RGB(255, 215, 0);
    COLOR_DARK_GRAY = Viper.Graphics.Color.RGB(64, 64, 64);
    COLOR_LIGHT_GRAY = Viper.Graphics.Color.RGB(192, 192, 192);
    COLOR_BACKGROUND = Viper.Graphics.Color.RGB(20, 20, 40);
    COLOR_PLATFORM = Viper.Graphics.Color.RGB(100, 80, 60);
    COLOR_LADDER = Viper.Graphics.Color.RGB(180, 140, 80);
    COLOR_PLAYER = Viper.Graphics.Color.RGB(100, 200, 255);
    COLOR_ENEMY = Viper.Graphics.Color.RGB(255, 80, 80);
    COLOR_COLLECTIBLE = Viper.Graphics.Color.RGB(255, 215, 0);
    COLOR_MENU_BG = Viper.Graphics.Color.RGB(30, 30, 60);
    COLOR_MENU_HIGHLIGHT = Viper.Graphics.Color.RGB(100, 100, 200);
}
...
entity Platform {
    ...
    expose func draw(canvas: Viper.Graphics.Canvas) {
        ...
        // Top highlight
        drawLine(x, y, x + width - 1, y, Viper.Graphics.Color.RGB(140, 120, 100));
    }
}
...
entity Player {
    ...
    expose func draw(canvas: Viper.Graphics.Canvas) {
        ...
        if onLadder == 1 {
            if legAnim == 0 {
                drawLine(x + 4, y + PLAYER_HEIGHT - 3, x + 2, y + PLAYER_HEIGHT, Viper.Graphics.Color.RGB(70, 150, 200));
                drawLine(x + PLAYER_WIDTH - 4, y + PLAYER_HEIGHT - 3, x + PLAYER_WIDTH - 2, y + PLAYER_HEIGHT - 2, Viper.Graphics.Color.RGB(70, 150, 200));
            } else {
                drawLine(x + 4, y + PLAYER_HEIGHT - 3, x + 2, y + PLAYER_HEIGHT - 2, Viper.Graphics.Color.RGB(70, 150, 200));
                drawLine(x + PLAYER_WIDTH - 4, y + PLAYER_HEIGHT - 3, x + PLAYER_WIDTH - 2, y + PLAYER_HEIGHT, Viper.Graphics.Color.RGB(70, 150, 200));
            }
        } else {
            drawLine(x + 4, y + PLAYER_HEIGHT - 3, x + 3, y + PLAYER_HEIGHT, Viper.Graphics.Color.RGB(70, 150, 200));
            drawLine(x + PLAYER_WIDTH - 4, y + PLAYER_HEIGHT - 3, x + PLAYER_WIDTH - 3, y + PLAYER_HEIGHT, Viper.Graphics.Color.RGB(70, 150, 200));
        }
    }
}
...
func drawPlatforms(canvas: Viper.Graphics.Canvas) {
    var i = 0;
    while i < platformCount {
        var px = platformX.get(i);
        var py = platformY.get(i);
        ...
        drawLine(px, py, px + pw - 1, py, Viper.Graphics.Color.RGB(140, 120, 100));
        i = i + 1;
    }
}

func drawLadders(canvas: Viper.Graphics.Canvas) {
    var i = 0;
    while i < ladderCount {
        var lx = ladderX.get(i);
        var ly = ladderY.get(i);
...
func drawCollectibles(canvas: Viper.Graphics.Canvas) {
    var i = 0;
    while i < collectCount {
        if collectCollected.get(i) == 0 {
            var cx = collectX.get(i);
...
func drawEnemies(canvas: Viper.Graphics.Canvas) {
    var i = 0;
    while i < enemyCount {
        if enemyActive.get(i) == 1 {
            var ex = enemyX.get(i);
...
func drawPlayer(canvas: Viper.Graphics.Canvas) {
    ...
    var legColor = Viper.Graphics.Color.RGB(70, 150, 200);
...
func drawExit(canvas: Viper.Graphics.Canvas) {
    if collectedCount >= totalCollectibles {
        drawBox(exitX, exitY, 20, 24, COLOR_GREEN);
        drawFrame(exitX, exitY, 20, 24, Viper.Graphics.Color.RGB(100, 255, 100));
...
func drawHUD(canvas: Viper.Graphics.Canvas) {
    drawText(10, 10, "SCORE: " + Viper.Fmt.Int(score), COLOR_YELLOW);
    drawText(SCREEN_WIDTH / 2 - 50, 10, "HIGH: " + Viper.Fmt.Int(highScore), COLOR_GOLD);
    drawText(SCREEN_WIDTH - 100, 10, "LVL: " + Viper.Fmt.Int(currentLevel), COLOR_WHITE);
    ...
    if remaining > 0 {
        drawText(SCREEN_WIDTH - 120, 30, "ITEMS: " + Viper.Fmt.Int(remaining), COLOR_COLLECTIBLE);
    } else {
...
func drawTitleScreen(canvas: Viper.Graphics.Canvas) {
    // Background gradient
    var y = 0;
    while y < SCREEN_HEIGHT {
        var shade = 20 + y / 20;
        if shade > 60 { shade = 60; }
        drawLine(0, y, SCREEN_WIDTH, y, Viper.Graphics.Color.RGB(shade / 3, shade / 3, shade));
        y = y + 1;
    }
...
func drawPauseMenu(canvas: Viper.Graphics.Canvas) {
...
func drawGameOver(canvas: Viper.Graphics.Canvas) {
    ...
    drawText(boxX + 30, boxY + 60, "Score: " + Viper.Fmt.Int(score), COLOR_YELLOW);
    drawText(boxX + 30, boxY + 80, "Level: " + Viper.Fmt.Int(currentLevel), COLOR_WHITE);
    drawText(boxX + 30, boxY + 100, "High: " + Viper.Fmt.Int(highScore), COLOR_GOLD);
...
func drawLevelComplete(canvas: Viper.Graphics.Canvas) {
    drawBox(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, Viper.Graphics.Color.RGB(0, 50, 0));
    ...
    drawText(boxX + 30, boxY + 20, "LEVEL " + Viper.Fmt.Int(currentLevel) + " COMPLETE!", COLOR_GREEN);

    var levelScore = score - levelStartScore;
    drawText(boxX + 30, boxY + 60, "Level Score: " + Viper.Fmt.Int(levelScore), COLOR_YELLOW);
    drawText(boxX + 30, boxY + 80, "Bonus: " + Viper.Fmt.Int(SCORE_PER_LEVEL), COLOR_GOLD);
    drawText(boxX + 30, boxY + 100, "Total: " + Viper.Fmt.Int(score + SCORE_PER_LEVEL), COLOR_WHITE);
...
func updateGameplay(canvas: Viper.Graphics.Canvas) {
...
func main() {
    var canvas = new Viper.Graphics.Canvas(TITLE, SCREEN_WIDTH, SCREEN_HEIGHT);
    _canvas = canvas;
