...
_append(_out "//===----------------------------------------------------------------------===//")
_append(_out "//")
_append(_out "// Part of the Zanna project, under the GNU GPL v3.")
...
_append(_out "//")
_append(_out "// Links: docs/il/il-guide.md#reference, codegen/aarch64/AsmEmitter.cpp")
_append(_out "//")
_append(_out "//===----------------------------------------------------------------------===//")
_append(_out "")
...
_append(_out "{")
_append(_out "    if (op.kind != MOperand::Kind::Reg)")
_append(_out "        ZANNA_ICE(\"expected register operand in AArch64 generated emitter\");")
_append(_out "    if (!op.reg.isPhys)")
_append(_out "        ZANNA_ICE(\"virtual register v\" + std::to_string(op.reg.idOrPhys) +")
_append(_out "                  \" reached AArch64 generated emitter (register allocation bug)\");")
_append(_out "    return static_cast<PhysReg>(op.reg.idOrPhys);")
_append(_out "};")
_append(_out "auto imm = [](const MOperand &op)")
_append(_out "{")
_append(_out "    if (op.kind != MOperand::Kind::Imm)")
_append(_out "        ZANNA_ICE(\"expected immediate operand in AArch64 generated emitter\");")
_append(_out "    return op.imm;")
_append(_out "};")
_append(_out "")
...
foreach(_i RANGE 0 ${_LAST})
  ...
  elseif(_ENUM STREQUAL "AdrPage")
    _append(_out "    case K::${_ENUM}:")
    _append(_out "        emitAdrPage(reg(mi.ops[0]), mi.ops[1].label);")
    _append(_out "        break;")
    continue()
  elseif(_ENUM STREQUAL "AddPageOff")
    _append(_out "    case K::${_ENUM}:")
    _append(_out "        emitAddPageOff(reg(mi.ops[0]), reg(mi.ops[1]), mi.ops[2].label);")
    _append(_out "        break;")
    continue()
